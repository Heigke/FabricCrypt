...
## O30 oracle synthesis (2026-05-07)

**Top-3 to execute first** (consensus):
1. **I.1 Hetero-cell ratio** — both rank #1 for upside (1 day)
2. **I.3 Lateral inhibition** — both rank top-3, gemini's pick for FIRST (0.5 day, tractable in single wake-up)
3. **I.5 Programmable VG2 schedules** (gpt-5) OR **I.4 Spike readout** (gemini)

**Tonight's pick**: I.3. Reason: I.1 needs thick-ox surrogate first
(1-2 day ramp); I.3 is additive on existing z200 framework.

### Additions to plan from oracles

**Topology** (gpt-5): orthogonal/low-rank W, fractal/H-tree, E/I-balanced, ring-of-rings.
**Topology** (gemini): activity-dependent wiring (warm-up + prune).
**Plasticity** (both): Oja/BCM/STDP, anti-Hebbian, FORCE/RLS readout.
**Input encoding** (gpt-5): multiplicative VG2 + additive VG1, tapped-delay FIR, delta/event encoding.
**Input encoding** (gemini): non-linear projection (polynomial basis or RFF expansion).
**Readout** (both): gated/attention readout, temporal multi-tap pools.

### Chip-component priority (CONSENSUS)

1. **IV.2 per-cell programmable body-leak** — both #1. Direct τ control + erases latch-up. Critical reliability fix.
2. **IV.1 Active C-coupling** — adds short-term dynamics.
3. (gpt-5: IV.5 quasi-2D; gemini: IV.4 two-NPN)

### Killer benchmark (CONSENSUS)

**Streaming keyword spotting** (Google Speech Commands v2, 12-class). Always-on, <50 µJ/inference, <20 ms latency, noise-robust. Maps NS-RAM strengths (continuous-time, temporal memory, low power).

### Wildcard ideas (worth chip-spec amendment)

- **FDM virtualization** (gpt-5): K virtual reservoirs on one fabric via orthogonal VG2 carriers; demod in readout. Parallel tasks/timescales on one die.
- **Closed-loop edge-of-chaos bias servo** (gpt-5): on-chip activity monitor drives VG2/leak to maintain target spectral radius. Solves PVT drift + auto-tunes per task.
- **Computational spectrograph** (gemini): treat chip as bank of coupled non-linear oscillators tuned via body-leak. Encode input as VG2 frequencies, compute via resonance/mode-locking, readout in FFT domain. Reframes NS-RAM from RNN to programmable spectrograph.

These three converge on **frequency-domain computation** as a major
unexplored axis. Worth a Phase IV.6 spec for next chip:
"FDM-friendly chip: VG2 needs ≥ 100 kHz update rate; body-leak per-cell;
on-chip FFT or analog spectrum-readout block".
