...
## Plan revision after O29 oracle review (2026-05-07)

Both gpt-5 and gemini stamped "send-it" with the following changes:

### Week 1 sequencing (final)

```
Day 1 (immediate):    A.12 + G.2  (5-min sends, start Sebas clock)
Day 1-2 (parallel):   F7 + B.1    (independent ngspice runs)
Day 2-3 (parallel):   B.2 behind feature flag (synthetic-case unit tests)
                      No deps on B.1 outcome
Day 4+:               B.3 regularizer ONLY after B.1 confirms physical branch
                      (else regularizer "solves" a model bug)
```

### B.5 decision tree (revised, bootstrap CI)

Bootstrap median log-RMSE delta across 25 evaluated biases (1000
resamples; report 95% CI of `lumped_RMSE - quasi2d_RMSE`):

- **Win**: 95% CI lower bound > 0.02 dec → proceed to Plan C
- **Marginal**: point estimate in [0.02, 0.05] dec OR CI crosses 0
  → finish characterization but don't sink M6 schedule
- **Null**: 95% CI upper bound < 0.015 dec → declare null, pivot to D.1

This avoids chasing noise; 0.02 dec ≈ 5% log-RMSE which can be
eaten by cross-bias variance.

### Hygiene additions

| ID | Item |
|----|------|
| H.A | Pin ngspice version + record netlists/commits/seeds for all cross-vals |
| H.B | One Xyce or Spectre run on a "golden" bias as independence check |
| H.C | 3-point PVT corner sweep + 20-run mismatch Monte Carlo after B.5 |

### F.4 expansion

Add to hard-benchmark suite:
- Parity-N (temporal-XOR generalization, longer dependencies)
- Copy-memory task
- Santa-Fe laser chaotic forecast (regression)
- Noise-robustness sweep (SNR levels)
- Drift test (parameter perturbation)
- Temporal pattern recognition (simplified spoken-digit task) —
  gemini's suggestion: showcases NS-RAM memory aspect

### F.5 (NEW) — M9 fan-out DOE stub

0.5-1 day SPICE DOE: sweep M9 fan-out (FO=1/2/4), measure gain,
stability, power. Include in tape-out notes for next chip
fabrication. Run NOW (not post-M6) to de-risk M9 deliverable.

### G.3 expansion

Conference paper narrative selection — add sub-task: "Select 2-3
key network demonstrations (Mackey–Glass + Pavlovian + best
hard-bench result) to form the conference-paper core."

### Risk register (revised)

| Rank | Risk | Mitigation |
|------|------|------------|
| 1 | Sebas data delay (Plan E external) | Run Plan B/D in parallel; don't gate on E |
| 2 | Quasi-2D <0.02 dec gain (consumes 2-3 weeks) | Strict timebox to end of Week 3; CI-based decision; immediate D.1 pivot if marginal |
| 3 | Brief gets push-back from NRF panel | Already vetted by O26+O27+O28; have residual-diagnostic figure on standby |

### Cron strategy (final)

Replace `abd4f469` with three crons:

1. **`work_hours`**: every 73 minutes during 08:00–22:00 local
   (off-minute to avoid global thrash). Picks next non-blocked
   item from priority queue. ~12 fires/day.
2. **`daily_synthesis`**: 02:13 every day. Reviews log, pushes
   backup, re-prioritizes plan, idempotent (<10 min unless
   milestone flipped).
3. **`mid_week_risk`**: Thu 16:07. Specifically reviews risk
   register, flags any wait-state that's been blocked > 5 days.
4. **`weekly_review`**: Mon 09:23. Full plan review + status
   message to user.

Guardrails baked into wake-up handler:
- Per-task timebox; if > 40 min wall, abort and log
- 3 consecutive failures on same task → escalate (push notification
  to user)
- Queue states: plan / running / blocked(with reason) / done.
  Skip blocked tasks until the blocker is cleared
- Artifact drops each wake (env hash, ngspice version, seed)
- Don't start new compute if APU > 75°C OR if another long
  job is running (TaskList check)

7-day cron auto-expiry: a final `re_arm` self-cron fires Day 6 to
recreate the schedule.

---

**Verdict from oracles**: send-it.
**Verdict from me**: locking this in now.
