...
## O31 oracle synthesis (2026-05-07)

Both gpt-5 and gemini stamp "go" with these refinements:

### Track ordering (CONSENSUS)
- P first (CPU wrapper, NOT GPU port for Week 1).
- gpt-5 adds: start V pilot (6 seeds) IN PARALLEL day 1; pull S
  (CI utils) into day 1; do a 1-hour R sanity (N=16 pyport-direct)
  to catch surrogate artifacts early.

### Track P (refined)
- Hard env: `OMP_NUM_THREADS=1`, MKL/OPENBLAS/NUMEXPR=1; pin via
  `taskset`; 2-3 workers max; nice/ionice low; thermal pause ≥75°C,
  kill >90°C.
- GPU port deferred to 24-48 h after wrapper if compute-bound.

### Validation power (gpt-5's sequential design)
- After 6 paired seeds, compute σ_d (stdev of acc-delta).
- Required n ≈ (2.8·σ_d/0.05)². Examples: σ_d=0.03→n≈3, 0.05→n≈8,
  0.07→n≈16.
- Test set: 240/seed for tiny datasets, ≥1000/seed for SeqMNIST/KWS.
- **Paired tests on per-seed accuracies** (not 240×10 iid).

### Track C cost model (gpt-5's triangulation)
- Anchor: standard-cell LEF (sky130 inv/tgate areas as proxy for 130 nm).
- Bracket: transistor-count × λ² × guard-ring factor (1.5-2×).
- Routing: O(N·r²) wires → metal-pitch / track-density area.
- Present LOW/MED/HIGH bands.
- **Alternative comparator**: time-multiplexed inhibition (2-pass, ~2× latency, ~0 extra area) as cheap-mod baseline.

### Decision gate refinement
- Paired 95% CI lower bound > 0 pp **AND**
- Median relative error reduction ≥ 20% **OR** absolute +3 pp if
  baseline >90% acc **AND**
- Consistent sign in ≥8/10 seeds **AND**
- **Beats area-matched baseline** (r=0 with N↑ to same area cost).
- This last point is gpt-5's #1 critique: without Pareto control,
  Mario can't justify fabric change over "just make N bigger."

### Killer omissions (NOW INCLUDED)
- gpt-5: **Area/power-normalized comparator** (above).
- gemini: **Scale-gap test** — surrogate accuracy as function of N
  (32, 64, 128). Otherwise candidate at N=256 may be a surrogate
  artifact that doesn't scale.

### Process hygiene
- gpt-5: Preregister Week-1 hypotheses/metrics; lock analysis-script
  git hash; append-only run ledger.
- gemini: **Baseline-watchdog cron** — fixed-seed simplest baseline
  daily, assert performance within tight known range. Catches silent
  bugs that improve candidate by degrading baseline.

### Cron schedule (CONFIRMED)
1. work-hours hourly :13 (existing)
2. daily synthesis 02:13 (existing)
3. mid-week risk Thu 16:07 (existing)
4. weekly review Mon 09:23 (existing)
5. **NEW**: GPU off-hours 03:23 daily (run heavy compute when ambient cool)
6. **NEW**: Oracle feedback every 12 h (gpt-5 correction: NOT 6 h)
   OR event-triggered on gate crossings.
7. **NEW**: Track-progress audit every 6 h work-hours (gpt-5 correction: NOT 4 h)
8. **NEW**: Baseline watchdog 04:43 daily (gemini)
9. **NEW**: Re-arm one-shot day 6 (already exists from prior re-arm)

### Verdict from oracles

**gpt-5**: "Stamp: Proceed with P(min wrapper) + S today, run a
6-seed pilot V and compute σ_d, raise test-set sizes where feasible,
and add the area-matched baseline to the Week-1 gate."

**gemini**: "Implement these fixes, and you have a green light."


---

## O31b grok additions (2026-05-07)

User flagged: "grok is the most critical one so always include it in oracle calls". Re-dispatched plan to grok separately. Grok caught what gpt-5 + gemini both missed:

### KILLER OMISSION (grok #7) — ABLATION STUDY

The plan tests the full candidate (`ER_SPARSE + r=2/s=0.3 + ff + N=256`) but NOT isolated effects. Without ablation we may attribute the win to the WRONG component. **Mandatory ablation matrix added to Track V**:

| Variant | topo | inh r | inh s | rule |
|---------|------|-------|-------|------|
| baseline | ER_SPARSE | 0 | 0 | ff |
| inh-only | ER_SPARSE | 2 | 0.3 | ff |
| rule-only | ER_SPARSE | 0 | 0 | hebb_ip |
| topo-only | RAND_GAUSS | 0 | 0 | ff |
| inh+rule | ER_SPARSE | 2 | 0.3 | hebb_ip |
| candidate | ER_SPARSE | 2 | 0.3 | ff |

Each at 20 seeds. Pairwise paired t-tests reveal which axes carry the win.

### Other grok refinements
- **20 seeds** (not 10) for >0.9 power at σ=0.1 worst-case
- **Decision gate tightened**: paired t-test p<0.05 + CI not overlapping baseline (more strict than gpt-5's already-strict criterion)
- **Daily resource audit** cron (disk/CPU/GPU >80% flag, 00:00 daily)

### Verdict
**grok**: "Stamp: Go with fixes: (3) 20 seeds; (5) tighten gate to p<0.05; (6) add resource audit; (7) insert ablation. Total rev <1 day."

Plan now has 3-oracle approval (gpt-5 + gemini + grok), with grok contributing the ablation requirement that fundamentally changes Track V's experimental design.
