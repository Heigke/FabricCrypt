...
On two AMD Ryzen AI Max+ 395 "Strix Halo" laptops (`hostA` and
`hostB`) we obtain **100% leave-one-out per-die classification** on
...
- Has full control of a second machine of the same SKU (`hostB` is
  the example throughout: same chassis model, same APU, same kernel
  family, BIOS X53 v01.02.02 vs v01.01.08 on `hostA`).
...
kHz-scale tick. Observed: hostA (0,1) p50 = 7080 cyc, hostB (0,1)
...
hostA nanosleep p99.9 = 67428 ns (matched governor) vs hostB
...
between the two hosts' Jacobians is **‖J_hostA − J_hostB‖_F = 98.71**,
and the L2 distance between their top-5 eigenvalue spectra is **9.35**
(hostA: 1.67, 0.25, 0.22, 0.19, 0.12; hostB: 3.06, 0.66, 0.32, 0.24,
0.20). The top eigenvalue ratio (3.06 / 1.67 = 1.83×) shows the hostB
...
*delta function* — every read of hostA's S20–S26 produces identical
bytes, and every read of hostB's S20–S26 produces a *different*
...
be *governor-determined*: at matched `performance` governor, hostA
RDRAND p50 = 120 cyc *equals* hostB RDRAND p50 = 120 cyc (KS-D ≈
...
466-dim signatures, top-2 components, hostA vs hostB
...
governors differ: hostA = `powersave`, hostB = `performance`) was
resolved in Phase 14D. We re-measured the hostA side at both
`powersave` and at `performance` (matched to hostB's default) and
re-ran T2 (anomaly detection) and T3 (twin-paradox host attribution)
against the frozen hostB reference.

| Metric                                  | Original (mixed)  | hostA@powersave  | hostA@performance |
|-----------------------------------------|-------------------|-------------------|--------------------|
...
| HostB-peer transplant          | ≤0.05             | 0.020    | ✓   |
...
inter acceptance is 0/10 (hostB signatures are at Hamming μ = 134
from hostA), and imposter acceptance over 100 random 512-bit vectors
...
(challenge, response) pairs on hostA, a 2-layer MLP achieves Pearson
...
hostA + hostB replicates. Hamming μ tracks the random floor of
...
**L2 — BIOS version confound.** hostA runs BIOS X53 v01.01.08 and
hostB runs v01.02.02. The Phase 14D matched-governor sweep
...
is the thermally-feasible budget on a single hostA laptop without a
