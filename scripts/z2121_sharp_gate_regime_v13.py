#!/usr/bin/env python3
"""
z2121v13: Sharp Gate + Regime Labels — Hardware Determines the Task
===================================================================
v13: Make the gate commit to the silicon signal, and make the correct
     answer depend on physical state. Three silicon-close changes:

  1. probe_mode=True in Phase A too — proprio test was blind
  2. GATE_TEMP=8.0 — sharp sigmoid forces gate to commit to HW state
     (z2095 proved: GATE_TEMP=8 → kill-shot PASS). Not fabrication,
     just making the gate respond decisively to the signal it already has.
  3. Regime-dependent labels — low DVFS = next-token, high DVFS = skip-gram.
     The model MUST feel the hardware to get the right answer, because what
     "right" means shifts with the physical state. (z2091 proved: label
     shift → kill-shot ratio 2.1x). This is the most silicon-honest
     embodiment: the task itself is bound to the physical reality.

  Carries forward all v12 fixes:
  - probe_mode=True during training
  - PULSE_EPS scaled by weight norm, PULSE_ALPHA=0.65
  - Gate init small random, LoRA layers 4-11
...
Hardware setup:
  ...
  sudo HSA_OVERRIDE_GFX_VERSION=11.0.0 PYTORCH_ROCM_ARCH=gfx1100 \\
    venv/bin/python -u scripts/z2121_sharp_gate_regime_v13.py
"""
...
# v13: Sharp gate temperature — forces sigmoid to commit to HW signal
GATE_TEMP = 8.0
...
def run_phase_b(model, train_data, test_data, test_data_code, baseline_ppl, phase_a_results):
    ...
    for batch_idx in range(N_TRAIN_BATCHES):
        ...
        for chunk_i in range(n_chunks - 1):
            ...
            # v13: Regime-dependent labels — the task itself depends on physical state.
            # Low DVFS (cold/nominal): standard next-token prediction (offset=1)
            # High DVFS (hot/throttled): skip-gram prediction (offset=2, predict 2 ahead)
            # The model MUST sense the hardware to know which task it's doing.
            if CTX.regime >= REGIME_HOT:
                # Skip-gram: predict token i+2 from position i
                shift_logits = logits[:, MICRO_CHUNK-1:-2, :].contiguous()
                shift_labels = chunk[:, MICRO_CHUNK+1:].contiguous()
...
def main():
    ...
    print("=" * 60)
    print("z2121v13: Haptic Resonance GPT-2 — Lighter Backbone")
    ...
    print(f"\n" + "=" * 60)
    print(f"z2121v13 Haptic Resonance GPT-2 LM: {n_pass}/{n_total} PASS")
    print("=" * 60)

    # Save results
    out_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'z2121_sharp_gate_regime.json')
    out_path = os.path.abspath(out_path)
    result_obj = {
        'experiment': 'z2121v13_sharp_gate_regime',
        'description': (
            'v13: Sharp gate regime — GATE_TEMP=8.0 (near-binary sigmoid), '
            'regime-dependent labels (next-token in cold/nominal, skip-gram in hot/throttled), '
            'probe_mode=True in Phase A T5 proprio test. '
            'Builds on v12 silicon-unblocked fixes. Model MUST sense DVFS to know its task.'
        ),
