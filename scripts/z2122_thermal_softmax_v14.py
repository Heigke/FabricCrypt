#!/usr/bin/env python3
"""
z2122v14: Thermal Softmax + Physical Noise — Silicon Shapes Attention
=====================================================================
v14: Two physically-grounded mechanisms replace the failed skip-gram regime:

  1. THERMAL SOFTMAX (Boltzmann kT): Die temperature (40-90°C) maps to
     attention softmax temperature. Cold silicon = sharp attention (low T),
     hot silicon = diffuse attention (high T). This is the Boltzmann
     distribution — same physics that governs thermal noise in transistors.

  2. PHYSICAL NOISE INJECTION: Measured GPU power (from gpu_metrics) maps
     to Gaussian noise variance injected into hidden states. Higher clock
     = more power = more noise. Based on stochastic thermodynamics.

  3. PULSE_EPS=0.05 (was 0.15): Tame the pulse overdose for GPT-2.
  4. GATE_TEMP=4.0 (was 8.0): Sharp but not locked.
  5. NO SKIP-GRAM: Both regimes do standard next-token prediction.

  Carries forward all v12 silicon fixes.
...
Hardware setup:
  ...
  sudo HSA_OVERRIDE_GFX_VERSION=11.0.0 PYTORCH_ROCM_ARCH=gfx1100 \\
    venv/bin/python -u scripts/z2122_thermal_softmax_v14.py
"""
...
# v10: Pulse field constants — v14: EPS tamed for GPT-2 (0.15 was overdose)
PULSE_ALPHA = 0.65        # v12: was 0.85 — faster response to current execution state
PULSE_GAIN = 2.0          # tanh gain for sensed signal
PULSE_EPS = 0.05          # v14: was 0.15 — z2121 T37 bypass showed 10x PPL improvement = overdose
...
# v14: Gate temperature — sharp but not locked (was 8.0 in v13, gave AUROC=1.0 but too binary)
GATE_TEMP = 4.0

# v14: Thermal Softmax constants — Boltzmann kT maps die temp to attention temperature
THERMAL_SOFTMAX_T_MIN = 40.0    # °C — coldest expected die temp
THERMAL_SOFTMAX_T_MAX = 90.0    # °C — hottest expected die temp
THERMAL_SOFTMAX_SCALE_MIN = 0.8  # attention temp multiplier at T_MIN (sharper)
THERMAL_SOFTMAX_SCALE_MAX = 1.4  # attention temp multiplier at T_MAX (more diffuse)

# v14: Physical Noise Injection constants — GPU power maps to noise variance
NOISE_POWER_REF = 30000.0  # mW reference power (typical mid-load)
NOISE_SIGMA_BASE = 0.002   # base noise sigma at reference power
NOISE_SIGMA_MAX = 0.01     # max noise sigma (clip for stability)
...
class ForwardContext:
    ...
    def __init__(self):
        ...
        # v14: Thermal softmax — die temperature for attention scaling
        self.temp_gfx = 50.0       # GPU die temperature in °C
        self.gfx_power = 0.0       # GPU power in mW (for noise injection)
        self.thermal_attn_scale = 1.0  # computed attention temperature multiplier
        self.noise_sigma = 0.0     # computed noise injection sigma
...
class MetabolicController:
    ...
    def update_context(self):
        ...
        # v14: Update thermal softmax and noise injection from real hardware
        CTX.temp_gfx = self._last_temp
        CTX.gfx_power = self._last_power

        # Thermal attention scale: linear map from die temp to softmax temperature
        t_frac = max(0.0, min(1.0, (self._last_temp - THERMAL_SOFTMAX_T_MIN) /
                                    max(THERMAL_SOFTMAX_T_MAX - THERMAL_SOFTMAX_T_MIN, 1.0)))
        CTX.thermal_attn_scale = THERMAL_SOFTMAX_SCALE_MIN + t_frac * (THERMAL_SOFTMAX_SCALE_MAX - THERMAL_SOFTMAX_SCALE_MIN)

        # Physical noise sigma: proportional to sqrt(power/reference)
        # Higher power = more thermal noise in substrate
        power_ratio = max(self._last_power, 1.0) / NOISE_POWER_REF
        CTX.noise_sigma = min(NOISE_SIGMA_BASE * math.sqrt(power_ratio), NOISE_SIGMA_MAX)
...
class BodyGatedLoRA(nn.Module):
    ...
    def __init__(self, analog_linear, layer_idx, rank=8, alpha=16):
        ...
        # v12: ADDITIVE gate — small random init so real kernel signals are heard from batch 1
        # (v10 zero-init meant zeros*zeros=zeros — gates were permanently deaf)
    ...
    def forward(self, x):
        ...
        gate = torch.sigmoid(gate_pre * GATE_TEMP)  # v13: sharp gate — commit to HW signal
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# v14: THERMAL SOFTMAX — patch GPT-2 attention to scale by die temp
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
_THERMAL_HOOKS = {}

def patch_attention_thermal(model, layers):
    """Register forward hooks on GPT-2 attention to scale query by CTX.thermal_attn_scale.

    GPT-2's attention computes: attn_weights = (Q @ K^T) / sqrt(d_k)
    We scale Q by 1/thermal_attn_scale BEFORE it enters the attention computation,
    which is equivalent to dividing attention logits by thermal_attn_scale.

    When thermal_attn_scale > 1 (hot): attention is more diffuse (higher entropy)
    When thermal_attn_scale < 1 (cold): attention is sharper (lower entropy)

    Implementation: We monkey-patch each GPT2Attention.forward() to wrap the original,
    scaling query_states after the QKV split but before the attention_interface call.
    """
    global _THERMAL_HOOKS
    for layer_idx in layers:
        block = model.transformer.h[layer_idx]
        attn = block.attn
        if layer_idx in _THERMAL_HOOKS:
            continue  # already patched

        orig_forward = attn.forward

        def make_thermal_forward(orig_fn, idx):
            import functools
            @functools.wraps(orig_fn)
            def thermal_forward(hidden_states, **kwargs):
                scale = CTX.thermal_attn_scale
                if scale != 1.0 and scale > 0:
                    # Scale hidden states before c_attn projection.
                    # Since c_attn is linear: c_attn(h/s) = c_attn(h)/s
                    # This scales Q,K,V all by 1/s. But we only want Q scaled.
                    # Instead, we call original forward, but we need to intercept
                    # after QKV split. Simplest correct approach: temporarily
                    # adjust scale_attn_weights to incorporate thermal scaling.
                    # GPT-2 attention: attn_weights = Q @ K^T  (then /sqrt(d) if scale_attn_weights)
                    # We want: attn_weights / thermal_scale  = Q @ K^T / (sqrt(d) * thermal_scale)
                    # This is equivalent to scaling Q by 1/sqrt(thermal_scale) AND K by 1/sqrt(thermal_scale)
                    # OR just dividing the full attn logits. Easiest: scale hidden_states
                    # feeding into Q only.
                    #
                    # Best approach: patch c_attn output. We wrap c_attn temporarily.
                    attn_mod = model.transformer.h[idx].attn
                    orig_c_attn_forward = attn_mod.c_attn.forward
                    split_size = attn_mod.split_size

                    def scaled_c_attn(x):
                        qkv = orig_c_attn_forward(x)
                        q, k, v = qkv.split(split_size, dim=2)
                        q = q / scale  # thermal scaling on Q only
                        return torch.cat([q, k, v], dim=2)

                    attn_mod.c_attn.forward = scaled_c_attn
                    try:
                        result = orig_fn(hidden_states, **kwargs)
                    finally:
                        attn_mod.c_attn.forward = orig_c_attn_forward
                    return result
                else:
                    return orig_fn(hidden_states, **kwargs)
            return thermal_forward

        attn.forward = make_thermal_forward(orig_forward, layer_idx)
        _THERMAL_HOOKS[layer_idx] = orig_forward
    print(f"  [ThermalSoftmax] Patched {len(layers)} attention layers")


def restore_attention_thermal(model, layers):
    """Restore original attention forward methods."""
    for layer_idx in layers:
        if layer_idx in _THERMAL_HOOKS:
            model.transformer.h[layer_idx].attn.forward = _THERMAL_HOOKS[layer_idx]
            del _THERMAL_HOOKS[layer_idx]


# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# v14: PHYSICAL NOISE INJECTION — inject power-proportional Gaussian noise
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
class PhysicalNoiseInjector(nn.Module):
    """v14: Injects Gaussian noise into hidden states proportional to GPU power.

    Physics: Higher clock speed → more switching transistors → more thermal noise.
    The noise variance scales with sqrt(power/reference_power).
    During eval (model.eval()), noise is disabled for clean PPL measurement.
    """
    def __init__(self):
        super().__init__()
        # Learnable scaling factor — lets the model learn how much noise to accept
    ...
    def forward(self, hidden_states):
        ...
        if sigma <= 0:
            return hidden_states
        # Gate the noise with a learnable factor (clamped positive)
        effective_sigma = sigma * torch.clamp(self.noise_gate, min=0.01, max=3.0)
        noise = torch.randn_like(hidden_states) * effective_sigma
...
def run_phase_a(model, test_data, test_data_code, baseline_ppl):
    ...
    if DVFS_AVAILABLE:
        ...
        for dvfs_level, dvfs_name in [(0, 'low'), (2, 'high')]:
            ...
            CTX.probe_mode = True   # v13: let kernel telemetry through for proprio measurement
            ppl = eval_ppl(model, test_data)
...
def run_phase_b(model, train_data, test_data, test_data_code, baseline_ppl, phase_a_results):
    print("\n" + "=" * 60)
    print("PHASE B: v14 3-Phase Curriculum (thermal softmax + physical noise)")
    print("  Phase1: Calibration 0-40 | Phase2: Reflex 40-120 | Phase3: Integration 120-200")
    print("  v14: thermal softmax (Boltzmann kT) + power-proportional noise + PULSE_EPS=0.05")
    print("=" * 60)
    ...
    # v14: Patch attention with thermal softmax
    patch_attention_thermal(model, list(range(len(model.transformer.h))))

    # v14: Physical noise injector (learnable gate)
    noise_injector = PhysicalNoiseInjector().to(DEVICE)
    print(f"  [PhysicalNoise] noise_gate init={float(noise_injector.noise_gate):.2f}")
    ...
    all_params = (list(soma_head.parameters()) + list(soma_probe.parameters()) +
                  lora_params + list(noise_injector.parameters()))
    ...
    for batch_idx in range(N_TRAIN_BATCHES):
        ...
        for chunk_i in range(n_chunks - 1):
            ...
            # v14: Standard next-token prediction in ALL regimes.
            # Skip-gram removed (z2121 proved: inflated PPL from 45→504, not physically grounded).
            # Physical differentiation comes from thermal softmax + noise injection instead.
            ...
            # v14: Physical noise injection — power-proportional Gaussian noise
            hidden = noise_injector(hidden)
            ...
            optimizer.zero_grad()
            total_loss.backward()
            torch.nn.utils.clip_grad_norm_(all_params, 0.5)  # v14: tighter clip (was 1.0)
        ...
        if (batch_idx + 1) % 25 == 0:
            ...
            print(f"    Batch {batch_idx+1}/{N_TRAIN_BATCHES} [{phase_name}]: "
                  ...
                  f"therm_scale={CTX.thermal_attn_scale:.3f}, "
                  f"noise_σ={CTX.noise_sigma:.5f}")
    ...
    # Cleanup
    set_analog_mode_override(model, ANALOG_LAYERS, None)
    restore_attention_thermal(model, list(range(len(model.transformer.h))))
...
def main():
    ...
    print("=" * 60)
    print("z2122v14: Thermal Softmax + Physical Noise — Silicon Shapes Attention")
    print("GPU-resident pulse field + Boltzmann kT attention + power-proportional noise")
    print(f"  PULSE_EPS={PULSE_EPS}, GATE_TEMP={GATE_TEMP}")
    print(f"  THERMAL_SOFTMAX: T_range=[{THERMAL_SOFTMAX_T_MIN},{THERMAL_SOFTMAX_T_MAX}]°C, "
          f"scale=[{THERMAL_SOFTMAX_SCALE_MIN},{THERMAL_SOFTMAX_SCALE_MAX}]")
    print(f"  NOISE: ref_power={NOISE_POWER_REF}mW, sigma_base={NOISE_SIGMA_BASE}, "
          f"sigma_max={NOISE_SIGMA_MAX}")
    print(f"  FREEZE_PULSE={FREEZE_PULSE_STATE}, BYPASS_SENSING={BYPASS_LIVE_SENSING}, "
          f"DISABLE_WRITES={DISABLE_MODE_WRITES}")
    print("=" * 60)
    ...
    print(f"\n" + "=" * 60)
    print(f"z2122v14 Thermal Softmax GPT-2 LM: {n_pass}/{n_total} PASS")
    print("=" * 60)

    # Save results
    out_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'z2122_thermal_softmax.json')
    out_path = os.path.abspath(out_path)
    result_obj = {
        'experiment': 'z2122v14_thermal_softmax',
        'description': (
            'v14: Thermal Softmax + Physical Noise — die temp maps to attention temperature '
            '(Boltzmann kT), GPU power maps to hidden state noise variance. '
            'PULSE_EPS=0.05 (tamed from 0.15 overdose), GATE_TEMP=4.0, no skip-gram. '
            'Physically grounded: hotter silicon → more diffuse attention + more noise.'
        ),
        ...
        'gate_temp': GATE_TEMP,
        'thermal_softmax_t_range': [THERMAL_SOFTMAX_T_MIN, THERMAL_SOFTMAX_T_MAX],
        'thermal_softmax_scale_range': [THERMAL_SOFTMAX_SCALE_MIN, THERMAL_SOFTMAX_SCALE_MAX],
        'noise_power_ref': NOISE_POWER_REF,
        'noise_sigma_base': NOISE_SIGMA_BASE,
