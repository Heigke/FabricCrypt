#!/usr/bin/env python3
"""
z2135v27: Deep Coupling — Patch ALL projections + RAPL energy + Longer training
================================================================================
v27: Based on v26 (best 31/40) + 4 major structural changes from research review:
...
Hardware setup:
  ...
  sudo HSA_OVERRIDE_GFX_VERSION=11.0.0 PYTORCH_ROCM_ARCH=gfx1100 \\
    venv/bin/python -u scripts/z2135_deep_coupling_v27.py
"""
...
class BodyGatedLoRA(nn.Module):
    ...
    def __init__(self, analog_linear, layer_idx, rank=8, alpha=16):
        ...
        # v12: ADDITIVE gate — small random init
        ...
        nn.init.normal_(self.gate_fast.weight, std=0.02)
        nn.init.normal_(self.gate_mid.weight, std=0.02)
        nn.init.normal_(self.gate_slow.weight, std=0.02)
    ...
    def forward(self, x):
        ...
        gate = torch.sigmoid(gate_pre)  # [rank], in [0,1]
...
def run_phase_b(model, train_data, test_data, test_data_code, baseline_ppl, phase_a_results):
    print("\n" + "=" * 60)
    print("PHASE B: v27 3-Phase Curriculum (deep coupling)")
    print("  Phase1: Calibration 0-80 | Phase2: Reflex 80-240 | Phase3: Integration 240-400")
    print("  v27: ALL projections + RAPL energy + thermal rounding + contrastive kill-shot")
    print("=" * 60)
    ...
    for batch_idx in range(N_TRAIN_BATCHES):
        ...
        for chunk_i in range(n_chunks - 1):
            ...
            # v16: Contrastive kill-shot loss — wrong regime gate → higher loss
            contrastive_loss = torch.tensor(0.0, device=DEVICE)
            if (phase_name != 'calibration' and target_dvfs in ('low', 'high')
                    and random.random() < CONTRASTIVE_PROB):
                # Compute "wrong gate" CE by temporarily flipping regime context
                orig_regime = CTX.regime
                # Wrong regime: if we're cold (0), pretend hot (2); if hot/throttled, pretend cold
                wrong_regime = 2 if orig_regime <= 1 else 0
                CTX.regime = wrong_regime
                with torch.no_grad():
                    ...
                    wrong_ce = F.cross_entropy(wrong_shift.view(-1, wrong_shift.size(-1)),
                                                shift_labels.view(-1))
                CTX.regime = orig_regime
                # Margin loss: correct_ce should be < wrong_ce - margin
                # loss = max(0, correct_ce - (wrong_ce - margin))
                contrastive_loss = torch.clamp(
                    ce_loss - (wrong_ce - CONTRASTIVE_MARGIN), min=0.0)

            # v16: total loss includes contrastive kill-shot + pulse prediction
            total_loss = (ce_loss
...
def main():
    ...
    print("=" * 60)
    print("z2135v27: Deep Coupling — ALL projections + RAPL energy + 400 batches + thermal rounding")
    print("Dual LoRA banks (cold/hot) + contrastive loss + agreement modulation")
    ...
    print(f"  AGREEMENT_POWER={AGREEMENT_POWER}")
    ...
    print(f"\n" + "=" * 60)
    print(f"z2135v27 Deep Coupling GPT-2 LM: {n_pass}/{n_total} PASS")
    print("=" * 60)

    # Save results
    out_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'z2135_deep_coupling.json')
    out_path = os.path.abspath(out_path)
    result_obj = {
        'experiment': 'z2135v27_deep_coupling',
        'description': (
    ...
    ts_path = os.path.join(os.path.dirname(out_path), f'z2135_v27_{ts}_{n_pass}of{n_total}.json')
