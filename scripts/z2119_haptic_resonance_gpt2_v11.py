#!/usr/bin/env python3
"""
z2119v11: Haptic Resonance GPT-2 — Lighter Backbone for Stability
=================================================================
v11: Step back to GPT-2 (124M) from Qwen3-8B for faster iteration.

  v11 CORE CHANGES (from v10/z2118):
  1. GPT-2 backbone (124M) instead of Qwen3-8B (8.2B)
  2. 12 layers (768 hidden), access via model.transformer.h[i].mlp.c_fc
  3. AnalogLinear handles Conv1D weight transpose (nx,nf) -> (nf,nx)
  4. LoRA layers 4-11 (was 8-31)
  5. All v10 features preserved: pulse field, freeze/bypass/disable,
     soma heads, salience sweep, 3-phase curriculum, full test battery
...
Hardware setup:
  ...
  sudo HSA_OVERRIDE_GFX_VERSION=11.0.0 PYTORCH_ROCM_ARCH=gfx1100 \\
    venv/bin/python -u scripts/z2119_haptic_resonance_gpt2_v11.py
"""
...
def main():
    ...
    print("=" * 60)
    print("z2119v11: Haptic Resonance GPT-2 — Lighter Backbone")
    ...
    print(f"\n" + "=" * 60)
    print(f"z2119v11 Haptic Resonance GPT-2 LM: {n_pass}/{n_total} PASS")
    print("=" * 60)

    # Save results
    out_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'z2119_haptic_resonance_gpt2.json')
    out_path = os.path.abspath(out_path)
    result_obj = {
        'experiment': 'z2119v11_haptic_resonance_gpt2',
        'description': (
            'v11: GPT-2 backbone + GPU-resident pulse field + weighted sensing '
            '(0.50*dt+0.30*var+0.20*wgp) + freeze/bypass/disable ablation switches '
            '+ pulse prediction heads + no-cheat soma head (hidden-only) + salience sweep'
        ),
