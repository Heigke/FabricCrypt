#!/usr/bin/env python3
"""z2275_synergy_bridge.py — Synergy Bridge (targeting T16: BRIDGE_FULL best on 3/5)

Fixes over z2274 (MAC saturated at ~0.97 → BRIDGE_FULL regressed):
  1. MAC signal reverted to z2273's mean(abs) — mean ~0.43, good dynamic range
  2. Ridge alpha sweep extended: 8 alphas from 1e-5 to 1e5
  3. Bridge readout weights FPGA features ×2 to amplify FPGA contribution
  4. Kept from z2275: PCA_DIMS=120, 1500 continuous steps, NARMA-5

Run:
  HSA_OVERRIDE_GFX_VERSION=11.0.0 PYTHONUNBUFFERED=1 venv/bin/python scripts/z2275_synergy_bridge.py
"""
...
SAVE_FILE = RESULTS / 'z2275_synergy_bridge.json'
TEXT_FILE = RESULTS / 'z2275_synergy_bridge.txt'
...
def compile_stress_kernel():
    src_path = '/tmp/z2275_gpu_stress.hip'
    bin_path = '/tmp/z2275_gpu_stress'
...
class MACBridgeReservoir:
    ...
    def _compute_mac_from_gpu(self, gpu_state_t):
        ...
        Uses mean(abs) — gives mean ~0.43 with good input-dependent variation.
        z2275's pop_std approach saturated at ~0.97, removing input dependence.
        """
    ...
    def run_trial(self, input_signal, condition):
        ...
        if condition == 'FPGA_ONLY':
            ...
            # Weight FPGA features ×2 to amplify MAC-modulated contribution
            return np.hstack([gpu_states, fpga_states * 2.0])
...
def print_summary_and_tests(all_results):
    ...
    p("\n" + "=" * 72)
    p("z2275: SYNERGY BRIDGE — SUMMARY")
    p("=" * 72)
...
def main():
    ...
    print("=" * 72)
    print("  z2275: SYNERGY BRIDGE — Fixed MAC + FPGA weighting")
    print("  NOISE_SCALE=0.02, LEAK_COND=4 (τ=210ms), BIAS_GAIN=0.03125")
    print("  Fixes: MAC=mean(abs), FPGA×2 weighting, wider alpha sweep, NARMA-5")
