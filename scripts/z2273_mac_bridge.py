#!/usr/bin/env python3
"""z2273_mac_bridge.py — True Closed-Loop GPU→FPGA MAC Bridge

Three fixes over z2272 (10/17):
  1. BRIDGE_FULL now uses REAL MAC feedback: GPU ESN output → fpga.set_mac_signal()
     This activates BIAS_GAIN=0.03125 in hardware (I_bias = 0.03125 × mac)
  2. NARMA: more steps (1000), PCA dim reduction, higher ridge alphas
  3. PCA on bridge features to avoid underdetermined regression (512 feat > 315 samples)

Run:
  HSA_OVERRIDE_GFX_VERSION=11.0.0 PYTHONUNBUFFERED=1 venv/bin/python scripts/z2273_mac_bridge.py
"""
...
SAVE_FILE = RESULTS / 'z2273_mac_bridge.json'
TEXT_FILE = RESULTS / 'z2273_mac_bridge.txt'
...
N_WAVE_TRIALS = 80
N_WAVE_STEPS = 60
N_CONTINUOUS_STEPS = 1000   # 2× z2272 for better regression
MC_MAX_DELAY = 10
WARMUP = 100                # longer warmup for MAC transients
NARMA_ORDER = 10            # standard NARMA-10 (Jaeger 2002)
PCA_DIMS = 80               # reduce bridge features via PCA
...
def main():
    ...
    print("=" * 72)
    print("  z2273: TRUE MAC BRIDGE — GPU→FPGA Closed-Loop Feedback")
    print("  NOISE_SCALE=0.02, LEAK_COND=4 (τ=210ms), BIAS_GAIN=0.03125")
    print("  Fixes: MAC feedback, NARMA-10, PCA regression, 1000 steps")
