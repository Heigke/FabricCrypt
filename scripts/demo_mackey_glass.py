...
Network: 64-cell ER_SPARSE (z142 cross-norm-robust, post-O18/O19 honest cell),
Bf=100 (physical), η-bounded ∈ [0,1]. Updated 2026-05-04 from unphysical
Bf=2e4 + MESH_4N.
...
def run_reservoir(N, T, kappa, mg_signal, seed=42, Bf=100.0):
    ...
    W_rec = torch.tensor(z119.build_W("ER_SPARSE", N, rho=0.9, rng=rng),
                         dtype=torch.float64)
...
def main():
    ...
    horizon = 6
    kappa = 0.30
    print(f"[demo_mg] N={N}, T={T}, horizon={horizon}, kappa={kappa}, ER_SPARSE, Bf=100 honest")
    ...
    # JSON dump
    json.dump({"N": N, "T": T, "horizon": horizon, "topology": "ER_SPARSE",
