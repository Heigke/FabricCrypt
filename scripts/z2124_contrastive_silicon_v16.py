#!/usr/bin/env python3
"""
z2124v16: Contrastive Silicon — Kill-Shot Training on z2120 Base
================================================================
v16: z2120's "let hardware through" base (26/40) + contrastive kill-shot
training from z2095 (which gave T7=1.283 PASS on GPT-2).

  z2122-z2123 diagnosis: Thermal softmax and noise injection HURT more
  tests than they help (corrupted attention, degraded pulse signal).
  z2120 (26/40) beats z2123 (23/40) through simplicity.

  v16 strategy: Keep z2120 exactly as-is, add ONLY:
  1. CONTRASTIVE KILL-SHOT LOSS (z2095): λ=0.3, margin=0.3. During forced
     DVFS, compute PPL with correct vs wrong gate and penalize if too close.
     Directly trains the model to NEED the correct hardware state.
  2. AGREEMENT MODULATION (z2095): body_scale *= (1 - |delta_regime - freq_gate|)^2
     When gate output disagrees with actual hardware regime, dampen LoRA output.
     This multiplicatively amplifies the kill-shot penalty.

  NO thermal softmax. NO noise injection. Pure silicon coupling.
...
Hardware setup:
  ...
  sudo HSA_OVERRIDE_GFX_VERSION=11.0.0 PYTORCH_ROCM_ARCH=gfx1100 \\
    venv/bin/python -u scripts/z2124_contrastive_silicon_v16.py
"""
...
class BodyGatedLoRA(nn.Module):
    ...
    def forward(self, x):
        ...
        # v16: Agreement modulation — dampen LoRA when gate disagrees with regime
        # freq_gate = how much gate "thinks" we're high-DVFS (mean gate value)
        # delta_regime = actual regime fraction (0=cold, 1=throttled)
        freq_gate = gate.mean()
        delta_regime = CTX.regime / max(N_REGIMES - 1, 1)
        disagreement = torch.abs(freq_gate - delta_regime)
        agreement_scale = (1.0 - disagreement) ** AGREEMENT_POWER
        main_lora_out = main_lora_out * agreement_scale.to(main_lora_out.dtype)
...
def main():
    ...
    print("=" * 60)
    print("z2124v16: Contrastive Silicon — Kill-Shot Training on z2120 Base")
    print("z2120 base + contrastive kill-shot loss + agreement modulation")
    ...
    print(f"\n" + "=" * 60)
    print(f"z2124v16 Contrastive Silicon GPT-2 LM: {n_pass}/{n_total} PASS")
    print("=" * 60)

    # Save results
    out_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'z2124_contrastive_silicon.json')
    out_path = os.path.abspath(out_path)
    result_obj = {
        'experiment': 'z2124v16_contrastive_silicon',
        'description': (
            'v16: z2120 base (26/40) + contrastive kill-shot loss (λ=0.3, margin=0.3) '
            '+ agreement modulation (body_scale *= (1-|disagreement|)^2). '
            'No thermal softmax, no noise injection — pure silicon coupling.'
        ),
