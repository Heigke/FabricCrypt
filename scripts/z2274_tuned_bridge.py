#!/usr/bin/env python3
"""z2274_tuned_bridge.py — Tuned MAC Bridge (targeting synergy T16)

Improvements over z2273 (14/19):
  1. Dynamic MAC signal: uses per-population std instead of mean(abs) — more input-dependent
  2. Higher PCA_DIMS=120 for bridge conditions (was 80) — preserve more MC-relevant features
  3. Longer continuous: 1500 steps (was 1000) for better regression statistics
  4. NARMA-5 instead of NARMA-10 (10 is too hard for 50Hz reservoir)

Run:
  HSA_OVERRIDE_GFX_VERSION=11.0.0 PYTHONUNBUFFERED=1 venv/bin/python scripts/z2274_tuned_bridge.py
"""
...
SAVE_FILE = RESULTS / 'z2274_tuned_bridge.json'
TEXT_FILE = RESULTS / 'z2274_tuned_bridge.txt'
...
N_WAVE_TRIALS = 80
N_WAVE_STEPS = 60
N_CONTINUOUS_STEPS = 1500   # 3× z2272 for better regression
MC_MAX_DELAY = 10
WARMUP = 150                # longer warmup for MAC transients
NARMA_ORDER = 5             # NARMA-5 (more tractable at 50Hz)
PCA_DIMS = 120              # higher PCA dims to preserve MC features
...
def compile_stress_kernel():
    src_path = '/tmp/z2274_gpu_stress.hip'
    bin_path = '/tmp/z2274_gpu_stress'
...
def ridge_regress(X_tr, y_tr, X_te, y_te):
    """Ridge regression with PCA if underdetermined."""
    ...
    alphas = [1e-4, 0.01, 0.1, 1.0, 10.0, 100.0, 1000.0]
    best_nrmse = 1e10
    best_r2 = -1e10
    for a in alphas:
        I = np.eye(X_tr_s.shape[1])
...
class MACBridgeReservoir:
    ...
    def _compute_mac_from_gpu(self, gpu_state_t):
        ...
        Uses population-level std (variance across neurons) — this is more
        input-dependent than mean(abs) which stays ~0.43 regardless of input.
        High variance = active computation → high MAC → more FPGA current injection.
        """
        # Split into 4 populations of 64 neurons each
        pops = [gpu_state_t[i*64:(i+1)*64] for i in range(4)]
        # Use std of each population — measures activity spread
        pop_stds = [np.std(p) for p in pops]
        # Combine: max pop std × 2 gives good [0, 1] range
        activity = max(pop_stds) * 2.0
...
def run_continuous_benchmark_safe(reservoir, benchmark_name, all_results):
    ...
    # Verify NARMA target has variance
    nt_valid = narma_target[WARMUP:]
    print(f"  [NARMA] Target stats: mean={nt_valid.mean():.4f}, std={nt_valid.std():.4f}, "
    ...
    for cond in SAFE_ORDER:
        ...
        print(f"    NARMA-{NARMA_ORDER}: NRMSE={nrmse:.3f}, R2={r2:.3f}", flush=True)
...
def main():
    ...
    print("=" * 72)
    print("  z2274: TUNED BRIDGE — Dynamic MAC + Higher PCA + NARMA-5")
    print("  NOISE_SCALE=0.02, LEAK_COND=4 (τ=210ms), BIAS_GAIN=0.03125")
    print("  Fixes: Dynamic MAC (pop std), NARMA-5, PCA=120, 1500 steps")
    ...
    print("\n" + "=" * 72)
    print(f"BENCHMARKS 3-5: MC + XOR + NARMA-{NARMA_ORDER} ({N_CONTINUOUS_STEPS} steps each)")
    print("=" * 72, flush=True)
