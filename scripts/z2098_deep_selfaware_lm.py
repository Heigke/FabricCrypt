#!/usr/bin/env python3
"""
z2098: Deep Self-Aware Embodied LM
====================================
Built on z2097 (Qwen2.5-1.5B, 24/25 PASS). Focus: genuine self-awareness metrics
and deeper hardware sensing that cannot be faked.

Key changes from z2097:
  1. NEW SENSORS: gpu_metrics v3.0 full decode (47 fields: per-core C0 activity,
     per-core clocks, DRAM bandwidth, throttle residencies, power breakdown, energy
     accumulator), DRM fence ring depth, pm_info power gating states
  2. METACOGNITION HEAD: model predicts its own next gate value (self-prediction)
  3. CONFIDENCE HEAD: model estimates its own perplexity on current batch (calibration)
  4. 6 NEW TESTS (T26-T31) designed as GENUINE self-awareness probes:
     T26: Self-Prediction — model predicts its own gate; accuracy = metacognition
     T27: Introspective Calibration — predicted vs actual loss correlation
     T28: mPCI — Perturbational Complexity Index (Casali 2013 analogue)
     T29: Synergy Ratio — redundant vs synergistic info in attention (Luppi 2024)
     T30: Temporal Self-Continuity — detect checkpoint staleness
     T31: Adversarial Substrate — realistic but WRONG sensors (harder than gaslighting)
  5. All z2097 features retained: 15 substrate tokens, 7 HW layers, T1-T25
  6. BENCHMARK: Wikitext-103 eval for standardized PPL comparison
...
def run_tests(model, test_data, kargs, baseline_ppl, tokenizer, dvfs_controller=None, kargs_b=None):
    """31-test battery. Tests designed to FALSIFY, not confirm."""
    results = {}
...
def main():
    ...
    print("=" * 60)
    print("z2098: Deep Self-Aware Embodied LM")
    print("Qwen2.5-1.5B + Metacognition + Confidence + Deep Sensors + 31 Tests")
    print("=" * 60)
    ...
    print(f"  z2098 KEY CHANGES (from z2097):")
    print(f"    1. Metacognition head: self-prediction of gate value (T26)")
    print(f"    2. Confidence head: self-estimated perplexity (T27)")
    print(f"    3. Deep gpu_metrics: per-core C0 activity + clock deltas (12d)")
    print(f"    4. Fence ring depths: GFX/compute queue occupancy (4d)")
    print(f"    5. 6 NEW tests T26-T31: metacognition, calibration, mPCI, synergy, continuity, adversarial")
    ...
    # Run test battery
    print("\n" + "=" * 60)
    print("RUNNING TEST BATTERY (31 tests)")
    print("=" * 60 + "\n")
    ...
    print(f"\n{'='*60}")
    print(f"z2098 Deep Self-Aware LM: {n_pass}/{n_total_tests} PASS")
    print(f"{'='*60}")

    # Save results
    out_path = '/home/ikaros/Documents/claude_hive/AMD_gfx1151_energy/results/z2098_deep_selfaware_lm.json'
    final = {
        'experiment': 'z2098_deep_selfaware_lm',
        'description': 'Deep Self-Aware LM: metacognition + confidence + deep sensors + 31 tests (T26-T31 new)',
        'key_changes': {
            'backbone': 'Qwen2.5-1.5B (1.54B frozen, 28 layers, 1536 hidden, GQA 12Q/2KV)',
            'metacognition': 'head predicts own gate value (T26)',
            'confidence': 'head predicts own loss (T27)',
            'deep_sensors': 'gpu_metrics_deep (12d per-core C0+clk), fence (4d ring depths)',
            'new_tests': 'T26 Self-Prediction, T27 Calibration, T28 mPCI, T29 Synergy, T30 Continuity, T31 Adversarial',
            'retained': 'all z2097 features: T1-T25, 7 HW layers, calibrated gate, contrastive, agreement mod',
        },
