#!/usr/bin/env python3
"""
z2096: Spatial Thermal Self-Model + CPU PMU Embodied LM
=======================================================
Key changes from z2095 (sharp-gate, calibrated DVFS):
  T11 thermal fix + expanded sensing:

  1. SPATIAL THERMAL (32 ADC sensors): Bank A (0x598A4-0x598E0) + Bank B (0x599C0-0x599FC)
     via SMN. 3 new substrate tokens: thm_spatial_a, thm_spatial_b, cpu_pmu.
  2. CPU PMU via perf_event_open: Zen 5 retired instructions, branches, branch mispredicts.
     Delta counters per batch → log1p normalized. Genuine CPU-side embodiment channel.
  3. T11 FIX: thermal_head now predicts ALL 32 spatial sensors (not just 1 global temp).
     Uses 6 tokens (analog, energy, thermal, pm_deep, thm_spatial_a, thm_spatial_b).
     32-output sigmoid * 100°C → per-sensor MAE over spatial thermal map.
  4. N_SUBSTRATE_TOKENS = 15 (was 12): +thm_spatial_a(9), +thm_spatial_b(10), +cpu_pmu(11).
     Status→12, action→13, reported_delta→14.
  5. All z2095 fixes retained: calibrated gate, contrastive loss, agreement modulation,
     availability mask (now 15-wide, new tokens marked absent in lite mode).
"""
...
def main():
    ...
    print("=" * 60)
    print("z2096: Spatial Self-Model Embodied LM (32 thermal ADC + CPU PMU + T11 fix)")
    ...
    print(f"  z2096 KEY CHANGES (from z2095):")
    print(f"    1. Spatial thermal: 32 ADC sensors (Bank A + Bank B via SMN)")
    print(f"    2. CPU PMU: instructions, branches, br_mispredict via perf_event_open")
    print(f"    3. T11 fix: thermal_head outputs 32 predictions (not 1)")
    print(f"    4. N_SUBSTRATE_TOKENS = {N_SUBSTRATE_TOKENS} (+thm_spatial_a, +thm_spatial_b, +cpu_pmu)")
    print(f"    5. Availability mask 15-wide (new tokens absent in lite mode)")
    print(f"  Retained from z2095: calibrated gate (temp={GATE_TEMP}), contrastive loss, agreement mod")
    ...
    print(f"\n{'='*60}")
    print(f"z2096 Spatial Self-Model Embodied LM: {n_pass}/{n_total_tests} PASS")
    print(f"{'='*60}")

    # Save results
    out_path = '/home/ikaros/Documents/claude_hive/AMD_gfx1151_energy/results/z2096_spatial_selfmodel_lm.json'
    final = {
        'experiment': 'z2096_spatial_selfmodel_lm',
        'description': 'Spatial thermal (32 ADC) + CPU PMU + T11 fix (32-output thermal head)',
        'key_fixes': {
            'T11_fix': 'thermal_head predicts 32 spatial ADC sensors (not 1 global temp)',
            'new_sensors': '32 spatial thermal ADC (Bank A+B via SMN) + 3 CPU PMU counters',
            ...
            'T4_fix': 'availability mask: lite-absent sensors dont shrink presence_frac (15-wide)',
