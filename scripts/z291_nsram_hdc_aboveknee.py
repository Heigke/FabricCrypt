"""DS-N5e: NS-RAM HDC with V_G2 swing BOTH arms ABOVE surrogate turn-on knee.

DS-N5d's diagnosis: surrogate I_d(V_G1, V_G2) has hard threshold at
V_G2 ~ 0.30 V — below that I_d clamps to ~910 fA NOISE FLOOR regardless
of V_G1. So Variant A (V_G2_LOW=0.00) and Variant B (V_G2_LOW=0.10)
both had LOW arm at/below floor — asymmetric pair.

DS-N5e (this script) locks the ONLY remaining V_G2-as-bit lever:
    V_G1_BIAS = 0.30   (same as DS-N5c)
    V_G2_LOW  = 0.30   (AT knee — conducting)
    V_G2_HIGH = 0.50   (well above knee)

Both arms now conducting. 2 physical neurons per HD bit. 4 seeds.

Pre-registered gates (vs DS-N5c=60.97%, fair baseline=81.10%):
  INTERMEDIATE : mean_acc CI_lo > 0.6097 (non-overlap above DS-N5c)
  CONSERVATIVE : mean_acc >= 0.76
  AMBITIOUS    : mean_acc >= 0.811

If FAIL: V_G2-as-bit is fundamentally infeasible on this surrogate.
"""
...
from z285_nsram_hdc import (
    ...
    hdc_query_normalize, get_device, query_surrogate,
)


# LOCKED single variant — above-knee V_G2 swing
V_G1_BIAS = 0.30
V_G2_LOW  = 0.30   # AT turn-on knee
V_G2_HIGH = 0.50   # above knee


def symmetry_diagnostic(surr, device, V_G1_BIAS, V_G2_LOW, V_G2_HIGH,
                        C_b_F=8e-15, dt_s=1e-7, T_steps=100, vd=1.0):
    """One-bit sanity at V_G1=V_G1_BIAS (zero query input).

    Compute steady-state rates for a +1 prototype neuron and a -1 prototype
    neuron, in BOTH arms (positive / negative arm of diff-pair), at zero
    input (so V_G1 = V_G1_BIAS exactly). Then compute symmetry ratio.
    """
    # 4 (bit, arm) combinations at V_G1=V_G1_BIAS:
    #   bit=+1: pos arm uses V_G2_HIGH, neg arm uses V_G2_LOW
    #   bit=-1: pos arm uses V_G2_LOW,  neg arm uses V_G2_HIGH
    VG1 = torch.tensor([[V_G1_BIAS]], dtype=torch.float32, device=device)
    def run(vg2):
        VG2 = torch.tensor([[vg2]], dtype=torch.float32, device=device)
        rates, _ = nsram_rates(VG1, VG2, surr, C_b_F, dt_s, T_steps, vd=vd)
        return float(rates.item())
    I_d_high = run(V_G2_HIGH)
    I_d_low  = run(V_G2_LOW)
    # bit=+1: diff = rates(HIGH) - rates(LOW)
    # bit=-1: diff = rates(LOW)  - rates(HIGH)
    ...
    return {
        ...
        "V_G2_LOW":  V_G2_LOW,
        "V_G2_HIGH": V_G2_HIGH,
        "I_d_at_V_G2_LOW_A":  I_d_low,
        "I_d_at_V_G2_HIGH_A": I_d_high,
...
def gate_for(mean_acc, ci_lo, ds_n5c_acc, fair_baseline):
...
def main():
    ...
    p.add_argument("--out",
                   default="results/z291_nsram_hdc_aboveknee/summary.json")
    args = p.parse_args()

    device = get_device()
    print(f"[z291] device={device}", flush=True)
    surr = load_surrogate_torch(args.surrogate, device)
    Xtr, ytr, Xte, yte = load_uci_har(args.data_root)
    print(f"[z291] train {Xtr.shape} test {Xte.shape}", flush=True)
    n_classes = int(max(ytr.max(), yte.max())) + 1

    # ---- 1) Symmetry diagnostic FIRST ----
    diag = symmetry_diagnostic(surr, device, V_G1_BIAS, V_G2_LOW, V_G2_HIGH,
                               T_steps=args.T_steps)
    print(f"\n[z291] === Symmetry diagnostic ===", flush=True)
    print(f"  V_G1_BIAS={V_G1_BIAS}  V_G2_LOW={V_G2_LOW}  V_G2_HIGH={V_G2_HIGH}",
          flush=True)
    print(f"  I_d @ V_G2=LOW ({V_G2_LOW}V): {diag['I_d_at_V_G2_LOW_A']:.3e} A",
          flush=True)
    print(f"  I_d @ V_G2=HIGH({V_G2_HIGH}V): {diag['I_d_at_V_G2_HIGH_A']:.3e} A",
          flush=True)
    ...
    if diag["symmetry_ratio"] is not None and (
            diag["symmetry_ratio"] > 2.0 or diag["symmetry_ratio"] < 0.5):
        print(f"  WARNING: asymmetry > 2x — expected partial failure, continuing",
              flush=True)
    ...
    summary = {
        "experiment": "z291_nsram_hdc_aboveknee_uci_har",
    ...
    print(f"\n[z291] DONE mean_acc={summary['mean_acc']} "
