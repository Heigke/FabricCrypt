"""z466 — Mario-target BBO fit, 7D (knee-gate extension of z465).

7D Bayesian optimisation (skopt.gp_minimize) over:
  - snap_Is              ∈ [1e-9, 1e-5]  A     (log)
  - R_body               ∈ [1e3, 1e7]    Ω     (log)
  - β = Bf               ∈ [10, 1e4]            (log)
  - C_body               ∈ [1e-15, 1e-11] F    (log)
  - snap_V_knee          ∈ [0.8, 1.8]    V     (linear)
  - snap_npn_V_knee      ∈ [0.8, 1.8]    V     (linear)
  - snap_npn_V_BE_offset ∈ [0.1, 0.5]    V     (linear)

z465 hit a structural ceiling at Bf=10^4 with the three knee-gate parameters
FIXED at SB_HOT defaults (1.6, 1.8, 0.3). z466 opens the knee-gates to test
whether I_D peak / Mario-target count can be raised.

Output: results/z466_mario_bbo_7d/
  summary.json
  bbo_convergence.png
...
Pre-registered gates (line 1 of run.log):
  ...
  DISCOVERY   = >=4 of 7 Mario targets within 30%
  AMBITIOUS   = >=6 of 7 within 30% AND I_D peak within 1 dec of 4.8 mA
                AND DC RMSE < 1.5 dec
  KILL_SHOT   = >=5 of 7 STILL FAIL (>= 3 PASS but <4): structural problem
                deeper than knee-gates.
"""
...
OUT = ROOT / "results/z466_mario_bbo_7d"
...
LOG.write(
    ...
    "DISCOVERY=>=4/7 targets within 30%% | "
    "AMBITIOUS=>=6/7 AND I_D peak within 1 dec of 4.8mA AND DC_RMSE<1.5dec | "
    "KILL_SHOT=>=5/7 STILL FAIL (structural problem deeper than knee-gates)\n")
LOG.flush()
...
def make_cfg_flags(snap_Is: float, snap_Bf: float, R_body: float, C_body: float,
                   snap_V_knee: float, snap_npn_V_knee: float,
                   snap_npn_V_BE_offset: float):
    """Return cfg flags dict for a 7D BBO point."""
    ...
    flags["snap_V_knee"]          = float(snap_V_knee)
    flags["snap_npn_V_knee"]      = float(snap_npn_V_knee)
    flags["snap_npn_V_BE_offset"] = float(snap_npn_V_BE_offset)
    return flags
...
def objective(x):
    """skopt objective: x = [log10(snap_Is), log10(R_body), log10(Bf), log10(C_body),
                              snap_V_knee, snap_npn_V_knee, snap_npn_V_BE_offset]."""
    snap_Is = 10.0 ** x[0]
    R_body  = 10.0 ** x[1]
    Bf      = 10.0 ** x[2]
    C_body  = 10.0 ** x[3]
    snap_V_knee          = float(x[4])
    snap_npn_V_knee      = float(x[5])
    snap_npn_V_BE_offset = float(x[6])
    t0 = time.time()
    cfg_flags = make_cfg_flags(snap_Is, Bf, R_body, C_body,
                               snap_V_knee, snap_npn_V_knee,
                               snap_npn_V_BE_offset)
    try:
    ...
    rec = {
        ...
        "snap_V_knee": snap_V_knee,
        "snap_npn_V_knee": snap_npn_V_knee,
        "snap_npn_V_BE_offset": snap_npn_V_BE_offset,
        "fit_transient": fit_t,
        "dc_rmse": dc,
        "dc_penalty": dc_pen,
        "fit_total": fit_total,
    ...
    log(f"  it={len(EVAL_HIST):3d}  Is={snap_Is:.2e}  Rb={R_body:.2e}  "
        f"Bf={Bf:.1f}  Cb={C_body:.2e}  Vk={snap_V_knee:.2f}  "
        f"Vk_n={snap_npn_V_knee:.2f}  Voff={snap_npn_V_BE_offset:.2f}  "
        f"fit_t={fit_t:.3f}  DC={dc:.2f}dec  total={fit_total:.3f}  ({wall:.1f}s)")
    return fit_total
...
def main():
    ...
    space = [
        ...
        Real( 0.8,  1.8, name="snap_V_knee"),
        Real( 0.8,  1.8, name="snap_npn_V_knee"),
        Real( 0.1,  0.5, name="snap_npn_V_BE_offset"),
    ]

    # Seed 1: SNAP_HOT canonical (knee-gate defaults)
    log("=== Sanity: canonical seed (SNAP_HOT defaults) ===")
    x_seed = [math.log10(6.0256e-9 * 5.0),  # snap_Is = SNAP_HOT default
              math.log10(1e7),                # R_body
              math.log10(417.0),              # Bf
              math.log10(1e-15),              # C_body
              1.6,                            # snap_V_knee (SNAP_BASE default)
              1.8,                            # snap_npn_V_knee
              0.3]                            # snap_npn_V_BE_offset
    f_seed = objective(x_seed)
    log(f"  SNAP_HOT seed fit_total = {f_seed:.3f}")

    # Seed 2: z465 best
    log("=== Sanity: z465 best seed ===")
    x_z465 = [math.log10(2.88e-7),            # snap_Is
              math.log10(3.87e5),              # R_body = 387kΩ
              math.log10(1.0e4),               # Bf = 10^4 ceiling
              math.log10(5.6e-15),             # C_body = 5.6 fF
              1.6, 1.8, 0.3]
    f_z465 = objective(x_z465)
    log(f"  z465-best seed fit_total = {f_z465:.3f}")

    log("=== BBO: gp_minimize, 50 iterations (7D) ===")
    t0 = time.time()
    res = gp_minimize(
        func=objective,
        dimensions=space,
    ...
    log(f"      Vknee={best['snap_V_knee']:.3f}  Vknee_npn={best['snap_npn_V_knee']:.3f}  "
        f"V_BE_off={best['snap_npn_V_BE_offset']:.3f}")
    ...
    cfg_flags_best = make_cfg_flags(
        best['snap_Is'], best['Bf'], best['R_body'], best['C_body'],
        best['snap_V_knee'], best['snap_npn_V_knee'],
        best['snap_npn_V_BE_offset'])
    ...
    ax.set_title(f"z466 7D BBO convergence (best={res.fun:.3f})")
    ...
    fig.suptitle(f"z466 best cell: Is={best['snap_Is']:.2e} Rb={best['R_body']:.1e} "
                 f"Bf={best['Bf']:.0f} Cb={best['C_body']:.2e}  "
                 f"Vk={best['snap_V_knee']:.2f} Vk_n={best['snap_npn_V_knee']:.2f} "
                 f"Voff={best['snap_npn_V_BE_offset']:.2f}")
    ...
    # Gate verdict (z466 pre-registered)
    Id_peak_meas = meas_best.get("Id_peak_A", float("nan"))
    Id_peak_tgt  = TARGETS["Id_peak_A"]
    if (math.isfinite(Id_peak_meas) and Id_peak_meas > 0):
        Id_dec_off = abs(math.log10(Id_peak_meas) - math.log10(Id_peak_tgt))
    else:
        Id_dec_off = float("inf")
    n_fail = 7 - n_within_30_hi
    if n_within_30_hi >= 6 and Id_dec_off <= 1.0 and dc_full < 1.5:
        verdict = "AMBITIOUS"
    elif n_within_30_hi >= 4:
        verdict = "DISCOVERY"
    elif n_fail >= 5:
        verdict = "KILL_SHOT"
    else:
        verdict = "INFRA_ONLY"
    log(f"GATE VERDICT: {verdict}  (n_within_30 hi-res={n_within_30_hi}/7, "
        f"I_D off by {Id_dec_off:.2f} dec, DC_full={dc_full:.3f})")

    # mario_target_table.md
    units = {"period_s":"s","Vd_peak_V":"V","Id_peak_A":"A","rise_s":"s","fall_s":"s",
             "Vbody_swing_V":"V","E_spike_J":"J"}
    lines = ["# Mario target table — z466 best (7D BBO)", "",
             f"Best params: snap_Is={best['snap_Is']:.4e}, R_body={best['R_body']:.4e}, "
             f"β=Bf={best['Bf']:.2f}, C_body={best['C_body']:.4e},",
             f"snap_V_knee={best['snap_V_knee']:.3f} V, "
             f"snap_npn_V_knee={best['snap_npn_V_knee']:.3f} V, "
             f"snap_npn_V_BE_offset={best['snap_npn_V_BE_offset']:.3f} V", "",
             f"Full DC RMSE = **{dc_full:.3f} dec**",
             f"I_D peak off Mario target by **{Id_dec_off:.2f} dec** "
             f"(achieved {meas_best.get('Id_peak_A', float('nan')):.3e} A, "
             f"target {TARGETS['Id_peak_A']:.3e} A)", "",
    ...
    honest = [
        "# z466 honest analysis (7D BBO, knee-gate extension of z465)", "",
        "## What we did",
        "7D Bayesian optimisation over (snap_Is, R_body, β, C_body,",
        "snap_V_knee, snap_npn_V_knee, snap_npn_V_BE_offset) against the same",
        "7 Mario targets used in z465. Fitness = weighted sum of relative errors",
        "+ DC RMSE penalty (kicks in past 2 dec).",
        "50 BBO iterations, gp_minimize (skopt), 20 initial random points,",
        "seeded with SNAP_HOT canonical AND z465 best.", "",
        "## Why z466",
        "z465 hit a structural ceiling at β=10^4: only 2/7 targets within 30%,",
        "I_D peak 4 decades below Mario's 4.8 mA. The three knee-gate params",
        "had been FIXED at SB_HOT defaults; z466 opens them.", "",
        "## Caveats", "",
        ...
        *structural, "",
        ...
        f"- I_D peak off Mario by {Id_dec_off:.2f} dec",
        f"- DC RMSE (full subset) = {dc_full:.3f} dec",
        f"- best fit_total = {best['fit_total']:.4f}", "",
        "## Comparison to z465",
        "- z465 best: 2/7 within 30%, I_D peak 0.20 µA (4.4 dec off 4.8 mA), DC=1.373 dec",
        f"- z466 best: {n_within_30_hi}/7 within 30%, "
        f"I_D peak {meas_best.get('Id_peak_A', float('nan')):.3e} A "
        f"({Id_dec_off:.2f} dec off), DC={dc_full:.3f} dec", "",
    ]
    ...
    summary = {
        "best_params": {
            ...
            "Bf":      best['Bf'],
            "C_body":  best['C_body'],
            "snap_V_knee":          best['snap_V_knee'],
            "snap_npn_V_knee":      best['snap_npn_V_knee'],
            "snap_npn_V_BE_offset": best['snap_npn_V_BE_offset'],
        },
        "Id_peak_decades_off": Id_dec_off,
