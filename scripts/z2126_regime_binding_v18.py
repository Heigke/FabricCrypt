#!/usr/bin/env python3
"""
z2126v18: Regime-Bound Dual LoRA — 2 Banks for Cold vs Hot
==========================================================
v18: z2124 base (26/40) with DUAL LoRA banks.

  z2124-z2125 diagnosis:
  - T7 kill-shot=1.004 is Phase A (ISA rounding) — structural limit
  - T18/T19: live analog worse than fixed (LoRA adds noise, no adaptation)
  - T22: regime classification=0% (hidden states carry no regime info)
  - T31-T37: pulse field constant (0.173), below detection threshold

  v18 strategy: Replace single LoRA bank with DUAL banks (cold/hot).
  The body gate interpolates between them: gate=0 → cold LoRA, gate=1 → hot LoRA.
  This creates regime-specific weights that produce genuinely different output
  in different DVFS states. Model MUST know its regime to output correctly.

  Also keeps contrastive loss (λ=0.3) and agreement modulation from z2124.
  Pure silicon coupling, no thermal softmax/noise.
...
Hardware setup:
  ...
  sudo HSA_OVERRIDE_GFX_VERSION=11.0.0 PYTORCH_ROCM_ARCH=gfx1100 \\
    venv/bin/python -u scripts/z2126_regime_binding_v18.py
"""
...
def main():
    ...
    print("=" * 60)
    print("z2126v18: Regime-Bound Dual LoRA — Cold/Hot Banks on z2124 Base")
    ...
    print(f"\n" + "=" * 60)
    print(f"z2126v18 Regime-Bound Dual LoRA GPT-2 LM: {n_pass}/{n_total} PASS")
    print("=" * 60)

    # Save results
    out_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'z2126_regime_binding.json')
    out_path = os.path.abspath(out_path)
    result_obj = {
        'experiment': 'z2126v18_regime_binding_dual_lora',
        'description': (
