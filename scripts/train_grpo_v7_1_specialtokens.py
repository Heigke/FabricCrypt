#!/usr/bin/env python3
"""
FEEL v7.1: Embodied GRPO - Special-token actions + on-policy z_feel conditioning (hook-free)

Key fixes vs v7.0:
- On-policy: sampling and scoring both use z_feel injection.
- Single-token action outputs: <|FEEL_OK|>, <|FEEL_WARM|>, <|FEEL_HOT|>, <|FEEL_REST|>, <|FEEL_CRITICAL|>
- Train only: injector + embedding/lm_head rows for new tokens (via gradient masking).
"""
...
class AdditiveZFeelInjector(nn.Module):
    def __init__(self, z_dim: int, embed_dim: int, scale: float = 0.10, dtype: torch.dtype = torch.bfloat16):
        super().__init__()
        self.scale = scale
...
class GRPOConfig:
    ...
    action_penalty_wrong: float = 0.8
    math_weight: float = 0.2
    ...
    def train(self, out_dir: str):
        ...
        try:
            for epoch in range(self.cfg.num_epochs):
                ...
                print(f"\\nEpoch {epoch+1}/{self.cfg.num_epochs} (instruct_prob={p:.2f})")
                for step in range(self.cfg.steps_per_epoch):
                    ...
                    print(f"  Step {step+1}: R={m['mean_reward']:.3f} ActRate={m['action_present_rate']:.0%} "
                          f"Align={m['alignment']:.0%} T={m['mean_temp']:.1f}C LR={m['lr']:.2e}")
                ...
                print("  Correlation report:", self.corr.report())
        finally:
            self.bg.stop()
...
def main():
    ...
    ap.add_argument("--output", default="models/feel_grpo_v7_1")
    ...
    ap.add_argument("--injection-scale", type=float, default=0.10)
    args = ap.parse_args()
