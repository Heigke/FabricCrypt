#!/usr/bin/env python3
"""
z2106: Activation Steering Embodied LM — Zero-LoRA Approach
=============================================================
Built on z2105 (Qwen3-8B). REMOVES ALL LoRA. Uses activation steering instead.
...
class EmbodiedQwen2(nn.Module):
    ...
    def _run_qwen3_attn(self, attn_module, q, k, v, position_embeddings, thermal_scale=None):
...
def evaluate_text_only_perplexity(model, test_data, kargs, tokenizer, n_batches=N_EVAL_BATCHES):
    """Text-only perplexity for language retention.

    Runs the model with ALL embodiment paths disabled (no ISA, no substrate).
    This is the closest apples-to-apples comparison with the frozen backbone.
    """
    model.eval()
    total_loss = 0.0
    total_tokens = 0
    with torch.no_grad():
        for i in range(0, min(len(test_data), n_batches * BS), BS):
            ...
            B = input_ids.shape[0]
            sensor_batch = {
                'analog': torch.zeros(B, ANALOG_DIM, device=DEVICE),
                'energy': torch.zeros(B, ENERGY_DIM, device=DEVICE),
                'freq': torch.zeros(B, FREQ_DIM, device=DEVICE),
                'thermal': torch.zeros(B, THERMAL_DIM, device=DEVICE),
                'pm_deep': torch.zeros(B, PM_DEEP_DIM, device=DEVICE),
                'smn_raw': torch.zeros(B, SMN_RAW_DIM, device=DEVICE),
                'gpu_metrics': torch.zeros(B, GPU_METRICS_DIM, device=DEVICE),
                'thm_spatial_a': torch.zeros(B, THM_SPATIAL_A_DIM, device=DEVICE),
                'thm_spatial_b': torch.zeros(B, THM_SPATIAL_B_DIM, device=DEVICE),
                'cpu_pmu': torch.zeros(B, CPU_PMU_DIM, device=DEVICE),
                'gpu_metrics_deep': torch.zeros(B, GPU_METRICS_DEEP_DIM, device=DEVICE),
                'fence': torch.zeros(B, FENCE_DIM, device=DEVICE),
                'status': torch.zeros(B, STATUS_DIM, device=DEVICE),
                'action': torch.zeros(B, ACTION_DIM, device=DEVICE),
                'delta': torch.zeros(B, DELTA_DIM, device=DEVICE),
                'intrinsic': torch.zeros(B, INTRINSIC_DIM, device=DEVICE),
                'reported_delta': torch.zeros(B, REPORTED_DELTA_DIM, device=DEVICE),
            }
...
def run_tests(model, test_data, kargs, baseline_ppl, tokenizer, dvfs_controller=None, kargs_b=None,
    ...
    # NEW: text-only perplexity (embodiment disabled) to verify language retention
    ppl_textonly = evaluate_text_only_perplexity(model, test_data, kargs=kargs, tokenizer=tokenizer)
    ratio_text = ppl_textonly / max(baseline_ppl, 1.0)
    ratio_r0 = ppl_r0 / max(baseline_ppl, 1.0)
    # PASS if text-only hasn't regressed (adapter didn't destroy language)
    t1_pass = ratio_text < 1.05
    results['T1_perplexity'] = {
        'ppl_r0': ppl_r0, 'ppl_r1': ppl_r1, 'baseline_ppl': baseline_ppl,
        'ppl_textonly': ppl_textonly,
        'ratio_textonly': ratio_text,
        'ratio_r0': ratio_r0, 'body_scale_r0': bs_r0, 'body_scale_r1': bs_r1,
        'pass': str(t1_pass)
    }
    print(f"T1 Perplexity: textonly={ppl_textonly:.2f} r0={ppl_r0:.2f} r1={ppl_r1:.2f} base={baseline_ppl:.2f} "
          f"ratio_text={ratio_text:.3f} ratio_r0={ratio_r0:.3f} bs={bs_r0:.3f} {'PASS' if t1_pass else 'FAIL'}")
    ...
    if delta_low and delta_high:
        ...
        for d in range(dl.shape[1]):
            try:
                ...
                pass
        t6_pass = (np.isfinite(max_t_d) and max_t_d > 3.0)
    else:
        max_t_d = 0
        t6_pass = False
    ...
    with torch.no_grad():
        for trial in range(20):
            ...
            # Gaslit: corrupt ONLY reported_delta (actual delta stays truthful)
...
def _generate_tokens_embodied_fixed(model, prompt_ids, sb_gen, kargs, n_tokens=32,
                                    mode='greedy', temperature=0.7, top_k=20, top_p=0.8,
                                    rep_penalty=1.2):
    """Generate tokens with embodiment enabled but fixed sensors.

    This probes whether substrate/LoRA/steering paths destabilize generation when
    sensors are held constant and temporal state is frozen.
    """
    gen_ids = prompt_ids
    generated_tok_ids = []
    for _ in range(n_tokens):
        out_gen = model(gen_ids, sb_gen, kargs,
                        skip_isa=True, skip_substrate=False,
                        freeze_body_state=True, skip_agreement=True)
...
def main():
    ...
    print("=" * 60)
    print("z2106: Qwen3-8B NormBounded LoRA + Steering Embodied LM")
    ...
    # Save results
    out_path = '/home/ikaros/Documents/claude_hive/AMD_gfx1151_energy/results/z2106_activation_steering_lm.json'
    final = {
        'experiment': 'z2106_normbounded_lora_lm',
        'description': 'z2106: Qwen3-8B NormBounded LoRA + Steering — LoRA clamped to 10% base norm, L2-normalized steering at 5% h_norm',
        'key_changes': {
