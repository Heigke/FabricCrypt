#!/usr/bin/env python3
"""
z2120v12: Silicon Unblocked — Let the Hardware Through
======================================================
v12: 5 silicon-close fixes to make the real analog signals audible.

  z2119 got 24/40. Root cause: probe_mode=False during training cut off
  ALL kernel proprioception (wall_clock64 jitter, WGP routing, tile var).
  Body gates received zeros(12) every forward pass. Pulse field converged
  to a constant (active=0.173 never changed). The silicon was screaming
  and we plugged our ears.

  v12 FIXES (all silicon-close, no software scaffolding):
  1. probe_mode=True during training — unblock kernel telemetry to gates
  2. PULSE_EPS scaled by layer weight norm — pulse field audible vs weights
  3. Gate init: small random (not zero) — real signals heard from batch 1
  4. PULSE_ALPHA 0.85->0.65 — pulse tracks real execution state per-batch
  5. LoRA layers 4-11 (8 layers, not just 8-11) — more HW-coupled surface
...
Hardware setup:
  ...
  sudo HSA_OVERRIDE_GFX_VERSION=11.0.0 PYTORCH_ROCM_ARCH=gfx1100 \\
    venv/bin/python -u scripts/z2120_silicon_unblocked_v12.py
"""
...
def main():
    ...
    print("=" * 60)
    print("z2120v12: Haptic Resonance GPT-2 — Lighter Backbone")
    ...
    print(f"\n" + "=" * 60)
    print(f"z2120v12 Haptic Resonance GPT-2 LM: {n_pass}/{n_total} PASS")
    print("=" * 60)

    # Save results
    out_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'z2120_silicon_unblocked.json')
    out_path = os.path.abspath(out_path)
    result_obj = {
        'experiment': 'z2120v12_silicon_unblocked',
        'description': (
            'v12: Silicon unblocked — probe_mode=True during training, '
            'PULSE_EPS scaled by weight norm, gate init small random (not zero), '
            'PULSE_ALPHA 0.65 (was 0.85), LoRA layers 4-11 (8 layers). '
            'All fixes silicon-close: let real kernel telemetry through to body gates.'
        ),
