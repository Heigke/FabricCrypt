#!/usr/bin/env python3
"""
z2114v6: Visceral Architecture — Multiplicative 3-Branch Embodied LM
=====================================================================
v6 changes from v5 (20/27):
  1. RunningZScore for fast features — replaces /1e6 normalization
  2. FAST_DIM=8: mean,std,tail,d_mean,spread,ewma_resid,burst,d_jitter
  3. Multiplicative 3-branch gate: g_fast * g_mid * g_slow (no additive shortcut)
  4. Modality dropout: force fast path learning (30% slow-drop, 20% fast-only)
  5. LoRA B tiny nonzero init (0.001) for immediate gate gradients
  6. Branch usage loss + logging for T28/T29
  7. 3-phase curriculum: calibration / reflex forcing / integration
  8. Contention co-kernel on separate CUDA stream
  9. T22 fix: use training confusion matrix + proper warmup
  10. T26 v6: zero/negate/noise interventions (not shuffle)
  11. New tests: T28 branch balance, T29 fast grad, T30 contention sensitivity
...
Hardware setup:
  ...
  sudo HSA_OVERRIDE_GFX_VERSION=11.0.0 PYTORCH_ROCM_ARCH=gfx1100 \
    venv/bin/python -u scripts/z2114_analog_linear_lm_v6.py
"""
...
# v6: 3-timescale body dimensions — FAST expanded to 8
FAST_DIM = 8   # mean,std,tail,d_mean,spread,ewma_resid,burst,d_jitter
...
BODY_DIM = FAST_DIM + MID_DIM + SLOW_DIM  # 20 total
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# v6: 3-TIMESCALE FORWARD CONTEXT with RunningZScore
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

class ForwardContext:
    """v6: 3-timescale body state snapshot with per-layer z-scoring.
    fast:  per-layer proprio (wall_clock64 jitter) — z-scored online
    ...
    def get_per_layer_fast_vec(self, layer_idx):
        """v6: Per-layer fast body vector [FAST_DIM=8] with online z-scoring.
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# CUSTOM HIP KERNEL — IDENTICAL to v5
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
_HIP_MODULE = None
...
def compile_hip_kernel():
    ...
    print("[HIP] Compiling v6 analog GEMM kernel (TILE=32 + wall_clock64 proprio)...")
...
"""
    ...
    try:
        ...
        print("[HIP] v6 Kernel compiled OK")
    except Exception as e:
        ...
        if hip_mod is not None:
            ...
            if self.mode_override is None:
                ...
                if len(ticks_pos) > 0:
                    ...
                    jitter = spread
                    # Burst flag: fraction of blocks > 2 sigma from mean
                    if std_t > 0:
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# v6: BODY-GATED LORA — MULTIPLICATIVE 3-BRANCH GATE
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
class BodyGatedLoRA(nn.Module):
    """v6: LoRA on gate_proj with multiplicative 3-branch gating.
    gate = sigmoid(g_fast) * sigmoid(g_mid) * sigmoid(g_slow)
    Each branch must contribute — no additive shortcut.
    Includes modality dropout and branch usage tracking."""
    ...
    def __init__(self, analog_linear, layer_idx, rank=8, alpha=16):
        ...
        # v6: tiny nonzero B init for immediate gate gradients
        self.lora_B = nn.Parameter(torch.randn(out_f, rank, dtype=dtype) * 0.001)
        # v6: THREE separate gates
        ...
        # Init: all weights zero, biases 0.5 -> sigmoid(0.5)~0.62, product~0.24
        nn.init.zeros_(self.gate_fast.weight)
        nn.init.zeros_(self.gate_mid.weight)
        nn.init.zeros_(self.gate_slow.weight)
        nn.init.constant_(self.gate_fast.bias, 0.5)
        nn.init.constant_(self.gate_mid.bias, 0.5)
        nn.init.constant_(self.gate_slow.bias, 0.5)
        # v6: modality dropout control
        self._modality_dropout = True
        # v6: branch usage tracking
        self.last_g_fast_norm = 0.0
        self.last_g_mid_norm = 0.0
        self.last_g_slow_norm = 0.0
        self.last_fast_grad_norm = 0.0

    def _apply_modality_dropout(self, g_fast, g_mid, g_slow):
        """Drop branches to force fast path learning."""
        r = random.random()
        if r < 0.3:
            return g_fast * g_mid        # drop slow
        elif r < 0.5:
            return g_fast                 # fast-only (proprioceptive reflex)
        elif r < 0.6:
            return g_mid * g_slow         # drop fast (test slow adequacy)
        else:
            return g_fast * g_mid * g_slow  # full
    ...
    def forward(self, x):
        ...
        # Fast gate: per-layer z-scored proprio
        ...
        g_fast = torch.sigmoid(self.gate_fast(fast_input))  # [rank]

        # Mid gate: gpu_metrics
        mid_input = torch.from_numpy(CTX.mid_vec.copy()).float().to(self.gate_mid.weight.device)
        g_mid = torch.sigmoid(self.gate_mid(mid_input))  # [rank]

        # Slow gate: thermal EMA
        slow_input = torch.from_numpy(CTX.slow_vec.copy()).float().to(self.gate_slow.weight.device)
        g_slow = torch.sigmoid(self.gate_slow(slow_input))  # [rank]

        # Track branch norms
        self.last_g_fast_norm = float(g_fast.detach().norm().item())
        self.last_g_mid_norm = float(g_mid.detach().norm().item())
        self.last_g_slow_norm = float(g_slow.detach().norm().item())

        # Multiplicative gate with optional modality dropout
        if self.training and self._modality_dropout:
            gate = self._apply_modality_dropout(g_fast, g_mid, g_slow)
        else:
            gate = g_fast * g_mid * g_slow  # [rank]
...
def run_phase_a(model, test_data, test_data_code, baseline_ppl):
    ...
    print("  v6: ALL layers + TILE=32 + wall_clock64 + z-scored proprio")
    print("=" * 60)
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# PHASE B: v6 3-Phase Curriculum Training
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def run_phase_b(model, train_data, test_data, test_data_code, baseline_ppl, phase_a_results):
    print("\n" + "=" * 60)
    print("PHASE B: v6 3-Phase Curriculum (multiplicative 3-branch gate)")
    print("  Phase1: Calibration | Phase2: Reflex forcing | Phase3: Integration")
    print("=" * 60)
    ...
    for batch_idx in range(N_TRAIN_BATCHES):
        ...
        for chunk_i in range(n_chunks - 1):
            ...
            # v6: Branch usage penalty (encourage fast branch to be non-trivial)
            ...
            if fast_norms:
                mean_fast_norm = np.mean(fast_norms)
                # Penalize if fast norm variance is too low (collapsed)
...
def _run_phase_b_tests(model, soma_head, test_data, test_data_code, baseline_ppl,
                       phase_a_results, results, branch_norms_log, fast_grad_norms_log):
    """Phase B test battery — v6 with fixed T22, stronger T26, new T28-T30."""
    ...
    # T22: Regime Classification — v6 FIX: use TRAINING confusion matrix
    print("\n[T22] Regime Classification (v6: training confusion matrix)...")
    # Training confusion matrix metrics
    conf = soma_head.confusion
    train_total = conf.sum()
    train_acc = float(np.trace(conf)) / max(float(train_total), 1)
    # Balanced accuracy from training confusion
    per_class_recall = []
    for c in range(N_REGIMES):
        row_sum = conf[c].sum()
        if row_sum > 0:
            per_class_recall.append(float(conf[c, c]) / float(row_sum))
    train_balanced = np.mean(per_class_recall) if per_class_recall else 0.0
    results['regime_accuracy'] = train_acc
    results['regime_balanced_accuracy'] = float(train_balanced)
    print(f"  Training: acc={train_acc:.4f}, balanced={train_balanced:.4f}")
    # Also report eval metrics
    if soma_preds_regime:
        eval_acc = float(np.mean(np.array(soma_preds_regime) == np.array(soma_targets_regime)))
        results['regime_eval_accuracy'] = eval_acc
        print(f"  Eval (25 samples): acc={eval_acc:.4f}")
        ...
        for _ in range(3):
            ...
            for layer_idx in LORA_LAYERS:
                ...
                if isinstance(g, BodyGatedLoRA):
                    ...
                    gf = torch.sigmoid(g.gate_fast(fast_t))
                    gm = torch.sigmoid(g.gate_mid(mid_t))
                    gs = torch.sigmoid(g.gate_slow(slow_t))
                    gate_val = (gf * gm * gs).mean().item()
    ...
    # T26: Causal Ablation v6 (zero/negate/noise)
    print("\n[T26] Causal Ablation v6 (zero/negate/noise)...")
...
def score_tests(phase_a, phase_b, baseline_ppl):
    ...
    if phase_b:
        ...
        add('T26', 'Causal Ablation', 'PASS' if ca.get('worst_ratio', 1) > 1.005 else 'FAIL',
...
def main():
    ...
    print("=" * 60)
    print("z2114v6: Visceral Architecture — Multiplicative 3-Branch Embodied LM")
    print("RunningZScore + FAST_DIM=8 + multiplicative gate + modality dropout")
    print("3-phase curriculum + contention co-kernel + branch usage tracking")
    print("=" * 60)
    ...
    print(f"\n" + "=" * 60)
    print(f"z2114v6 Visceral Architecture: {n_pass}/{n_total} PASS")
    print("=" * 60)
    ...
    result_obj = {
        'experiment': 'z2114v6_visceral_embodiment',
        'description': 'v6: RunningZScore + FAST_DIM=8 + multiplicative 3-branch gate + modality dropout + 3-phase curriculum',
