#!/usr/bin/env python3
"""
z2113: Deep Geometric Embodiment — 4 Poison-Pill Fixes
======================================================
Built on z2112 (first metabolic dialect attempt). z2112 generated punctuation soup
because of 3 specific "poison pills" in the code, plus forced-magnitude noise injection.

  FOUR SURGICAL FIXES:

  1. FIX THE PRESENCE BUG: Sensors reading 0.0 (idle GPU) were counted as DEAD,
     collapsing presence_frac to ~0.12 → 90% anesthesia. Now presence is based on
     API validity (valid_mask from read_all_sensor_dict), not numerical value.

  2. REMOVE TELEMETRY INTERLEAVING: interleave_telemetry_tokens() was overwriting
     text tokens every 32 steps with hardware IDs, destroying syntax. DELETED.
     Domain split (wikitext vs code) already makes hardware informationally necessary
     via freq_gate routing to correct LoRA adapter.

  3. CAPPED (NOT FORCED) MAGNITUDE: Fixed 2% forced noise → language destruction.
     Now: allow LoRA/FiLM to start at 0 (safe baseline), CAP at 5% max. Same for
     FiLM (2% cap) and steering (5% cap). Model learns naturally, can't explode.

  4. GATE THERMAL DELIRIUM: ThermalSoftmax clamp [0.70, 1.80] → 1.43x attention
     amplification across 18 layers. Tightened to [0.85, 1.15] and gated by body_scale
     so thermal→1.0 when ablated.

  Preserved from z2112:
  - Metabolic dialect (casual vs formal text style per regime)
  - Deterministic body_scale (no learnable mute button)
  - IA3 removed, ISA-coupled FiLM at layers 15/20/25
...
def read_all_sensor_dict(prev_df=None, prev_action=None, lite=False, prev_cpu_pmu_snapshot=None):
    ...
    # z2113 FIX 1: valid_mask based on API availability, NOT sensor numerical value.
    # Sensors reading 0.0 (idle GPU) are VALID, not dead.
    # Order matches N_SUBSTRATE_TOKENS: delta, analog, energy, freq, intrinsic,
    #   thermal, pm_deep, smn_raw, gpu_metrics, thm_spatial_a, thm_spatial_b,
    #   cpu_pmu, status, action, reported_delta, gpu_metrics_deep, fence
    valid_mask = torch.tensor([
        1.0,  # delta — always valid (computed by ISA kernel or zero-init)
        1.0,  # analog — always valid (basic sysfs reads)
        1.0,  # energy — always valid (RAPL)
        1.0,  # freq — always valid (sclk always readable)
        1.0,  # intrinsic — always valid (ISA kernel output)
        1.0,  # thermal — always valid (sysfs hwmon)
        0.0 if lite else 1.0,  # pm_deep — skipped in lite mode
        0.0 if lite else (1.0 if SMN_AVAILABLE else 0.0),  # smn_raw
        0.0 if lite else 1.0,  # gpu_metrics — skipped in lite mode
        1.0 if SMN_AVAILABLE else 0.0,  # thm_spatial_a — needs SMN
        1.0 if SMN_AVAILABLE else 0.0,  # thm_spatial_b — needs SMN
        1.0 if CPU_PMU_AVAILABLE else 0.0,  # cpu_pmu
        1.0,  # status — always valid
        1.0,  # action — always valid
        1.0,  # reported_delta — always valid (copy of delta)
        0.0 if lite else 1.0,  # gpu_metrics_deep — skipped in lite
        1.0 if FENCE_AVAILABLE else 0.0,  # fence
    ], dtype=torch.float32)
    ...
    return {
        ...
        'valid_mask': valid_mask,  # z2113: API-based validity, not value-based
...
class BodyEncoder(nn.Module):
    ...
    def forward(self, sensor_dict, availability_mask=None, freeze_temporal=False, valid_mask=None):
        """Forward pass.
        availability_mask: optional [B, N_SUBSTRATE_TOKENS] binary tensor (legacy).
        valid_mask: z2113 FIX 1 — API-based sensor validity [B, 17] or [17].
            1.0 = sensor API available, 0.0 = lite-skipped or hardware absent.
            Replaces value-based presence detection that counted 0-reading sensors as dead.
        ...
        # z2113 FIX 1: DETERMINISTIC body_scale from API-based valid_mask.
        # z2112 bug: (tok.abs().sum() > 1e-8) counted zero-valued sensors as DEAD.
        # cpu_pmu deltas, fence rings, status vectors legitimately read 0.0 when GPU idle.
        # This collapsed presence_frac to ~0.12 → "90% anesthesia" → punctuation soup.
        # NOW: valid_mask is set by API availability (1.0 if hw API works, 0.0 if skipped).
        if valid_mask is not None:
            # valid_mask: [B, 17] or [17] — 1.0 = sensor API available, 0.0 = lite-skipped/absent
            vm = valid_mask
            if vm.dim() == 1:
                vm = vm.unsqueeze(0).expand(B, -1)
            presence_frac = vm.mean(dim=-1, keepdim=True)  # [B, 1]
        elif availability_mask is not None:
            ...
            presence_frac = torch.ones(B, 1, device=tokens[0].device)
...
class NormBoundedLoRALinear(nn.Module):
    ...
    def forward(self, x, regime_gate=None, body_scale=None, domain_gate=None):
        ...
        # 4. z2113 FIX 3: CAPPED (not forced) magnitude.
        # z2112 forced 2% = injecting 2% pure noise from random init → killed language.
        # Now: allow LoRA to start at 0 (safe baseline) but CAP at 5% max.
        # Natural learning from 0, explosion prevention at 5%.
        base_norm = base_out.float().norm(p=2, dim=-1, keepdim=True).detach()
        lora_norm = lora_out.float().norm(p=2, dim=-1, keepdim=True)
        max_lora_norm = base_norm * 0.05  # CAP at 5% — not forced
        scale_factor = torch.clamp(max_lora_norm / (lora_norm + 1e-8), max=1.0)
        lora_out = lora_out * scale_factor  # natural magnitude, capped at 5%
...
class FiLMLayer(nn.Module):
    ...
    def forward(self, h_normed, cond_vec, body_scale=None, hw_entropy=None):
        ...
        # z2113 FIX 3: CAPPED (not forced) FiLM magnitude.
        # z2112 forced 1% = noise injection from random init. Now: cap at 2%.
        h_norm = h_normed.float().norm(dim=-1, keepdim=True).detach().clamp(min=1e-8)  # [B, T, 1]
        max_film_norm = h_norm * 0.02  # CAP at 2%

        g_delta_raw = g_raw - 1.0  # raw delta from identity
        g_delta_norm = g_delta_raw.float().norm(dim=-1, keepdim=True).clamp(min=1e-8)
        g_scale = torch.clamp(max_film_norm / g_delta_norm, max=1.0)
        g_delta = g_delta_raw * g_scale  # natural magnitude, capped at 2%

        b_delta_norm = b_raw.float().norm(dim=-1, keepdim=True).clamp(min=1e-8)
        b_scale = torch.clamp(max_film_norm / b_delta_norm, max=1.0)
        b_delta = b_raw * b_scale  # natural magnitude, capped at 2%
...
class EmbodiedQwen2(nn.Module):
    ...
    def forward(self, input_ids, sensor_dict, kargs, labels=None,
        ...
        # z2113 FIX 1: Pass valid_mask from sensor_dict to body_encoder
        _valid_mask = sensor_dict.get('valid_mask', None)
        ...
        # z2113 FIX 2: No telemetry tokens in text stream — pure base vocab embedding.
        ...
        if skip_substrate:
            ...
            raw_thermal_scale = torch.exp(alpha * delta10).clamp(0.85, 1.15)  # z2113 FIX 4: tighter [0.85, 1.15]
        ...
        for i, layer in enumerate(self.backbone.model.layers):
            ...
            if i in self.steering_layer_set:
                ...
                # z2113 FIX 3: CAPPED steering — direction learned, magnitude capped at 5%
                ...
                if not skip_substrate:
                    ...
                    # z2113: capped at 5%, not forced — natural magnitude * body_scale
                    steer_raw = steer_dir * bs  # direction * amplitude
                    steer_norm = steer_raw.float().norm(dim=-1, keepdim=True).clamp(min=1e-8)
                    max_steer = h_norm * 0.05  # CAP at 5%
                    steer_scale = torch.clamp(max_steer / steer_norm, max=1.0).to(hidden_states.dtype)
                    hidden_states = hidden_states + steer_raw * steer_scale
            else:
        ...
        if labels is not None:
            ...
            # z2113 FIX 2: Pure text loss — no telemetry tokens in stream.
...
def train_lm_epoch(model, train_data, optimizer, epoch, kargs_a, tokenizer,
    ...
    for batch_num in range(min(100, len(indices_wiki))):  # max 100 batches per epoch
        ...
        # z2113 FIX 2: NO telemetry interleaving — it was destroying text syntax.
        # Domain split (wikitext vs code) already makes hardware necessary via freq_gate.
        ...
        sensor_batch['intrinsic'] = torch.zeros(B, INTRINSIC_DIM, device=DEVICE)
        sensor_batch['reported_delta'] = sensor_batch['delta'].clone()
        # z2113 FIX 1: Pass valid_mask from sensor read (API-based, not value-based)
        if 'valid_mask' in sd:
            sensor_batch['valid_mask'] = sd['valid_mask'].unsqueeze(0).expand(B, -1).to(DEVICE)
        ...
        if KL_LAMBDA > 0 and current_regime == 0:  # z2106: KL only on regime 0 — let regime 1 freely specialize
            ...
            with torch.no_grad():
                # z2113: No telem tokens in stream — input_ids are pure base vocab.
            ...
            # z2113: No telem masking needed — pure text stream
            ...
            kl_loss = (kl_per_pos.sum() / kl_per_pos.numel()).clamp(max=10.0)
...
def generation_sanity_check(model, test_data, kargs, tokenizer, phase_name=""):
    ...
    model.eval()
    model.body_encoder.reset_state()
    print(f"\n[z2113] Generation sanity check after {phase_name}...")
    try:
        with torch.no_grad():
...
def main():
    ...
    print(f"\n[z2113] Loading code data for regime 1 (domain shift + formal dialect)...")
    train_data_code = None
    test_data_code = None
    try:
    ...
    print(f"\n[z2113 STEERING] Domain split + metabolic dialect")
    ...
    # z2113 FIX 2: Telemetry interleaving REMOVED — no telem_proj or telem_lm_head needed.
    # Domain split (wikitext vs code) makes hardware necessary via freq_gate routing.
    ...
    # Save results
    out_path = '/home/ikaros/Documents/claude_hive/AMD_gfx1151_energy/results/z2113_rigorous_lm.json'
    final = {
        'experiment': 'z2113_rigorous_lm',
        'description': 'z2113: Capped Embodiment — API-valid presence + No telem interleave + Capped magnitude + Tight thermal',
        'key_changes': {
    ...
    try:
        ...
        torch.save({
            ...
            # z2113: telem_proj/telem_lm_head REMOVED (no telemetry interleaving)
