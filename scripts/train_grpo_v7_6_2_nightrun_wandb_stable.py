...
def compute_stress_level(
    ...
    # Require stronger evidence for CRITICAL to reduce label flip-flopping near the boundary.
...
class ValidatedTrainer:
    ...
    def step(self, problems: List[Dict]) -> Dict:
        ...
        for g in groups:
            for t in g:
                adv = torch.tensor(max(-2.0, min(2.0, t.advantage)), device=self.device, dtype=torch.float32)
                ...
                if self.cfg.force_action_first:
                    logits = logits.masked_fill(self.action_mask == 0, float('-inf'))
                # Compute everything on the 5-way action simplex ONLY (avoids 0 * -inf NaNs)
                ...
                chosen_logp = action_logp[0, pos]

                if not torch.isfinite(chosen_logp):
                    continue

                if ent_coef > 0:
                    action_probs = action_logp.exp()
                    entropy = -(action_probs * action_logp).sum(dim=-1).mean()
                    loss = -(adv * chosen_logp) - ent_coef * entropy
                else:
                    loss = -(adv * chosen_logp)
