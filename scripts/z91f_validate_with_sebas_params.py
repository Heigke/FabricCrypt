...
def patch_model_values(model, type_n: bool = True):
    ...
    pmap = {
        ...
        # R-29 (2026-05-14): showmod on ngspice-42 reveals lpe0n IS actually
        # resolved to 1.2439e-7 — the 2026-05-02 "fallback to default 1.74e-7"
        # assumption was wrong. Using card value closes ~88mV Vth gap → ~2.6 dec
        # Ids recovery. (Was: 1.74e-7 stale BSIM4 default.)
        "lpe0":  1.2439e-7,
    ...
    # A.5.y (2026-05-02 / corrected 2026-05-03 11:30): replicate ngspice-42's
    # documented .model parameter handling. Per the ngspice manual and source
    # (src/spicelib/parser/inpgmod.c, src/frontend/inpeval.c), `.param`
    # substitution inside .model bodies requires {...} braces or single
    # quotes; bare identifiers are not substituted and ngspice falls back
    # to BSIM4 defaults. Sebas's M2 card uses bare identifiers throughout
    # (e.g. `+vth0 = vth0n  wvth0 = -1.66e-8  pvth0 = pvth0n`), so several
    # parameters land at default in ngspice. Empirically, on lines that mix
    # bare identifiers with numeric literals, downstream tokens (wvth0)
    # also land at 0 — likely a line-desync side-effect of the failed
    # bare-identifier parse. Since the measured I-V data was fit against
    # ngspice's actual output, pyport must mirror the same fallback.
    # Verified via debug-printf-instrumented ngspice rebuild (build_dbg, A.5.x).
    # NOT a parser bug; it is documented (if easy-to-miss) syntax behaviour.
    # A feature-request draft for a parse-time warning is queued (see brief
    # Limitations bullet, 2026-05-03 11:14).
    for dropped in ("wvth0", "lvth0", "pvth0",
                    "wvoff", "voffl",
                    "wvsat", "lvsat", "pvsat",  # vsat binning (small)
                    "wnfactor", "lnfactor", "pnfactor",
                    "wlpe0",  # lpe0 binning
                    "wags", "lags", "pags",  # ags binning (was 2.6× factor)
                    # A.5.aa (corrected 2026-05-03 11:30): also lands at 0
                    # in ngspice's actual evaluation. `phin` was previously
                    # framed as a parser issue; it is more likely the same
                    # bare-identifier-mediated line-desync mechanism.
                    "phin",   # lands at 0 in ngspice; pyport mirrors
                    "ww",     # likewise — pyport mirrors the empirical 0
                    ):
        if dropped in model._values:
            model._values[dropped] = 0.0
    # R-29 (2026-05-14): showmod confirms ngspice resolves toxn=4e-9 correctly;
    # earlier "ngspice defaults to 3e-9" assumption was incorrect. Reverting to
    # card-defined 4e-9. Part of Vth gap (88mV) / Ids (2.6 dec) recovery.
    model._values["toxe"] = 4.0e-9
    model._values["toxp"] = 4.0e-9
    model._values["toxm"] = 4.0e-9
    # R-37 (z356, 2026-05-14): the prior "binunit=1" override was based on
    # an INCORRECT assumption about ngspice's parser. ngspice-42 prints
    # `.model nmosdnwfb nmos level=14 version=4.5 binunit=2 ...` and
    # actually uses binunit=2 (Inv_L = 1.0/Leff, m^-1).  Forcing binunit=1
    # in pyport (Inv_L = 1e-6/Leff, µm^-1) collapses every l/w/p binning
    # coefficient by 1e6, which silently zeroed the lbeta0=-9.5e-7 binning
    # on M1.  Effective beta0 should be 10.0 (not 19), giving Iii ≈ ngspice
    # isub.  Removing the override closes the 3.43-dec impact-ion gap to
    # <0.5 dec across the 27-bias grid.  Verified at flagship M1 standalone:
    # py Iii = 5.08e-4 vs ngspice isub = 5.08e-4 (was 3.51e-6).
