#!/usr/bin/env python3
"""
z2115v7: Substrate-IS-Computation — Intra-Kernel Feedback Embodied LM
=====================================================================
v7 changes from v6 (27/30):
  1. NEW HIP KERNEL: intra-tile feedback rounding (prev acc → rounding mode)
  2. Per-block analog correction: output += f(timing,cycles,wgp) * learned_scale
  3. Kernel outputs: proprio + correction + wgp_id + feedback_state per block
  4. Fast corrective adapter: separate rank-4 LoRA only fast path controls
  5. Additive gate pre-activation: sigmoid(fast+mid+slow+bias) single sigmoid
  6. Kernel correction_scale learned per layer, clamped [0, 0.1]
  7. Counterfactual batches (25%) with fast perturbed at same DVFS
  8. Counterfactual consistency loss: fast change → require LoRA output change
  9. T22 fixed: held-out eval with unseen PRBS schedule
  10. T26a/b/c: matched-state, kernel correction, logit sensitivity ablations
  11. FAST_DIM=12: +correction_mean/std, +wgp_diversity, +feedback_entropy
...
Hardware setup:
  ...
  sudo HSA_OVERRIDE_GFX_VERSION=11.0.0 PYTORCH_ROCM_ARCH=gfx1100 \
    venv/bin/python -u scripts/z2115_analog_linear_lm_v7.py
"""
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# v7 HIP KERNEL — Intra-tile feedback rounding + analog correction + WGP ID
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
_HIP_MODULE = None
...
def compile_hip_kernel():
    ...
    print("[HIP] Compiling v7 feedback kernel (intra-tile rounding + correction + WGP)...")
...
__global__ void analog_feedback_gemm_v7_kernel(
    ...
    for (int t = 0; t < n_tiles; t++) {
        ...
        // Rounding mode depends on: previous tile accumulation + cycles + wgp_id
        unsigned int acc_bits = __float_as_uint(acc);
        unsigned int feedback = (acc_bits >> 16) ^ prev_tile_bits ^ (cycles & 0xFFu);
        unsigned int rm = feedback & 0x3u;  // 4 FP16 rounding modes from feedback

        // Also mix in stress threshold for external control
        unsigned int hw_spin = cycles & 0xFFu;
        if ((int)hw_spin < stress_threshold) {
            rm = 0x1u;  // stress override: round toward +inf
        }

        unsigned int rm_both = (rm & 0x3u) | ((rm & 0x3u) << 2) | (old_mode & 0xF0u);
        unsigned int new_mode = (old_mode & ~0xFFu) | rm_both;
        // Promote VGPR→SGPR: new_mode derived from per-thread acc (VGPR), but s_setreg needs SGPR
        ...
        // FP16 accumulation (result depends on feedback-driven rounding mode)
    ...
    // v7: Per-block analog correction — timing-derived, stays on GPU
    unsigned long long t_end = wall_clock64();
    unsigned long long dt = t_end - t_start;

    // Correction factor: derived from timing + WGP identity + feedback state
    float dt_f = (float)(dt & 0xFFFFu);
    float timing_norm = dt_f * 1.52587890625e-5f;  // / 65536
    float wgp_phase = (float)(wgp_id & 0x7u) * 0.125f;  // [0, 0.875]
    float fb_norm = (float)(feedback_accum & 0xFFu) * 0.00390625f;  // / 256

    // Analog correction: small, bounded, hardware-derived
    float correction = (timing_norm - 0.5f) * wgp_phase * correction_scale;
    // Add feedback-dependent component
    correction += (fb_norm - 0.5f) * correction_scale * 0.5f;

    if (row < M && col < N) {
        float corrected = acc + correction * acc;  // multiplicative correction
    ...
    // v7: Write all 4 per-block outputs (thread 0,0 only)
...
std::vector<torch::Tensor> analog_gemm_v7(
    ...
    if (corr_scale < 0.0f) corr_scale = 0.0f;
    if (corr_scale > 0.1f) corr_scale = 0.1f;

    analog_feedback_gemm_v7_kernel<<<grid, block>>>(
    ...
    return {C, proprio, corr_out, wgp_out, fb_out};
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("analog_gemm_v7", &analog_gemm_v7, "v7 feedback GEMM with correction + WGP + feedback");
}
"""

    try:
        _HIP_MODULE = load_inline(
            name='analog_gemm_v7_ext',
            cpp_sources=[],
        ...
        print("[HIP] v7 Kernel compiled OK")
    except Exception as e:
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# v7: ANALOG LINEAR — kernel feedback + correction + WGP
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
class AnalogLinear(nn.Module):
    """v7: Reads ForwardContext, writes proprio + correction + wgp + feedback back.
    ...
    def __init__(self, original_linear, layer_idx, mode_override=None):
        ...
        # v7: learned correction scale (set by BodyGatedLoRA wrapper, default 0)
        self._correction_scale = 0.0
    ...
    def forward(self, x):
        ...
        if hip_mod is not None:
            ...
            # v7: pass correction_scale, get 5 outputs
            results = hip_mod.analog_gemm_v7(x_bf16, w_bf16, round_mode,
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# v7: BODY-GATED LORA — ADDITIVE GATE + FAST CORRECTIVE ADAPTER
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
class BodyGatedLoRA(nn.Module):
    """v7: LoRA on gate_proj with ADDITIVE gate + separate fast corrective adapter.
    ...
    Learned correction_scale per layer passed to kernel."""
    ...
    def __init__(self, analog_linear, layer_idx, rank=8, alpha=16):
        ...
        # v7: Learned kernel correction scale, clamped [0, 0.1]
        self.correction_scale = nn.Parameter(torch.tensor(0.001, dtype=torch.float32))
    ...
    def forward(self, x):
        # v7: Set correction_scale on the AnalogLinear before it runs the kernel
        self.analog_linear._correction_scale = float(self.correction_scale.clamp(0, 0.1).item())
        ...
        return base + main_lora_out.to(x.dtype) + fast_adapter_out.to(x.dtype)
...
def run_phase_b(model, train_data, test_data, test_data_code, baseline_ppl, phase_a_results):
    print("\n" + "=" * 60)
    print("PHASE B: v7 3-Phase Curriculum (additive gate + fast corrective adapter)")
    print("  Phase1: Calibration | Phase2: Reflex forcing + CF | Phase3: Integration")
    print("=" * 60)
    ...
    for batch_idx in range(N_TRAIN_BATCHES):
        ...
        for chunk_i in range(n_chunks - 1):
            ...
            # v7: Kernel correction auxiliary loss — encourage correction_scale > 0
            corr_loss = torch.tensor(0.0, device=DEVICE)
            for layer_idx in LORA_LAYERS:
                g = model.model.layers[layer_idx].mlp.gate_proj
                if isinstance(g, BodyGatedLoRA):
                    # Minimum effect: penalize if correction_scale is too small
                    cs = g.correction_scale.clamp(0, 0.1)
                    corr_loss = corr_loss + F.relu(0.005 - cs)
            corr_loss = corr_loss * 0.01
...
def _run_phase_b_tests(model, soma_head, test_data, test_data_code, baseline_ppl,
                       phase_a_results, results, branch_norms_log, fast_grad_norms_log):
    """Phase B test battery — v7 with held-out T22, T26a/b/c, new T28-T30."""
...
def main():
    ...
    print("=" * 60)
    print("z2115v7: Substrate-IS-Computation — Intra-Kernel Feedback Embodied LM")
    print("FAST_DIM=12 + additive gate + fast corrective adapter + kernel correction")
    print("Intra-tile feedback rounding + WGP identity + counterfactual loss")
    print("=" * 60)
    ...
    print(f"\n" + "=" * 60)
    print(f"z2115v7 Substrate-IS-Computation: {n_pass}/{n_total} PASS")
    print("=" * 60)

    # Save
    out_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'z2115_analog_linear_lm.json')
    out_path = os.path.abspath(out_path)
    result_obj = {
        'experiment': 'z2115v7_substrate_is_computation',
        'description': 'v7: Intra-tile feedback rounding + kernel correction + WGP + additive gate + fast corrective adapter + counterfactual loss',
