#!/usr/bin/env python3
"""
z2108: Rigorous Test Battery — Causal Masking + Test Fixes
=============================================================
Built on z2107. CRITICAL fixes for test validity:

  z2108 fixes:
  1. CRITICAL: Causal mask for ALL layers (was missing on non-steering layers 0-11, 30-35)
     - Uses HF create_causal_mask() → 4D additive mask passed to every layer
     - _run_qwen3_attn uses additive mask instead of boolean triu
     - Logit-equivalence sanity test: skip_substrate must match backbone logits
  2. T6: Replace t-test with Cohen's d effect-size (ISA delta is near-deterministic → t=inf)
  3. T14: RAPL wraparound uses max_energy_range_uj (not 1<<32), avoids package/subzone double-count
  4. T23/T34: Replace LZ n/log2(n) normalization with zlib compression ratio (valid at small n)
  5. T19/T22: Pass skip_isa=False explicitly to prevent auto-skip when sensors zeroed
...
Hardware setup (run BEFORE launching, as root or with sudo):
  ...
  # Then run with sudo for MSR, FENCE, perf_event access:
  #   sudo HSA_OVERRIDE_GFX_VERSION=11.0.0 python z2105_multiobjective_lm.py
"""
...
RAPL_PATHS = {}
RAPL_MAX_RANGE = {}  # z2108: per-domain max_energy_range_uj for correct wraparound

def check_rapl():
    global RAPL_AVAILABLE, RAPL_PATHS, RAPL_MAX_RANGE
    base = '/sys/class/powercap'
    # z2108: Only read package-level (intel-rapl:0), NOT subzones (intel-rapl:0:0)
        ...
        if os.path.exists(ej):
            ...
            try:
                with open(name_path, 'r') as f:
                    name = f.read().strip()
                RAPL_PATHS[name] = ej
                # z2108: Read max_energy_range_uj for correct wraparound
...
def compute_batch_joules(before, after, gpu_ppt_mw, elapsed_s=None):
    ...
    z2108: Uses max_energy_range_uj for wraparound (not 1<<32).
    """
    total_uj = 0
    for name in before:
        if name in after:
...
class EmbodiedQwen2(nn.Module):
    ...
    def forward(self, input_ids, sensor_dict, kargs, labels=None,
        ...
        # z2108: Auto-detect only when skip_isa is NOT explicitly set by caller
        ...
        if skip_substrate:
            ...
            # z2105 FIX: thermal_scale >= 1.0 ALWAYS. Never sharpen attention.
            # Sharpening (scale < 1) causes greedy repetition collapse post-training.
            # Hot GPU → flatter attention (diffuse). Cold GPU → normal (scale=1.0).
            delta10 = (temp_c - 50.0) / 10.0  # per-10°C units
            thermal_scale = torch.exp(alpha * F.relu(delta10)).clamp(1.0, 1.25)  # z2106: 1.25 not 1.8 (1.8 destroys attention)
        ...
        # z2108 CRITICAL: Create proper 4D causal mask for ALL layers
        ...
        mask_kwargs = dict(
            ...
            input_embeds=inputs_embeds,  # z2108: this version of transformers uses input_embeds (not inputs_embeds)
        ...
        for i, layer in enumerate(self.backbone.model.layers):
            ...
            # z2108: Get the proper causal mask for this layer type
            ...
            if i in self.steering_layer_set:
                ...
                hidden_states = layer.mlp.act_fn(gate_out) * up_out
                hidden_states = layer.mlp.down_proj(hidden_states, regime_gate=_rg, body_scale=_bs)
                ...
                # Standard layer pass — z2108: pass causal mask to prevent future-token leakage
    ...
    def _run_qwen3_attn(self, attn_module, q, k, v, position_embeddings,
        ...
        z2108: Uses HF-compatible additive causal mask (4D float tensor).
        ...
        # z2108: Scaled dot-product attention with ADDITIVE causal mask (matches HF Qwen3)
        ...
        # z2108: Additive causal mask (4D float: 0.0 = attend, -inf = mask)
        if causal_mask is not None:
            causal_mask_slice = causal_mask[:, :, :, :T]  # slice to key length
            attn_weights = attn_weights + causal_mask_slice
        else:
            # Fallback: manual causal mask (should not happen in z2108)
        ...
        # z2108: Match HF attention dropout behavior (only during training)
...
def train_lm_epoch(model, train_data, optimizer, epoch, kargs_a, tokenizer,
    ...
    for batch_num in range(min(100, len(indices_wiki))):  # max 100 batches per epoch
        ...
        ce_loss_val = out['loss'].item()  # z2108: track CE-only for PPL interpretation
        loss = out['loss']
...
def run_tests(model, test_data, kargs, baseline_ppl, tokenizer, dvfs_controller=None, kargs_b=None,
    ...
    # z2108: LOGIT-EQUIVALENCE SANITY TEST (non-negotiable)
            ...
            sensor_batch = {k: torch.zeros(B, d, device=DEVICE) for k, d in
                           zip(['delta', 'analog', 'energy', 'freq', 'intrinsic',
                               ...
                               [DELTA_DIM, ANALOG_DIM, ENERGY_DIM, FREQ_DIM, INTRINSIC_DIM,
                                ...
                                GPU_METRICS_DEEP_DIM, FENCE_DIM])}
            out = model(input_ids, sensor_batch, kargs, labels=labels,
                        regime_gate_override=torch.ones(BS, device=DEVICE))  # z2106: regime 1 (code)
            if out['loss'] is not None:
    ...
    if delta_low and delta_high:
        ...
        # z2108: Cohen's d effect-size instead of t-test (ISA delta is near-deterministic → t=inf)
        # Cohen's d = |mean_diff| / pooled_std. For near-deterministic data, use
        # mean L2 separation as fallback when pooled_std ≈ 0
        max_cohens_d = 0
        mean_l2_sep = 0
        for d in range(dl.shape[1]):
            mean_diff = abs(dl[:, d].mean() - dh[:, d].mean())
            pooled_var = (dl[:, d].var(ddof=1) + dh[:, d].var(ddof=1)) / 2
            pooled_std = max(np.sqrt(pooled_var), 1e-10)
            cd = mean_diff / pooled_std
            if np.isfinite(cd) and cd > max_cohens_d:
                max_cohens_d = cd
        # Also compute L2 separation of mean vectors (robust when variance ≈ 0)
        mean_l2_sep = np.linalg.norm(dl.mean(axis=0) - dh.mean(axis=0))
        # PASS: Cohen's d > 0.8 (large effect) OR mean L2 separation > 0.01
        t6_pass = max_cohens_d > 0.8 or mean_l2_sep > 0.01
    else:
        max_cohens_d = 0
        mean_l2_sep = 0
        t6_pass = False
    results['T6_isa_delta'] = {
        'cohens_d': float(max_cohens_d), 'mean_l2_sep': float(mean_l2_sep),
        'pass': str(t6_pass)
    }
    print(f"T6 ISA Delta: Cohen's_d={max_cohens_d:.2f} L2_sep={mean_l2_sep:.4f} "
    ...
    with torch.no_grad():
        for i in range(0, min(len(_t19_data), 30 * BS), BS):
            ...
            # z2108: Pass skip_isa=False to prevent auto-skip (keeps internal pathways identical)
            sensor_batch_oracle = {}
            for k in ['analog', 'energy', 'freq', 'thermal', 'pm_deep', 'smn_raw',
                       'gpu_metrics', 'thm_spatial_a', 'thm_spatial_b', 'cpu_pmu',
                       'status', 'action']:
            ...
            out_oracle = model(input_ids, sensor_batch_oracle, kargs, labels=labels,
                              ...
                              skip_isa=False)  # z2108: prevent auto-skip on zero sensors
    ...
    # z2108: Use zlib compression ratio instead of LZ n/log2(n) normalization
...
def generation_sanity_check(model, test_data, kargs, tokenizer, phase_name=""):
    ...
    model.eval()
    model.body_encoder.reset_state()
    print(f"\n[z2108] Generation sanity check after {phase_name}...")
    try:
        with torch.no_grad():
...
def main():
    ...
    print("=" * 60)
    print("z2108: Rigorous Test Battery — Causal Masking + Test Fixes")
    ...
    print(f"\n[z2106 DOMAIN-SHIFT] Loading code data for regime 1...")
    try:
        ...
        train_data_code = None
        test_data_code = None
    print(f"\n[z2106 STEERING] Regime 0: wikitext (English), Regime 1: code (Python)")
    ...
    print(f"\n{'='*60}")
    print(f"z2106 Qwen3-8B Activation Steering LM: {n_pass}/{n_total_tests} PASS")
    print(f"{'='*60}")

    # Save results
    out_path = '/home/ikaros/Documents/claude_hive/AMD_gfx1151_energy/results/z2108_rigorous_lm.json'
    final = {
        'experiment': 'z2108_rigorous_lm',
        'description': 'z2108: Causal masking fix + T6 Cohen d + T14 RAPL max_range + T23/T34 zlib + T19/T22 skip_isa fix',
        'key_changes': {
            ...
            'thermal_softmax': 'Custom attention at steering layers for thermal_scale >= 1.0',
