#!/usr/bin/env python3
"""
z2110: True Embodiment LM — Remove IA³ Hypernetwork, Decouple Domain/Hardware, FiLM Conditioning
=================================================================================================
Built on z2109. Key changes:

  z2110 fixes (external analysis of z2109 regression: 24/41 → target >30/41):
  1. REMOVED IA³ hypernetwork (~30M params → 0). Was v_proj(128→18*1024) + ffn_proj(128→18*12288)
     = 30.9M params that diluted steering and killed T7 kill-shot (9.48x → 1.065x).
  2. Added True IA³: per-layer learned rescale vectors (~239K) + small FiLM conditioner (~4.8K).
     Classic IA³ init-to-1 vectors, with tiny workspace→scalar modulation per layer.
  3. Decoupled domain gate (text-predicted) from hardware gate (telemetry-driven).
     domain_gate = f(text embeddings), body_scale = f(hardware telemetry).
     LoRA blends via domain_gate, amplitude via body_scale. Kill-shot tests HW, not domain.
  4. FiLM conditioning at layers 15, 20, 25 — ISA-coupled via delta_vec + intrinsic_vec.
     cond_vec = cat(workspace_emb, delta_vec, intrinsic_vec) → gamma/beta per layer.
     Makes ISA kernel constitutive: different MODE → different normalization → different logits.
  5. T4 ablation uses skip_substrate=True (proven backbone-equivalent by SANITY test).
  6. T21 zombie twin: hardware-derived analog entropy from SMN thermal ADC + XTAL jitter.
     Not software timer — genuine silicon entropy from ADC quantization noise.
  7. T40 thermal: wider clamp [0.70, 1.80], thermal_alpha init=0.35, FFN inverse modulation.
  8. Telemetry tokens: 4 sensor-summary tokens appended at END of text sequence + interleaved
     every 32 tokens. Model must PREDICT hardware state → sensors instrumentally necessary.
  9. T7 kill-shot: tests hardware modulation (body_scale/IA³/FiLM), not domain switch.
     Same domain_gate for both, toggle DVFS → different body_scale/FiLM.
  10. AMD SMI direct struct read via ctypes (libamd_smi.so) with sysfs fallback.
      Reduces telemetry latency from ~5-10ms to ~50-100μs.
  11. CONTRASTIVE_KILLSHOT_LAMBDA restored to 0.3 (was 0.5).

  Prior fixes preserved:
  - CRITICAL: Causal mask for ALL layers (z2109)
  - PPL token-weighting (z2107)
  - T6 isfinite guard (z2107)
  - T10 single-variable isolation (z2107)
  - T14 RAPL wraparound (z2109)
  - All z2106 NormBounded LoRA + Norm-Preserving Steering architecture
...
class NormBoundedLoRALinear(nn.Module):
    ...
    def forward(self, x, regime_gate=None, body_scale=None, domain_gate=None):
        ...
        # 3. z2110: Blend based on DOMAIN gate (text-predicted), not hardware gate
        # Direction from domain_gate (text-driven), magnitude from body_scale (hardware-driven)
...
class FiLMLayer(nn.Module):
    ...
    def forward(self, h_normed, cond_vec, body_scale=None, hw_entropy=None):
        """
        Args:
            h_normed: [B, T, H] — hidden states after RMSNorm
            cond_vec: [B, cond_dim] = cat(workspace_emb, delta_vec, intrinsic_vec)
            body_scale: [B] — master volume knob, gates FiLM perturbation
            hw_entropy: optional float in [0, 1) — hardware analog entropy for T21
        Returns:
            modulated: [B, T, H]
        """
        ...
        # Safely extract delta from identity init (gamma bias=1.0)
        g_delta = torch.clamp(g_raw - 1.0, -0.05, 0.05)
        b_delta = torch.clamp(b_raw, -0.1, 0.1)
...
class EmbodiedQwen2(nn.Module):
    ...
    def __init__(self, backbone_model, body_encoder, steering_layers=STEERING_LAYERS):
        ...
        # z2110: True IA³ — per-layer rescale vectors + tiny conditioning (~244K params)
        ...
        self.ia3 = TrueIA3Conditioning(
            steering_layers=steering_layers,
            v_dim=n_kv_heads * head_dim_ia3,  # 1024
            ffn_dim=ffn_dim,
        ...
        # z2110: Domain gate — predicted from TEXT, decoupled from hardware
        # Supervised with domain labels during training (r0=0, r1=1)
        self.domain_gate_proj = nn.Sequential(
    ...
    def forward(self, input_ids, sensor_dict, kargs, labels=None,
        ...
        # z2110: True IA³ conditioning — per-layer scalar modulation (not 30M hypernetwork)
        workspace_emb = body_out.get('content_flat', None)
        ia3_v_scales, ia3_ff_scales = None, None
        if workspace_emb is not None and not skip_substrate:
            ia3_v_scales, ia3_ff_scales = self.ia3(workspace_emb.detach())  # [B, n_layers] each; detach: no backward through body_encoder

        # z2110: FiLM conditioning vector = cat(workspace_emb, delta_vec, intrinsic_vec)
        # DETACH from body_encoder graph: FiLM learns its own gamma/beta projections,
        # but backward should NOT flow through body_encoder → ISA kernel path.
        # This prevents SDMA ring timeout (GPU hang) during backward on gfx1151.
        if workspace_emb is not None and not skip_substrate:
            film_cond_vec = torch.cat([workspace_emb.detach(), delta_vec.detach(), intrinsic_vec.detach()], dim=-1)  # [B, 145]
        else:
            film_cond_vec = None
        ...
        # z2110: Domain gate predicted from TEXT (decoupled from hardware)
        # domain_gate = text-driven LoRA direction, regime_gate = backward-compat alias
        if regime_gate_override is not None:
            domain_gate = regime_gate_override
        elif skip_substrate:
            domain_gate = freq_gate  # fallback when substrate skipped
        else:
            # Domain gate from text prefix embedding
            h_prefix = self.backbone.model.embed_tokens(input_ids[:, :8]).mean(dim=1)  # [B, H]
            domain_gate_logit = self.domain_gate_proj(h_prefix.detach().float()).squeeze(-1)  # [B]
            domain_gate = torch.sigmoid(domain_gate_logit)  # [B]
        regime_gate = domain_gate  # backward compat alias for agreement/steering/results
        ...
        if skip_substrate:
            ...
            thermal_scale = torch.exp(alpha * delta10).clamp(0.70, 1.80)  # z2110: wider [0.70, 1.80]
        ...
        for i, layer in enumerate(self.backbone.model.layers):
            ...
            if i in self.steering_layer_set:
                ...
                # z2110: _dg = domain gate (text-driven direction), _bs = body scale (HW amplitude)
                ...
                # z2110: True IA³ gamma_v — per-layer rescale vector * (1 + scalar modulation)
                if ia3_v_scales is not None and i in self.ia3.layer_indices:
                    layer_list_idx = self.ia3.layer_indices.index(i)
                    gamma_v_base = self.ia3.l_v[layer_list_idx].to(v.dtype)  # [v_dim]
                    v_mod = ia3_v_scales[:, layer_list_idx].unsqueeze(-1).to(v.dtype)  # [B, 1]
                    if _bs is not None:
                        v_mod = v_mod * _bs.view(-1, 1).to(v.dtype)
                    v = v * (gamma_v_base * (1 + v_mod)).unsqueeze(1)  # [B, T, v_dim]
                ...
                # z2110: True IA³ gamma_ffn — per-layer rescale vector * (1 + scalar modulation)
                if ia3_ff_scales is not None and i in self.ia3.layer_indices:
                    layer_list_idx = self.ia3.layer_indices.index(i)
                    gamma_ff_base = self.ia3.l_ff[layer_list_idx].to(ffn_activated.dtype)  # [ffn_dim]
                    ff_mod = ia3_ff_scales[:, layer_list_idx].unsqueeze(-1).to(ffn_activated.dtype)  # [B, 1]
                    if _bs is not None:
                        ff_mod = ff_mod * _bs.view(-1, 1).to(ffn_activated.dtype)
                    ffn_activated = ffn_activated * (gamma_ff_base * (1 + ff_mod)).unsqueeze(1)  # [B, T, ffn_dim]
        ...
        return {
            ...
            'domain_gate': domain_gate,  # z2110: text-predicted domain gate
        }
...
def train_lm_epoch(model, train_data, optimizer, epoch, kargs_a, tokenizer,
    ...
    for batch_num in range(min(100, len(indices_wiki))):  # max 100 batches per epoch
        ...
        # z2110: Domain gate supervised loss — train text-predicted gate to match regime labels
        if 'domain_gate' in out:
            dg_target = torch.full((B,), float(current_regime), device=DEVICE)
            dg_loss = F.binary_cross_entropy(
                out['domain_gate'].clamp(1e-6, 1-1e-6), dg_target)
            loss = loss + 1.0 * dg_loss
...
def evaluate_perplexity(model, test_data, regime, kargs, tokenizer,
    ...
    with torch.no_grad():
        for i in range(0, min(len(_eval_data), n_batches * BS), BS):
            ...
            # z2110: Interleave telemetry tokens to match training distribution
            input_ids, telem_mask = interleave_telemetry_tokens(input_ids, sd, device=DEVICE)

            # z2105: PURE next-token for ALL regimes (domain shift, not task shift)
            labels = make_lm_labels(input_ids, offset=1)  # always next-token
            # z2110: Mask telemetry positions — only score TEXT tokens for PPL
            labels[:, telem_mask] = -100  # mask telemetry token positions
...
def generation_sanity_check(model, test_data, kargs, tokenizer, phase_name=""):
    ...
    model.eval()
    model.body_encoder.reset_state()
    print(f"\n[z2110] Generation sanity check after {phase_name}...")
    try:
        with torch.no_grad():
...
def main():
    ...
    print("=" * 60)
    print("z2110: True Embodiment LM — Remove IA³ Hypernetwork, Decouple Domain/HW, FiLM")
    ...
    try:
        ...
        train_data_code = None
        test_data_code = None
    print(f"\n[z2110 STEERING] Regime 0: wikitext (English), Regime 1: code (Python)")
    ...
    print(f"\n{'='*60}")
    print(f"z2110 Qwen3-8B True-Embodied LM: {n_pass}/{n_total_tests} PASS")
    print(f"{'='*60}")

    # Save results
    out_path = '/home/ikaros/Documents/claude_hive/AMD_gfx1151_energy/results/z2110_rigorous_lm.json'
    final = {
        'experiment': 'z2110_rigorous_lm',
        'description': 'z2110: True IA³ + FiLM conditioning + decoupled domain/HW gate + telemetry tokens + AMD SMI',
        'key_changes': {
            ...
            'true_ia3': 'Per-layer rescale vectors (~244K) + small workspace modulation (not 30M hypernetwork)',
            'film_conditioning': 'FiLM at layers 15,20,25 conditioned on cat(workspace, delta, intrinsic) = 145-dim',
            'domain_gate': 'Text-predicted domain gate (supervised), decoupled from hardware body_scale',
    ...
    try:
        ...
        torch.save({
            ...
            'ia3_state': model.ia3.state_dict(),  # z2110: True IA³ (~244K params)
            'film_layers_state': model.film_layers.state_dict(),  # z2110: FiLM layers
            'domain_gate_proj_state': model.domain_gate_proj.state_dict(),  # z2110: text-predicted gate
