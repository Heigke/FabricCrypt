...
class FPGAEthBridge:
    ...
    def set_synapse(self, neuron_id: int, syn_packed: int):
        """Set synapse weights for one neuron. syn_packed is 4×8-bit packed (N-2,N+2,N-1,N+1)."""
        pkt = struct.pack(">BBBI", 0x55, CMD_SET_SYNAPSE, neuron_id & 0x7F, syn_packed & 0xFFFFFFFF)
        self._send(pkt)

    def set_synapse_weights(self, neuron_id: int, w_nm1: float, w_np1: float, w_nm2: float, w_np2: float):
        """Set synapse weights for one neuron. Weights 0.0-1.0 mapped to 0-255."""
        ...
        packed = (b_nm2 << 24) | (b_np2 << 16) | (b_nm1 << 8) | b_np1
        self.set_synapse(neuron_id, packed)
