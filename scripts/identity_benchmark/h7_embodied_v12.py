...
OUT = ROOT / "results/IDENTITY_EMBODIED_V12_2026-06-11"
OUT.mkdir(parents=True, exist_ok=True)
LOG = OUT / f"v12_{HOST}.jsonl"
CKPT = OUT / f"v12_{HOST}.pt"
BEST_CKPT = OUT / f"v12_best_{HOST}.pt"
...
def main():
    ...
    args = ap.parse_args()
    Path(f"/tmp/h7_v12_{HOST}.pid").write_text(str(os.getpid()))   # for the thermal watchdog
    rng = np.random.default_rng(SEED)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[v12 regime-invariant+graded(feat-fix)] host={HOST} device={device} steps={args.steps} M_dep={M_DEP} cap={DEP_CAP}")
    ...
    for step in range(args.steps):
        ...
        if step > 0 and step % EVAL_EVERY == 0:
            ...
            blob = {"step": step, "host": HOST, "version": "v12", "stats": str(args.stats),
