"""Phase 21B — stylometric classifier (vanilla vs chip).

Light compute, runs anywhere (ikaros or daedalus). 5-fold CV, bootstrap CI.
Pre-reg gate: mean_acc >= 0.75.
"""
...
def top_tokens_by_logratio(records_by_label, k=20, vocab_min=5):
    label_counts = {}
    label_totals = {}
    ...
    for a in labels:
        ...
        scored = []
        all_words = set(label_counts[a].keys()) | set(rival_counts.keys())
        for w in all_words:
...
def analyze(jsonl_paths, out_dir, pass_threshold=0.75):
    os.makedirs(out_dir, exist_ok=True)
    records = []
    for p in jsonl_paths:
        records.extend(load_jsonl(p))
    print(f"[stylo] loaded {len(records)} records from {len(jsonl_paths)} files")

    X, y, feat_keys = build_feature_matrix(records)
    print(f"[stylo] X.shape={X.shape}, classes={sorted(set(y.tolist()))}")
    if X.shape[0] < 20 or len(set(y.tolist())) < 2:
        print("[stylo] not enough data")
        return None

    cls = classify_kfold(X, y, k=5)
    passed = cls['mean_acc'] >= pass_threshold
    print(f"[stylo] mean_acc={cls['mean_acc']:.3f} CI=[{cls['ci_lo']:.3f},{cls['ci_hi']:.3f}] "
          f"threshold={pass_threshold} PASS={passed}")

    by_label = {}
    for r in records:
        by_label.setdefault(r['label'], []).append(r)
    top_tok = top_tokens_by_logratio(by_label, k=20, vocab_min=5)
    ...
    result = {
        ...
        'feat_keys': feat_keys,
        'classifier': cls,
        'pre_reg_threshold': pass_threshold,
        'pre_reg_pass': bool(passed),
...
if __name__ == '__main__':
    ...
    args = ap.parse_args()
    analyze(args.jsonl, args.out_dir, pass_threshold=args.threshold)
