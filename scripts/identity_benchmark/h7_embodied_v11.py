...
OUT = ROOT / "results/IDENTITY_EMBODIED_V11_2026-06-10"
OUT.mkdir(parents=True, exist_ok=True)
LOG = OUT / f"v11_{HOST}.jsonl"
CKPT = OUT / f"v11_{HOST}.pt"
BEST_CKPT = OUT / f"v11_best_{HOST}.pt"
...
GRAD_BETA = 0.6          # nats: how far the live-signal feature steers output entropy
LAMBDA_GRAD = 0.5        # weight of the graded behavioral-dependence objective
...
def main():
    ...
    ap.add_argument("--stats", default=str(STATS),
                    help="per-die normalization stats (median/MAD). Must be THIS die's own "
                         "stats or the encoder saturates and can't learn its identity.")
    args = ap.parse_args()
    rng = np.random.default_rng(SEED)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[v11 regime-invariant+graded] host={HOST} device={device} steps={args.steps} M_dep={M_DEP} cap={DEP_CAP}")
    ...
    for step in range(args.steps):
        ...
        feat = float(np.tanh(np.std(w_real[:, GRAD_CHANNEL]) / (np.std(w_real) + 1e-6) - 1.0))
        p_r = F.softmax(lr, -1)
        ent = -(p_r * torch.log(p_r + 1e-9)).sum(-1).mean()
        with torch.no_grad():
            p_b = F.softmax(lb_full, -1)
            ent_b = -(p_b * torch.log(p_b + 1e-9)).sum(-1).mean()
        target_ent = ent_b + GRAD_BETA * feat
        ...
        if step > 0 and step % EVAL_EVERY == 0:
            ...
            blob = {"step": step, "host": HOST, "version": "v11", "stats": str(args.stats),
