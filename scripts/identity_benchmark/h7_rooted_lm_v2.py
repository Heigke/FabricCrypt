...
def train(steps=2000, ctx=128, batch=16, lr=3e-4):
    ...
    for step in range(steps):
        ...
        loss_real_ce = (ce * is_real).sum() / is_real.sum().clamp(min=1)

        # spoof penalty: punish performing well on spoof
        # objective = -CE on spoof  → equivalent to maximizing CE on spoof
        loss_spoof_neg = -(ce * is_spoof).sum() / is_spoof.sum().clamp(min=1)

        # phase penalty: punish performing well on phase-shifted substrate
        loss_phase_neg = -(ce * is_phase).sum() / is_phase.sum().clamp(min=1)
        ...
        # Total loss — full weights on spoof/phase penalties
        loss = (
            loss_real_ce
            + 0.5 * loss_spoof_neg
            + 0.5 * loss_phase_neg
