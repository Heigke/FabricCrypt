...
class MMCFGProbe:
    ...
    def smn_read_timed(self, addr):
        """C20 — return (value, read_latency_ns).

        Per O100 oracle synthesis (4/4 agreement): the *time* to complete the
        SMN read is itself a die-bound signal — fabric arbitration, micro-TLB,
        PHY effects produce stable p99 tails per chassis.
        """
    ...
    def snapshot(self):
        t = time.time_ns()
        # Each SMN read also returns its own latency (C20). Record latencies
        # for a fixed canonical-3 set per snapshot (XTAL, base thermal, energy[0])
        # to keep dimensionality bounded.
        cores = [self.smn_read(SMN_THERMAL_CORE_BASE + i * 4) for i in range(16)]
        base_v, base_lat = self.smn_read_timed(SMN_BASE_THERMAL)
        energy = [self.smn_read(a) for a in SMN_ENERGY]
        _, energy0_lat = self.smn_read_timed(SMN_ENERGY[0])
        fast = self.smn_read(SMN_FAST_COUNTER)
        xtal_v, xtal_lat = self.smn_read_timed(SMN_XTAL_CNTL)
        gfx_vid = self.smn_read(SMN_GFX_VID)
        soc_vid = self.smn_read(SMN_SOC_VID)
        return (t, cores, base_v, energy, fast, xtal_v, gfx_vid, soc_vid,
                base_lat, energy0_lat, xtal_lat)
...
def main():
    ...
    smn_arr = np.array([
        (s[0], *(s[1] or [0]*16), s[2] or 0, *(s[3] or [0,0,0]),
         ...
         s[8] if len(s) > 8 else 0, s[9] if len(s) > 9 else 0, s[10] if len(s) > 10 else 0)
        for s in smn.samples
    ], dtype=np.int64) if smn.samples else np.zeros((0, 1+16+1+3+4+3), dtype=np.int64)
