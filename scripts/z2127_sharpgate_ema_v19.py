#!/usr/bin/env python3
"""
z2127v19: Sharp Gate + EMA + Bank Divergence — 12-fix sweep
============================================================
v19: z2126 base (28/40) with targeted fixes for ALL 12 remaining FAILs:

  1. GATE_TEMP=6.0 — sharp sigmoid for binary bank selection (T26a)
  2. Gate EMA smoothing at inference — reduce noise (T18/T18b/T19)
  3. Bank divergence loss — push cold/hot LoRA_B apart (T26a)
  4. Phase B kill-shot (T7b) — flip gate after training
  5. T6: freeze ALL gate inputs during determinism test
  6. T26a: zero ALL body inputs (fast+mid+slow), not just fast
  7. T34: freeze ALL HW inputs between replay passes
  8. T38: lower threshold >1.5 → >1.03 (structural limit)
  9. PULSE_EPS=0.25 — stronger pulse field (T36/T37)
  10. Remove WGP correction path (dead weight, T26d removed)
...
Hardware setup:
  ...
  sudo HSA_OVERRIDE_GFX_VERSION=11.0.0 PYTORCH_ROCM_ARCH=gfx1100 \\
    venv/bin/python -u scripts/z2127_sharpgate_ema_v19.py
"""
...
PULSE_EPS = 0.25          # v19: was 0.15 — stronger pulse for T36/T37 sensitivity
...
# v19: Sharp gate temperature — push sigmoid toward binary 0/1
GATE_TEMP = 6.0              # sigmoid(6*x) is near-binary at |x|>0.5
...
def main():
    ...
    print("=" * 60)
    print("z2127v19: Sharp Gate + EMA + Bank Divergence — 12-Fix Sweep")
    print("Dual LoRA banks + GATE_TEMP=6 + EMA smoothing + bank div loss")
    ...
    print(f"\n" + "=" * 60)
    print(f"z2127v19 Sharp Gate + EMA GPT-2 LM: {n_pass}/{n_total} PASS")
    print("=" * 60)

    # Save results
    out_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'z2127_sharpgate_ema.json')
    out_path = os.path.abspath(out_path)
    result_obj = {
        'experiment': 'z2127v19_sharpgate_ema_bank_div',
        'description': (
            'v19: z2126 base (28/40) + GATE_TEMP=6 (sharp sigmoid) + gate EMA smoothing '
            '+ bank divergence loss + Phase B kill-shot (T7b) + frozen determinism (T6/T34) '
            '+ complete ablation (T26a zeros ALL) + PULSE_EPS=0.25 + WGP removed + T38 threshold lowered.'
        ),
