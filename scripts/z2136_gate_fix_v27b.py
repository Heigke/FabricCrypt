#!/usr/bin/env python3
"""
z2136v27b: Gate Fix — Direct gate supervision + temperature + fixed contrastive
================================================================================
v27b: Based on v27 (27/40) — fixes the GATE STUCK AT 0.5 problem:
  1. Direct gate supervision loss: MSE(gate, target) where target=0(cold)/1(hot)
  2. Gate temperature scaling: sigmoid(pre * 5.0) for sharper decisions
  3. Larger gate init: std=0.1 (was 0.02) to break symmetry faster
  4. Fixed contrastive loss: flips gate value directly (not CTX.regime which was wrong)
v27 base: Patch ALL projections + RAPL energy + Longer training

  Changes from v26:
  1. Patch ALL linear projections (c_attn, c_proj, c_fc) with AnalogLinear
     — v26 only patched c_fc (~20% of GEMM compute). Now MODE writes hit ~100%.
  2. Connect RAPL energy as actual slow-timescale body signal
     — v26 had check_rapl() but never used it. Now energy/batch feeds SLOW_DIM.
  3. Train 400 batches (doubled from 200) for better dual-bank divergence
     — Research showed 200 too few for cold/hot banks to meaningfully diverge.
  4. Add thermal-seeded stochastic rounding in HIP kernel
     — LSB of temperature ADC seeds rounding entropy per-tile. Novel coupling.

  v26 scored 25-31/40 (reliable core 27). v27 target: 33-35/40.
...
Hardware setup:
  ...
  sudo HSA_OVERRIDE_GFX_VERSION=11.0.0 PYTORCH_ROCM_ARCH=gfx1100 \\
    venv/bin/python -u scripts/z2136_gate_fix_v27.py
"""
...
SLOW_DIM = 6   # v27: stress_ema,d_stress,smn_adc,pcie_replay,rapl_power,d_rapl_power
...
PULSE_ALPHA = 0.65        # v12: was 0.85 — faster response to current execution state
PULSE_GAIN = 2.0          # tanh gain for sensed signal
PULSE_EPS = 0.30          # v26: optimal from salience sweep. 0.50 breaks T34 (conditional determinism)
...
# v16: Contrastive kill-shot loss (from z2095 which gave T7=1.283 PASS)
LAMBDA_CONTRASTIVE = 0.3     # weight for contrastive margin loss
CONTRASTIVE_MARGIN = 0.3     # margin: correct must beat wrong by this much (log-PPL)
CONTRASTIVE_PROB = 0.25      # probability of contrastive batch in Phase 2+
# v16: Agreement modulation power
AGREEMENT_POWER = 2.0        # body_scale *= (1 - |disagreement|)^AGREEMENT_POWER

# v27b: GATE FIX — direct gate supervision + temperature scaling
LAMBDA_GATE_SUPER = 0.5      # weight for gate supervision loss (push gate→target)
GATE_TEMPERATURE = 5.0       # pre-activation scaling for sharper sigmoid
GATE_INIT_STD = 0.1          # larger init to break symmetry faster
...
RAPL_PATHS = {}
RAPL_MAX_RANGE = {}
RAPL_LAST_UJ = {}    # v27: track last reading for delta computation
RAPL_LAST_TIME = {}  # v27: track last timestamp


def check_rapl():
    global RAPL_AVAILABLE, RAPL_PATHS, RAPL_MAX_RANGE
    base = '/sys/class/powercap'
    # v27: scan ALL intel-rapl domains (works on AMD too — kernel uses intel-rapl naming)
    for domain in ['intel-rapl:0', 'intel-rapl:0:0', 'intel-rapl:0:1', 'intel-rapl:0:2']:
...
def read_rapl_energy_uj():
    """v27: Read current RAPL energy in microjoules. Returns dict {domain: uj}."""
    readings = {}
    for name, path in RAPL_PATHS.items():
        try:
            with open(path, 'r') as f:
                readings[name] = int(f.read().strip())
        except:
            pass
    return readings


def read_rapl_power_w():
    """v27: Read instantaneous power in watts (delta energy / delta time).
    Returns package power in watts, or 0.0 if unavailable."""
    global RAPL_LAST_UJ, RAPL_LAST_TIME
    if not RAPL_AVAILABLE:
        return 0.0
    now = time.monotonic()
    current = read_rapl_energy_uj()
    power = 0.0
    # Use 'package-0' domain (total CPU+iGPU power)
    for domain in ['package-0', 'core']:
        if domain in current and domain in RAPL_LAST_UJ:
            dt = now - RAPL_LAST_TIME.get(domain, now)
            if dt > 0.001:  # at least 1ms gap
                duj = current[domain] - RAPL_LAST_UJ[domain]
                max_range = RAPL_MAX_RANGE.get(domain, 1 << 32)
                if duj < 0:
                    duj += max_range  # handle wraparound
                power = duj * 1e-6 / dt  # microjoules → joules / seconds = watts
                break
    # Update last readings
    for domain, uj in current.items():
        RAPL_LAST_UJ[domain] = uj
        RAPL_LAST_TIME[domain] = now
    return power
...
class MetabolicController:
    ...
    def snapshot_slow(self):
        ...
        # v27: RAPL energy as real body signal — normalized to [0,1] range (0-65W TDP)
        rapl_w = read_rapl_power_w()
        rapl_norm = max(0.0, min(1.0, rapl_w / 65.0))
        d_rapl = rapl_norm - (prev[4] if len(prev) > 4 else 0.0)
        CTX.slow_vec = np.array([
            self.stress_ema,
            self.stress_ema - prev[0] if self._n_updates > 0 else 0.0,
            smn_adc,
            pcie,
            rapl_norm,      # v27: instantaneous package power (normalized)
            d_rapl,         # v27: power delta (rising/falling)
        ], dtype=np.float32)
...
__global__ void pulse_embodiment_v10_kernel(
    ...
    unsigned int thermal_seed   // v27: LSB of temperature ADC — physical entropy source
) {
    ...
    for (int t = 0; t < n_tiles; t++) {
        ...
        // v27: THERMAL STOCHASTIC ROUNDING — physical entropy from junction temp LSBs
        // thermal_seed carries real ADC noise from the APU's thermal sensor
        // XOR with tile index + cycle count creates spatiotemporal entropy field
        unsigned int acc_bits = __float_as_uint(acc);
        unsigned int thermal_entropy = thermal_seed ^ (unsigned int)t ^ (cycles & 0xFFu);
        unsigned int feedback = (acc_bits >> 16) ^ prev_tile_bits ^ thermal_entropy;
...
std::vector<torch::Tensor> analog_gemm_v10(
    ...
    unsigned int thermal_seed  // v27: physical entropy from temperature ADC
) {
...
class AnalogLinear(nn.Module):
    """v11: Pulse buffers + physical signal kernel.
    - Handles both nn.Linear AND HuggingFace Conv1D (GPT-2)
    - Conv1D weight is (nx, nf) = (in, out) — transposed vs nn.Linear (out, in)
    ...
    def __init__(self, original_linear, layer_idx, mode_override=None):
        ...
        # v11: detect Conv1D (GPT-2) vs nn.Linear (Qwen etc)
        self._is_conv1d = hasattr(original_linear, 'nf')
        if self._is_conv1d:
            self._in_f = original_linear.nx
            self._out_f = original_linear.nf
        else:
            self._in_f = self.weight.shape[1]
            self._out_f = self.weight.shape[0]
    ...
    def forward(self, x):
        ...
        if hip_mod is not None and PULSE_BUFFERS is not None:
            ...
            # v11: Conv1D weight is (nx,nf)=(in,out), kernel needs (out,in)
            w_raw = self.weight.t() if self._is_conv1d else self.weight
            w_bf16 = w_raw.to(torch.bfloat16).contiguous()
            ...
            # v12: Scale PULSE_EPS by weight norm so pulse is proportionally
            # audible regardless of layer weight magnitude.
            # Reference norm ~0.5 (typical small layer). Larger weights get larger eps.
            w_norm = float(w_bf16.float().norm()) / max(w_bf16.numel() ** 0.5, 1.0)
            eff_pulse_eps = PULSE_EPS * max(w_norm / 0.5, 0.5)

            # v27: thermal seed from APU junction temperature LSBs
            # Read raw temp and use low 8 bits as entropy — real physical noise source
            thermal_seed = 0
            if SMN_AVAILABLE:
                try:
                    raw_adc = read_smn(0x00059800)
                    thermal_seed = raw_adc & 0xFF  # LSBs of thermal ADC = physical noise
                except:
                    thermal_seed = int(time.monotonic() * 1e6) & 0xFF

            results = hip_mod.analog_gemm_v10(
                x_bf16, w_bf16, round_mode, continuous_stress,
                pulse_in, pulse_out,
                PULSE_ALPHA, PULSE_GAIN, eff_pulse_eps,
            ...
            # v11: Conv1D weight is (nx,nf), need transpose for F.linear
            w_lin = self.weight.t() if self._is_conv1d else self.weight
            out = F.linear(x_2d, w_lin)

        if self.bias is not None:
            out = out + self.bias
        return out.reshape(*orig_shape[:-1], self._out_f)

# v27: Patch targets — which projection names to wrap with AnalogLinear
# c_attn (QKV fused), c_proj (output proj), c_fc (MLP up-proj)
# This gives MODE writes on ~100% of GEMM ops (vs ~20% with c_fc only)
ANALOG_PROJ_NAMES = ['c_attn', 'c_proj', 'c_fc']


def patch_with_analog(model, layers, mode_override=None, proj_names=None):
    """v27: Patch multiple projection types with AnalogLinear.
    Returns dict: {(layer_idx, proj_name): original_module}"""
    if proj_names is None:
        proj_names = ANALOG_PROJ_NAMES
    originals = {}
    count = 0
    for layer_idx in layers:
        ...
        for pname in proj_names:
            ...
            if pname == 'c_fc':
                ...
                orig = block.mlp.c_proj
            elif pname == 'c_attn':
                orig = block.attn.c_attn
            else:
                continue
            # Use unique layer_id for pulse buffer indexing:
            # c_attn=layer*3+0, c_proj=layer*3+1, c_fc=layer*3+2
            proj_offset = {'c_attn': 0, 'c_proj': 1, 'c_fc': 2}[pname]
            analog_layer_id = layer_idx * 3 + proj_offset
            analog = AnalogLinear(orig, analog_layer_id, mode_override=mode_override)
            if pname == 'c_fc':
                block.mlp.c_fc = analog
            elif pname == 'c_proj':
                block.mlp.c_proj = analog
            elif pname == 'c_attn':
                block.attn.c_attn = analog
            originals[(layer_idx, pname)] = orig
            count += 1
            if layer_idx in [0, 3, 6, 9, 11]:
                in_f = orig.nx if hasattr(orig, 'nx') else orig.weight.shape[1]
                out_f = orig.nf if hasattr(orig, 'nf') else orig.weight.shape[0]
                print(f"  [AnalogLinear] Patched layer {layer_idx} {pname} ({in_f}->{out_f})")
    print(f"  [AnalogLinear] Total: {count} projections patched across {len(layers)} layers")
    return originals


# v27: backward compat alias
def patch_c_fc_with_analog(model, layers, mode_override=None):
    return patch_with_analog(model, layers, mode_override=mode_override, proj_names=['c_fc'])


def restore_analog(model, originals):
    """v27: Restore all patched projections."""
    for (layer_idx, pname), orig in originals.items():
        block = model.transformer.h[layer_idx]
        if pname == 'c_fc':
            block.mlp.c_fc = orig
        elif pname == 'c_proj':
            block.mlp.c_proj = orig
        elif pname == 'c_attn':
            block.attn.c_attn = orig


# v27: backward compat alias
def restore_c_fc(model, originals):
    # Handle both old {layer_idx: orig} and new {(layer_idx, pname): orig} formats
    new_originals = {}
    for k, v in originals.items():
        if isinstance(k, tuple):
            new_originals[k] = v
        else:
            new_originals[(k, 'c_fc')] = v
    restore_analog(model, new_originals)


def set_analog_mode_override(model, layers, mode_override):
    for layer_idx in layers:
        block = model.transformer.h[layer_idx]
        for pname in ANALOG_PROJ_NAMES:
            if pname == 'c_fc':
                gate = block.mlp.c_fc
            elif pname == 'c_proj':
                gate = block.mlp.c_proj
            elif pname == 'c_attn':
                gate = block.attn.c_attn
            else:
                continue
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# v18: DUAL-BANK BODY-GATED LORA — cold bank + hot bank, gate interpolates
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
class BodyGatedLoRA(nn.Module):
    """v18: Dual LoRA banks — cold_B and hot_B — gate interpolates between them.
    When gate=0 (cold regime): uses cold_B weights.
    When gate=1 (hot regime): uses hot_B weights.
    This creates regime-specific output that REQUIRES correct hardware state."""
    ...
    def __init__(self, analog_linear, layer_idx, rank=8, alpha=16):
        ...
        # Shared LoRA_A (down-projection) — both banks share the bottleneck
        self.lora_A = nn.Parameter(torch.randn(rank, in_f, dtype=dtype) * 0.01)

        # v18: DUAL LoRA_B (up-projection) — cold and hot banks
        self.lora_B_cold = nn.Parameter(torch.randn(out_f, rank, dtype=dtype) * 0.001)
        self.lora_B_hot = nn.Parameter(torch.randn(out_f, rank, dtype=dtype) * 0.001)

        # v27b: ADDITIVE gate — larger init + temperature for sharper differentiation
        ...
        nn.init.normal_(self.gate_fast.weight, std=GATE_INIT_STD)
        nn.init.normal_(self.gate_mid.weight, std=GATE_INIT_STD)
        nn.init.normal_(self.gate_slow.weight, std=GATE_INIT_STD)
        ...
        # v23: Test mechanics flags
        self._force_wrong_gate = False   # flip gate for T7b kill-shot
        self._frozen_gate_inputs = None  # tuple (fast, mid, slow) for determinism tests
    ...
    def forward(self, x):
        ...
        # === Main LoRA path: shared A, dual B banks ===
        lora_mid = F.linear(x_cast, self.lora_A)  # [..., rank]

        dev = self.gate_fast.weight.device
        # v23: Allow frozen gate inputs for determinism tests
        if self._frozen_gate_inputs is not None:
            fast_input, mid_input, slow_input = self._frozen_gate_inputs
        else:
        ...
        gate = torch.sigmoid(gate_pre * GATE_TEMPERATURE)  # v27b: temperature sharpens sigmoid

        # v18: Dual-bank interpolation — gate mixes cold and hot LoRA_B outputs
        cold_out = F.linear(lora_mid, self.lora_B_cold) * self.scaling
        hot_out = F.linear(lora_mid, self.lora_B_hot) * self.scaling
        # gate.mean() is scalar: 0=cold, 1=hot
        gate_scalar = gate.mean()
        self.last_gate_scalar = gate_scalar  # v27b: expose for supervision loss
        # v23: Force wrong gate for T7b kill-shot test
        if self._force_wrong_gate:
            gate_scalar = 1.0 - gate_scalar
        main_lora_out = (1.0 - gate_scalar) * cold_out + gate_scalar * hot_out
...
def patch_c_fc_with_lora(model, layers, rank=8, alpha=16):
    originals = {}
    total_params = 0
    for layer_idx in layers:
        block = model.transformer.h[layer_idx]
        analog = block.mlp.c_fc
        if not isinstance(analog, AnalogLinear):
            print(f"  [WARN] Layer {layer_idx} c_fc is not AnalogLinear, skipping LoRA")
            continue
        ...
        total_params += n_p
        if layer_idx in [4, 7, 10, 11]:
            print(f"  [BodyGatedLoRA] Patched layer {layer_idx} c_fc (rank={rank}, {n_p} params)")
    print(f"  [BodyGatedLoRA] Total trainable: {total_params} across {len(originals)} layers")
    return originals, total_params


def restore_c_fc_lora(model, originals):
    for layer_idx, analog in originals.items():
        model.transformer.h[layer_idx].mlp.c_fc = analog
...
def run_phase_a(model, test_data, test_data_code, baseline_ppl):
    ...
    print("  v27: ALL projections + thermal stochastic rounding + RAPL energy")
    print("=" * 60)

    results = {'baseline_ppl': baseline_ppl}
    # v27: patch ALL projections (c_attn, c_proj, c_fc) for full MODE write coverage
    originals = patch_with_analog(model, ANALOG_LAYERS, mode_override=None)
    ...
    if DVFS_AVAILABLE:
        ...
        for dvfs_level, dvfs_name in [(0, 'low'), (2, 'high')]:
            ...
            # v25: Enable probe_mode for one forward pass to populate layer_proprio
            CTX.probe_mode = True
            CTX.clear_proprio()
            METAB.update_context()
            with torch.no_grad():
    ...
    for idx in ANALOG_LAYERS:
        ...
        gate = model.transformer.h[idx].mlp.c_fc
        if isinstance(gate, AnalogLinear):
...
def run_phase_b(model, train_data, test_data, test_data_code, baseline_ppl, phase_a_results):
    print("\n" + "=" * 60)
    print("PHASE B: v27b Gate Fix 3-Phase Curriculum")
    print("  Phase1: Calibration 0-80 | Phase2: Reflex 80-240 | Phase3: Integration 240-400")
    print("  v27b: GATE FIX + ALL projections + RAPL energy + thermal rounding")
    print("=" * 60)

    results = {}
    # v27: patch ALL projections with AnalogLinear
    analog_originals = patch_with_analog(model, ANALOG_LAYERS, mode_override=None)
    lora_originals, n_lora_params = patch_c_fc_with_lora(model, LORA_LAYERS,
    ...
    N_TRAIN_BATCHES = 400  # v27: doubled from 200 for better dual-bank divergence
    LR = 1e-4
    ...
    for batch_idx in range(N_TRAIN_BATCHES):
        # v27: 3-phase curriculum scaled to 400 batches (was 200)
        if batch_idx < 80:
            ...
            LAMBDA_INTERO = 0.1
            LAMBDA_STAB = 0.005
            target_dvfs = dvfs_schedule[batch_idx % len(dvfs_schedule)]
        ...
        for chunk_i in range(n_chunks - 1):
            METAB.update_context()
            CTX.probe_mode = True   # v12: LET THE SILICON THROUGH — kernel telemetry flows to body gates
            CTX.clear_proprio()
            ...
            # v27b: DIRECT GATE SUPERVISION — push gate toward hardware state
            # This is the KEY fix: gate must learn to track DVFS state
            gate_super_loss = torch.tensor(0.0, device=DEVICE)
            if target_dvfs in ('low', 'high'):
                # Target: gate→0 when cold, gate→1 when hot
                gate_target = 0.0 if target_dvfs == 'low' else 1.0
                gate_scalars = []
                for layer_idx in LORA_LAYERS:
                    g = model.transformer.h[layer_idx].mlp.c_fc
                    if isinstance(g, BodyGatedLoRA) and hasattr(g, 'last_gate_scalar'):
                        gate_scalars.append(g.last_gate_scalar)
                if gate_scalars:
                    mean_gate = torch.stack(gate_scalars).mean()
                    gate_super_loss = F.mse_loss(mean_gate,
                        torch.tensor(gate_target, device=DEVICE))

            # v27b: FIXED contrastive loss — flip gate value directly, not CTX.regime
            contrastive_loss = torch.tensor(0.0, device=DEVICE)
            if (phase_name != 'calibration' and target_dvfs in ('low', 'high')
                    and random.random() < CONTRASTIVE_PROB):
                # Force wrong gate on all BodyGatedLoRA layers
                for layer_idx in LORA_LAYERS:
                    g = model.transformer.h[layer_idx].mlp.c_fc
                    if isinstance(g, BodyGatedLoRA):
                        g._force_wrong_gate = True
                with torch.no_grad():
                    wrong_out = model(chunk, output_hidden_states=False)
                    wrong_logits = wrong_out.logits if hasattr(wrong_out, 'logits') else wrong_out[0]
                    wrong_shift = wrong_logits[:, MICRO_CHUNK-1:-1, :].contiguous()
                    wrong_ce = F.cross_entropy(wrong_shift.view(-1, wrong_shift.size(-1)),
                                                shift_labels.view(-1))
                for layer_idx in LORA_LAYERS:
                    g = model.transformer.h[layer_idx].mlp.c_fc
                    if isinstance(g, BodyGatedLoRA):
                        g._force_wrong_gate = False
                # Margin loss: correct_ce should be < wrong_ce - margin
                contrastive_loss = torch.clamp(
                    ce_loss - (wrong_ce - CONTRASTIVE_MARGIN), min=0.0)

            # v27b: total loss includes gate supervision + contrastive + pulse prediction
            total_loss = (ce_loss
                          ...
                          + LAMBDA_GATE_SUPER * gate_super_loss)
        ...
        if (batch_idx + 1) % 25 == 0:
            ...
            # v27b: log gate scalar to monitor gate training
            gate_vals = []
            for li in LORA_LAYERS:
                g = model.transformer.h[li].mlp.c_fc
                if isinstance(g, BodyGatedLoRA) and hasattr(g, 'last_gate_scalar'):
                    gate_vals.append(float(g.last_gate_scalar.detach()))
    ...
    # --- v25: Dedicated SomaProbe Training Phase ---
    # soma_probe was never called during training → weights random/decayed
    # Train it explicitly with DVFS cycling + supervised regime labels
    # KEY: Use DVFS state directly as label (not adaptive regime classifier)
    # low→0(COLD), auto→1(NOMINAL), high→2(HOT) — matches what T22 eval encounters
    print("\n  === SomaProbe Training Phase (v25) ===")
    # Re-initialize probe with proprio-biased weights
    # The shared layer is (768+12) → 128. Boost the last 12 columns (proprio) x10
    with torch.no_grad():
        soma_probe.shared.weight.data[:, -FAST_DIM:] *= 10.0
    probe_optimizer = torch.optim.AdamW(soma_probe.parameters(), lr=0.01, weight_decay=0.001)
    # Interleaved DVFS schedule: cycle low→auto→high every 3 batches
    # This prevents catastrophic forgetting (blocked schedule forgets earlier classes)
    probe_batches = 150  # v26: was 90, more batches for convergence
    probe_dvfs_schedule = ['low', 'auto', 'high'] * (probe_batches // 3 + 1)
    dvfs_to_regime = {'low': REGIME_COLD, 'auto': REGIME_NOMINAL, 'high': REGIME_HOT}
    probe_current_dvfs = 'auto'
    probe_correct = 0
    probe_total = 0
    model.eval()  # freeze model, only train probe
    for pb_idx in range(probe_batches):
        target_dv = probe_dvfs_schedule[pb_idx]
        if target_dv != probe_current_dvfs:
            set_dvfs(target_dv)
            time.sleep(DVFS_SETTLE_S)
            probe_current_dvfs = target_dv
        METAB.update_context()
        CTX.clear_proprio()
        with torch.no_grad():
            batch = torch.stack(train_data[(pb_idx * BS) % max(len(train_data) - BS, 1):
                                           (pb_idx * BS) % max(len(train_data) - BS, 1) + BS]).to(DEVICE)
            out = model(batch, output_hidden_states=True)
            hidden = out.hidden_states[-1].detach()
        proprio_stats = aggregate_proprio().to(DEVICE)
        regime_logits_p, stress_pred_p = soma_probe(hidden, proprio_stats)
        dvfs_regime = dvfs_to_regime[target_dv]
        target_regime = torch.full((regime_logits_p.size(0),), dvfs_regime,
                                    device=DEVICE, dtype=torch.long)
        probe_loss = F.cross_entropy(regime_logits_p, target_regime)
        probe_optimizer.zero_grad()
        probe_loss.backward()
        probe_optimizer.step()
        pred = int(regime_logits_p[0].argmax().item())
        probe_correct += int(pred == dvfs_regime)
        probe_total += 1
        if (pb_idx + 1) % 30 == 0:
            print(f"    Probe batch {pb_idx+1}/{probe_batches}: "
                  f"loss={probe_loss.item():.4f}, acc={probe_correct/probe_total:.4f}, "
                  f"dvfs={probe_current_dvfs}, label={dvfs_regime}")
    model.train()
    set_dvfs('auto')
    time.sleep(DVFS_SETTLE_S)
    probe_train_acc = probe_correct / max(probe_total, 1)
    results['probe_train_acc'] = probe_train_acc
    print(f"  SomaProbe training: {probe_correct}/{probe_total} = {probe_train_acc:.4f}")
    ...
    # Cleanup
    set_analog_mode_override(model, ANALOG_LAYERS, None)
    restore_c_fc_lora(model, lora_originals)
    restore_c_fc(model, analog_originals)
    return results
...
def _run_phase_b_tests(model, soma_head, soma_probe, test_data, test_data_code, baseline_ppl,
    ...
    # v23: T7b — Phase B Kill-Shot (flip dual-bank gate after training)
    print("\n[T7b] Phase B Kill-Shot (flip gate → wrong bank)...")
    set_analog_mode_override(model, ANALOG_LAYERS, None)
    METAB.update_context()
    correct_ppl = eval_ppl(model, test_data)
    for layer_idx in LORA_LAYERS:
        g = model.transformer.h[layer_idx].mlp.c_fc
        if isinstance(g, BodyGatedLoRA):
            g._force_wrong_gate = True
    wrong_ppl = eval_ppl(model, test_data)
    for layer_idx in LORA_LAYERS:
        g = model.transformer.h[layer_idx].mlp.c_fc
        if isinstance(g, BodyGatedLoRA):
            g._force_wrong_gate = False
    kill_ratio_b = wrong_ppl / max(correct_ppl, 0.01)
    results['kill_ratio_b'] = kill_ratio_b
    results['correct_ppl_b'] = correct_ppl
    results['wrong_ppl_b'] = wrong_ppl
    print(f"  Correct gate PPL: {correct_ppl:.4f}, Wrong gate PPL: {wrong_ppl:.4f}")
    print(f"  Phase B kill ratio: {kill_ratio_b:.6f}")
    ...
    # v26: 5 reps per DVFS (was 3) + probe_mode for proprio population
    ...
    for dvfs_name in ['low', 'high', 'auto']:
        if DVFS_AVAILABLE:
            set_dvfs(dvfs_name)
            time.sleep(DVFS_SETTLE_S)
        for _ in range(5):  # v26: 5 reps (was 3) for better correlation stability
            METAB.update_context()
            CTX.clear_proprio()
            CTX.probe_mode = True  # v26: enable proprio population
            with torch.no_grad():
                batch = torch.stack(test_data[:BS]).to(DEVICE)
                out = model(batch, output_hidden_states=True)
    ...
    # v25: Use DVFS-based labels (matches probe training) not adaptive regime classifier
    print("\n[T19] Held-Out Regime Classification (SomaProbeHead)...")
    dvfs_to_regime_eval = {'low': REGIME_COLD, 'auto': REGIME_NOMINAL, 'high': REGIME_HOT}
    ...
    for dvfs_name in ['low', 'high', 'auto']:
        ...
        for _ in range(5):
            METAB.update_context()
            CTX.clear_proprio()
            CTX.probe_mode = True  # v26: enable proprio population for probe eval
            with torch.no_grad():
                batch = torch.stack(test_data[:BS]).to(DEVICE)
                out = model(batch, output_hidden_states=True)
                hidden = out.hidden_states[-1]
                ...
                pred = int(regime_logits_p[0].argmax().item())
                true_r = dvfs_to_regime_eval[dvfs_name]
...
def _test_step_response(model):
    ...
    for fwd_i in range(20):
        # v26: sleep between fwd passes so HW telemetry propagates DVFS change
        time.sleep(0.3)
        METAB.update_context()
        CTX.clear_proprio()
        with torch.no_grad():
    ...
    if len(gate_trajectory) >= 5:
        ...
        g_range = g_max - g_min
        if g_range > 0.0005:  # v26: lowered from 0.001
...
def _test_causal_ablation_v6(model, test_data):
    ...
    CTX.mid_vec = np.random.randn(MID_DIM).astype(np.float32)
    CTX.slow_vec = np.random.randn(SLOW_DIM).astype(np.float32)
    ...
    print(f"  Zero ALL: {zero_ppl:.4f} (ratio={zero_ratio:.4f})")
    print(f"  Negate ALL: {negate_ppl:.4f} (ratio={negate_ratio:.4f})")
    print(f"  Noise ALL: {noise_ppl:.4f} (ratio={noise_ratio:.4f})")
...
def _test_pulse_stop(model, test_data):
    ...
    v26: Freeze pulse writes in both conditions to isolate just the buffer content."""
    if PULSE_BUFFERS is None:
        return {'ratio': 1.0}
    set_analog_mode_override(model, ANALOG_LAYERS, None)
    METAB.update_context()

    # v26: snapshot current pulse, freeze both evals to isolate buffer content
    # v26g: n_batches 25→50 for lower noise on marginal signals
    PULSE_BUFFERS.snapshot()
    with pulse_mode(freeze=True):
        normal_ppl = eval_ppl(model, test_data, n_batches=25)

    PULSE_BUFFERS.zero()
    with pulse_mode(freeze=True):
        zeroed_ppl = eval_ppl(model, test_data, n_batches=25)
...
def _test_pulse_shuffle(model, test_data):
    ...
    # v26: freeze both conditions to isolate buffer content
    # v26g: n_batches 25→50 for lower noise
    PULSE_BUFFERS.snapshot()
    with pulse_mode(freeze=True):
        normal_ppl = eval_ppl(model, test_data, n_batches=25)
    ...
    with pulse_mode(freeze=True):
        shuffled_ppl = eval_ppl(model, test_data, n_batches=25)
    PULSE_BUFFERS.restore_snapshot()
...
def _test_pulse_stale(model, test_data):
    ...
    with torch.no_grad():
        batch = torch.stack(test_data[:BS]).to(DEVICE)
        _ = model(batch)
    stale_snap = PULSE_BUFFERS.buffers.clone()  # save "stale" state

    with torch.no_grad():
        batch2 = torch.stack(test_data[BS:2*BS]).to(DEVICE)
        _ = model(batch2)

    # v26: freeze both conditions; "normal" = fresh pulse, "stale" = old pulse
    # v26g: n_batches 25→50 for lower noise
    PULSE_BUFFERS.snapshot()
    with pulse_mode(freeze=True):
        normal_ppl = eval_ppl(model, test_data, n_batches=25)

    PULSE_BUFFERS.buffers.copy_(stale_snap)
    with pulse_mode(freeze=True):
        stale_ppl = eval_ppl(model, test_data, n_batches=25)
    PULSE_BUFFERS.restore_snapshot()
...
def _test_conditional_determinism(model, test_data):
    ...
    v23: Freeze gate inputs + manual seed to eliminate telemetry variance."""
    if PULSE_BUFFERS is None:
        return {'mean_diff': 999.0}
    set_analog_mode_override(model, ANALOG_LAYERS, None)
    METAB.update_context()

    # v23: Freeze gate inputs so telemetry drift doesn't affect replays
    for layer_idx in LORA_LAYERS:
        g = model.transformer.h[layer_idx].mlp.c_fc
        if isinstance(g, BodyGatedLoRA):
            fast_vec = CTX.get_per_layer_fast_vec(layer_idx)
            fast_t = torch.from_numpy(fast_vec).float().to(DEVICE)
            mid_t = torch.from_numpy(CTX.mid_vec.copy()).float().to(DEVICE)
            slow_t = torch.from_numpy(CTX.slow_vec.copy()).float().to(DEVICE)
            g._frozen_gate_inputs = (fast_t, mid_t, slow_t)
    ...
    # v23: Unfreeze gate inputs
    for layer_idx in LORA_LAYERS:
        g = model.transformer.h[layer_idx].mlp.c_fc
        if isinstance(g, BodyGatedLoRA):
            g._frozen_gate_inputs = None
...
def _test_pulse_delay(model, test_data):
    ...
    for k in [1, 2, 4]:
        if k in snapshots:
            ...
            with pulse_mode(freeze=True):
                delayed_ppl = eval_ppl(model, test_data, n_batches=25)
...
def _test_freeze_pulse_ablation(model, test_data):
    ...
    normal_ppl = eval_ppl(model, test_data, n_batches=25)  # v26g: 50 (was 25)

    # Frozen — use pulse_mode() context manager to set FREEZE_PULSE_STATE=True
    with pulse_mode(freeze=True):
        frozen_ppl = eval_ppl(model, test_data, n_batches=25)  # v26g: 50 (was 25)
...
def _test_bypass_sensing_ablation(model, test_data):
    ...
    with pulse_mode(bypass_sensing=True):
        bypass_ppl = eval_ppl(model, test_data, n_batches=25)  # v26g: 50 (was 25)
...
def _test_disable_mode_writes_ablation(model, test_data):
    ...
    for mode_val in range(4):
        packed = pack_mode_byte(mode_val, mode_val, 3, 3)
        set_analog_mode_override(model, ANALOG_LAYERS, packed)
        mode_logits_live[mode_val] = collect_logits(model, test_data, n_batches=5)  # v26: 5 (was 3)
    set_analog_mode_override(model, ANALOG_LAYERS, None)

    # Collect logits with mode writes disabled — modes should be identical
    mode_logits_disabled = {}
    with pulse_mode(disable_mode=True):
        for mode_val in range(4):
            packed = pack_mode_byte(mode_val, mode_val, 3, 3)
            set_analog_mode_override(model, ANALOG_LAYERS, packed)
            mode_logits_disabled[mode_val] = collect_logits(model, test_data, n_batches=5)  # v26: 5 (was 3)
        set_analog_mode_override(model, ANALOG_LAYERS, None)
        disable_ppl = eval_ppl(model, test_data, n_batches=25)  # v26g: 50 (was 25)
...
def score_tests(phase_a, phase_b, baseline_ppl):
    ...
    if phase_b:
        ...
        # v23: T7b — Phase B kill-shot (flip dual-bank gate)
        kill_b = phase_b.get('kill_ratio_b', 0)
        add('T7b', 'Kill-Shot(B)', 'PASS' if kill_b > 1.02 else 'FAIL', f'{kill_b:.6f}', '>1.02')
        ...
        add('T25', 'Step Response', 'PASS' if 0 < rt <= 20 else 'FAIL',
            f'{rt} fwds', '0<rt<=20')
        ca = phase_b.get('causal_ablation', {})
        add('T26a', 'Causal Ablation', 'PASS' if ca.get('worst_ratio', 1) > 1.002 else 'FAIL',
            f'{ca.get("worst_ratio", 1):.4f}', '>1.002')
        # v23: T26d removed — WGP correction always ratio=1.000
        ...
        add('T34', 'Cond Determinism', 'PASS' if cd.get('mean_diff', 999) < 1.0 else 'FAIL',
            f'{cd.get("mean_diff", 999):.8f}', '<1.0')
        ...
        add('T38', 'Disable-Writes Abl', 'PASS' if dma.get('kl_ratio', 1) > 1.01 else 'FAIL',
            f'kl_ratio={dma.get("kl_ratio", 1):.4f}', '>1.01')
...
def main():
    ...
    print("=" * 60)
    print("z2136v27b: Gate Fix — supervision + temperature + fixed contrastive")
    print("Dual LoRA banks (cold/hot) + DIRECT gate supervision + ALL projections + RAPL")
    ...
    print(f"  LAMBDA_CONTRASTIVE={LAMBDA_CONTRASTIVE}, MARGIN={CONTRASTIVE_MARGIN}")
    print(f"  LAMBDA_GATE_SUPER={LAMBDA_GATE_SUPER}, GATE_TEMPERATURE={GATE_TEMPERATURE}")
    print(f"  GATE_INIT_STD={GATE_INIT_STD}, AGREEMENT_POWER={AGREEMENT_POWER}")
    print(f"  ANALOG_PROJ_NAMES={ANALOG_PROJ_NAMES}, SLOW_DIM={SLOW_DIM} (RAPL energy)")
    print("=" * 60)

    # Hardware Setup
    find_dvfs_sysfs()
    check_rapl()
    # v27: Prime RAPL readings so first power delta is valid
    if RAPL_AVAILABLE:
        _ = read_rapl_power_w()  # first read just initializes timestamps
        time.sleep(0.1)
        priming_power = read_rapl_power_w()
        print(f"[RAPL] Priming read: {priming_power:.1f}W")
    init_msr()
    try:
    ...
    print(f"  GPT-2: {n_params:.1f}M params, vocab={vocab}, layers={actual_layers}")

    N_LAYERS = actual_layers
    ANALOG_LAYERS = list(range(N_LAYERS))
    # v12: LoRA on layers 4-11 (8 layers) — more HW-coupled surface area
    # (z2119 had range(8,12)=4 layers — too few for signal to propagate)
    LORA_LAYERS = list(range(min(4, N_LAYERS), min(N_LAYERS, 12)))
    print(f"  Analog layers: 0-{N_LAYERS-1} ({N_LAYERS} total)")
    print(f"  LoRA layers: {LORA_LAYERS[0]}-{LORA_LAYERS[-1]} ({len(LORA_LAYERS)} total)")

    # v27: Initialize pulse buffers — 3x for all projections (c_attn, c_proj, c_fc)
    n_analog_slots = N_LAYERS * 3  # 3 projections per layer
    PULSE_BUFFERS = PulseBufferManager(n_analog_slots, max_blocks=PULSE_MAX_BLOCKS)
    print(f"  [PulseBuffers] {n_analog_slots+1} slots x {PULSE_MAX_BLOCKS} blocks = "
          f"{PULSE_BUFFERS.buffers.numel()} floats (v27: all projections)")
    ...
    # T1: Baseline PPL
    print(f"\n[T1] Baseline PPL (frozen GPT-2, no modifications)...")
    ...
    print(f"\n" + "=" * 60)
    print(f"z2136v27b Deep Coupling GPT-2 LM: {n_pass}/{n_total} PASS")
    print("=" * 60)

    # Save results
    out_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'z2136_gate_fix.json')
    out_path = os.path.abspath(out_path)
    result_obj = {
        'experiment': 'z2136v27b_deep_coupling',
        'description': (
            'v27: ALL projections (c_attn+c_proj+c_fc) patched with AnalogLinear for ~100% MODE write coverage. '
            'RAPL energy connected as slow-timescale body signal. 400 training batches (2x v26). '
            'Thermal-seeded stochastic rounding in HIP kernel. Dual LoRA banks + contrastive loss.'
        ),
        'backbone': f'GPT-2 ({n_params:.1f}M frozen)',
    ...
    # Also save timestamped copy so results are never overwritten
    ts = time.strftime('%Y%m%d_%H%M%S')
    ts_path = os.path.join(os.path.dirname(out_path), f'z2136_v27b_{ts}_{n_pass}of{n_total}.json')
