#!/usr/bin/env python3
"""
z2109: Rigorous Embodied LM — IA³ Conditioning + T4/T20/T21/T40 Fixes
=============================================================
Built on z2109. Key changes:

  z2109 fixes:
  1. T4: Pass skip_isa=False explicitly (auto-skip confounds sensor ablation test)
  2. Code-domain baseline PPL (benchmark for ppl_r1 comparison)
  3. T20: Use ppl_r0 as expected (test evaluates on wikitext, not code blend)
  4. T21: Use code domain + regime 1 override (wikitext PPL dominated by backbone)
  5. T40: BIDIRECTIONAL thermal_scale — remove relu, cold sharpens / hot diffuses
  6. T5/T6: Relabel as deterministic checks (ISA delta has zero variance → t=inf pathology)
  7. IA³-style per-layer modulation from workspace (~239K new params)
     workspace → per-layer gamma_v and gamma_ffn vectors for fine-grained conditioning
  8. Maximize low-level ISA control: use all MODE[7:0] bits for FP rounding + denorm

  Prior z2109 fixes preserved:
  1. CRITICAL: Causal mask for ALL layers
  2. T6: Cohen's d effect-size
  3. T14: RAPL wraparound uses max_energy_range_uj
  4. T23/T34: zlib compression ratio
  5. T19/T22: Pass skip_isa=False explicitly

  Prior z2107 fixes preserved:
  1. PPL token-weighting (was batch-weighted, should be token-weighted)
  2. T6 isfinite guard (inf was counting as PASS)
  3. T10 single-variable isolation (was corrupting 4 things, now only reported_delta)
  4. T10 delta criterion (gap-based, not absolute floor)
  5. T14 labels consistency (model-controlled path now computes loss too)
  6. T37 uses learned head (was self-referential: prediction = 50 * ground_truth)
  7. T40 tests on code domain + overrides ALL thermal sensors (wikitext too easy)
  8. evaluate_text_only_perplexity added (diagnostic: does adapter hurt backbone?)
  9. Embodied-fixed generation mode (does embodiment path break generation?)
  10. T1 reports text-only PPL as informational metric

Original z2106 architecture unchanged:

ROOT CAUSE: LoRA on ANY attention/MLP projection corrupts generation.
  - z2104: LoRA on q,k,v,o → collapse epoch 2
  - z2105: LoRA on v,o,gate,up,down (frozen q/k) → collapse epoch 4
  - Even v_proj LoRA alters what attention retrieves → degenerate attractors
  - 36M trainable LoRA params = massive overfitting capacity

SOLUTION — ACTIVATION STEERING (zero weight modification):
  Instead of modifying backbone weights (LoRA), learn small BIAS VECTORS
  added to the residual stream after each targeted layer.

  For each layer l in [12..29]:
    h_l = h_l + body_scale * ((1-gate) * r_A_l + gate * r_B_l)

  Where:
    r_A_l, r_B_l ∈ R^4096 are learnable steering vectors (zero-init)
    gate = regime gate (0=wikitext, 1=code)
    body_scale = hardware-derived amplitude (0 when sensors ablated)

  Properties:
    - ALL backbone weights 100% frozen (no LoRA anywhere)
    - Generation CAN'T collapse (backbone produces identical logits at body_scale=0)
    - 147K steering params (vs 36M LoRA = 245x reduction)
    - Still provides: embodiment (body_scale gates), kill-shot (wrong direction),
      thermal modulation (ThermalSoftmax preserved independently)

  Domain shift: regime 0 = wikitext, regime 1 = code. Both pure next-token.
  ThermalSoftmax: custom attention at layers 12-29 with thermal_scale injection.
  KL anchor: ALL regimes. Prevents drift (though drift should be minimal).

References:
  - Representation Engineering (Zou et al. 2023): steering vectors control behavior
...
CONTRASTIVE_KILLSHOT_LAMBDA = 0.5 # weight of contrastive kill-shot loss
    ...
    int N, int round_mode, int denorm_mode, int chain_code, int perm_code)
{
    ...
    // Key insight: near-zero values exercise denorm handling, near-0.5 ULP exercises rounding
    float a, b, c, d;
    if (chain_code == 0) {
        a = x * 1.5f + 0.3f;
        b = a * a - x * 0.7f;
        c = fmaf(a, b, x);              // FMA rounding differs nearest-even vs toward-zero
        // z2109: denorm-exercising chain — produce values near FLT_MIN
        d = c * 1.17549435e-38f;         // multiply toward denorm boundary
        d = fmaf(d, 1.0f, d);            // FMA on denorm — flush-to-zero vs preserve differs
        c = c + d;                        // fold denorm contribution back (0 if flushed, nonzero if preserved)
    } else {
        a = x * 0.7f - 0.3f;
        b = a * x + a * 0.5f;
        c = fmaf(b, a, -x);             // FMA with sign flip — rounding diverges
        // z2109: reciprocal chain — toward-zero vs nearest-even differ most for 1/x
        d = 1.0f / (fabsf(a) + 1.17549435e-38f);  // near-zero denominator → huge rounding divergence
        d = fmaf(d, 1.17549435e-38f, 0.0f);        // push result toward denorm
        c = c - d;                        // fold denorm contribution with opposite sign
    }

    // z2109: fp16 mix — half precision amplifies rounding mode differences (11-bit mantissa)
    __half h = __float2half(c);
    __half h2 = __float2half(d);         // z2109: denorm-path value also through fp16
    if (perm_code == 0) {
        h = __hadd(h, __hmul(h2, __float2half(0.001f)));  // z2109: blend denorm signal
    } else {
        h = __hmul(h, __float2half(-1.0f));
        h = __hneg(h);
        h = __hadd(h, h2);               // z2109: add denorm-path divergence
...
class BodyEncoder(nn.Module):
    ...
    def forward(self, sensor_dict, availability_mask=None, freeze_temporal=False):
        ...
        new_gate_hidden = self.gate_gru(freq_gate_instant, self.gate_hidden.detach())
...
class NormBoundedLoRALinear(nn.Module):
    ...
    def forward(self, x, regime_gate=None, body_scale=None):
        ...
        # 3. Blend based on hardware gate
        ...
        # 4. CRITICAL: Dynamic Norm-Bounding — prevent RMSNorm collapse
        # MUST cast to float32 to prevent overflow during L2 norm in bfloat16!
        base_norm = base_out.float().norm(p=2, dim=-1, keepdim=True).detach()
        lora_norm = lora_out.float().norm(p=2, dim=-1, keepdim=True)
        max_lora_norm = base_norm * 0.10  # max 10% of base magnitude
        scale_factor = torch.clamp(max_lora_norm / (lora_norm + 1e-8), max=1.0).to(base_out.dtype)
        lora_out = lora_out * scale_factor

        # 5. Apply hardware body scale (embodiment amplitude)
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# z2109: IA³ Conditioner — per-layer (gamma_v, gamma_ffn) from workspace
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
class IA3Conditioner(nn.Module):
    """z2109: Generate per-layer IA³ scaling vectors from workspace embedding.

    Replaces single-scalar body_scale with fine-grained per-channel modulation.
    For each steering layer, generates:
      gamma_v: [n_kv_heads * head_dim] — scales V projection outputs
      gamma_ffn: [ffn_dim] — scales FFN gate activation

    Initialized to all-ones (identity), so at epoch 0 the model is unchanged.
    Training learns which V-heads and FFN channels to amplify/suppress
    based on hardware state.

    Total params: n_layers * (n_kv_heads*head_dim + ffn_dim) ≈ 18 * (1024 + 12288) ≈ 239K
    """
    def __init__(self, workspace_dim, steering_layers, n_kv_heads, head_dim, ffn_dim):
        super().__init__()
        self.n_layers = len(steering_layers)
        self.layer_indices = sorted(steering_layers)
        self.n_kv_heads = n_kv_heads
        self.head_dim = head_dim
        self.ffn_dim = ffn_dim
        v_dim = n_kv_heads * head_dim  # 8 * 128 = 1024
        # Project workspace → per-layer gamma vectors
        self.v_proj = nn.Linear(workspace_dim, self.n_layers * v_dim)
        self.ffn_proj = nn.Linear(workspace_dim, self.n_layers * ffn_dim)
        # Init: all ones (identity at start) — zeros weights, ones bias
        nn.init.zeros_(self.v_proj.weight)
        nn.init.ones_(self.v_proj.bias)
        nn.init.zeros_(self.ffn_proj.weight)
        nn.init.ones_(self.ffn_proj.bias)
    ...
    def forward(self, workspace_emb):
        ...
        Returns:
            gamma_v: dict {layer_idx: [B, n_kv_heads, 1, head_dim]}
            gamma_ffn: dict {layer_idx: [B, 1, ffn_dim]}
        """
        B = workspace_emb.shape[0]
        v_dim = self.n_kv_heads * self.head_dim
        raw_v = self.v_proj(workspace_emb)    # [B, n_layers * v_dim]
        raw_ffn = self.ffn_proj(workspace_emb)  # [B, n_layers * ffn_dim]

        gamma_v = {}
        gamma_ffn = {}
        for i, layer_idx in enumerate(self.layer_indices):
            # Clamp gammas to [0.5, 2.0] to prevent instability
            gv = raw_v[:, i*v_dim:(i+1)*v_dim].clamp(0.5, 2.0)
            gv = gv.view(B, self.n_kv_heads, 1, self.head_dim)  # [B, n_kv_heads, 1, head_dim]
            gamma_v[layer_idx] = gv

            gf = raw_ffn[:, i*self.ffn_dim:(i+1)*self.ffn_dim].clamp(0.5, 2.0)
            gf = gf.view(B, 1, self.ffn_dim)  # [B, 1, ffn_dim]
            gamma_ffn[layer_idx] = gf

        return gamma_v, gamma_ffn
...
class EmbodiedQwen2(nn.Module):
    ...
    def __init__(self, backbone_model, body_encoder, steering_layers=STEERING_LAYERS):
        ...
        # randn init (not zeros) so L2 norm is safely > 0
        ...
        for layer_idx in steering_layers:
            self.steer_A[f'layer{layer_idx}'] = nn.Parameter(torch.randn(hidden_dim) * 0.01)
            self.steer_B[f'layer{layer_idx}'] = nn.Parameter(torch.randn(hidden_dim) * 0.01)
        ...
        # z2109: IA³ Conditioner — per-layer (gamma_v, gamma_ffn) from workspace
        # Much more expressive than single-scalar body_scale for hardware conditioning
        n_kv_heads = self.backbone.config.num_key_value_heads  # 8
        head_dim_ia3 = hidden_dim // self.backbone.config.num_attention_heads  # 128
        ffn_dim = self.backbone.config.intermediate_size  # 12288 for Qwen3-8B
        workspace_dim = N_WORKSPACE_SLOTS * TOKEN_DIM  # 4 * 32 = 128
        self.ia3_conditioner = IA3Conditioner(
            workspace_dim=workspace_dim,
            steering_layers=steering_layers,
            n_kv_heads=n_kv_heads,
            head_dim=head_dim_ia3,
            ffn_dim=ffn_dim)
    ...
    def forward(self, input_ids, sensor_dict, kargs, labels=None,
        ...
        # z2109: IA³ conditioning — per-layer gamma vectors from workspace
        workspace_emb = body_out.get('content_flat', None)
        ia3_gamma_v, ia3_gamma_ffn = {}, {}
        if workspace_emb is not None and not skip_substrate:
            ia3_gamma_v, ia3_gamma_ffn = self.ia3_conditioner(workspace_emb)
        if _dbg:
        ...
        if skip_substrate:
            ...
            thermal_scale = torch.exp(alpha * delta10).clamp(0.85, 1.40)  # z2109: bidirectional
        ...
        for i, layer in enumerate(self.backbone.model.layers):
            ...
            if i in self.steering_layer_set:
                ...
                v = layer.self_attn.v_proj(hidden_states, regime_gate=_rg, body_scale=_bs)

                # z2109: IA³ gamma_v — per-channel V modulation from workspace
                if i in ia3_gamma_v:
                    # v shape after v_proj: [B, T, n_kv_heads * head_dim]
                    # gamma_v[i]: [B, n_kv_heads, 1, head_dim]
                    _nkv = self.ia3_conditioner.n_kv_heads
                    _hd = self.ia3_conditioner.head_dim
                    v_4d = v.view(v.shape[0], v.shape[1], _nkv, _hd)  # [B, T, n_kv_heads, head_dim]
                    gv = ia3_gamma_v[i].to(v.dtype)  # [B, n_kv_heads, 1, head_dim]
                    v_4d = v_4d * gv.permute(0, 2, 1, 3)  # [B, T, n_kv_heads, head_dim] * [B, 1, n_kv_heads, head_dim]
                    v = v_4d.reshape(v.shape[0], v.shape[1], -1)  # [B, T, n_kv_heads * head_dim]
                ...
                attn_output = layer.self_attn.o_proj(attn_output, regime_gate=_rg, body_scale=_bs)
                hidden_states = residual + attn_output

                # --- MLP (manual for NormBounded LoRA on gate/up/down) ---
                residual = hidden_states
                hidden_states = layer.post_attention_layernorm(hidden_states)
                gate_out = layer.mlp.gate_proj(hidden_states, regime_gate=_rg, body_scale=_bs)
                up_out = layer.mlp.up_proj(hidden_states, regime_gate=_rg, body_scale=_bs)
                ffn_activated = layer.mlp.act_fn(gate_out) * up_out

                # z2109: IA³ gamma_ffn — per-channel FFN modulation from workspace
                if i in ia3_gamma_ffn:
                    gf = ia3_gamma_ffn[i].to(ffn_activated.dtype)  # [B, 1, ffn_dim]
                    ffn_activated = ffn_activated * gf

                hidden_states = layer.mlp.down_proj(ffn_activated, regime_gate=_rg, body_scale=_bs)
                hidden_states = residual + hidden_states

                # z2106: NORM-PRESERVING STEERING — L2-normalized, 5% of h_norm
                if not skip_substrate:
                    ...
                    # Calculate norms safely in float32 (bfloat16 overflows on L2 norm)
                    r_A_dir = F.normalize(self.steer_A[steer_key].float(), p=2, dim=-1).to(hidden_states.dtype)
                    r_B_dir = F.normalize(self.steer_B[steer_key].float(), p=2, dim=-1).to(hidden_states.dtype)
                    ...
                    steer = steer_dir * (h_norm * 0.05 * bs)
        ...
        # LM head — frozen (z2106: no weight modifications anywhere)
...
def train_lm_epoch(model, train_data, optimizer, epoch, kargs_a, tokenizer,
    ...
    """Train one epoch with regime alternation, domain shift, optional gaslighting.

    z2106 Activation Steering: regime 0 = wikitext, regime 1 = code.
    Both regimes use PURE next-token prediction (offset=1). Kill-shot via
    steering vector mismatch: wrong regime gate → wrong steering direction → PPL spike.
    NO LoRA — only steering vectors + body encoder are trainable.
    """
    model.train()
    total_loss = 0
    batch_idx = 0
    ...
    for batch_num in range(min(100, len(indices_wiki))):  # max 100 batches per epoch
        ...
        if KL_LAMBDA > 0 and current_regime == 0:  # z2106: KL only on regime 0 — let regime 1 freely specialize
            with torch.no_grad():
                frozen_out = model.backbone(input_ids)
                frozen_logits = frozen_out.logits[:, :-1, :].float()  # [B, T-1, V]
            tuned_logits = out['logits'][:, :-1, :].float()
            # KL(tuned || frozen) = sum(tuned * log(tuned/frozen))
            ...
            kl_loss = F.kl_div(tuned_log_probs, frozen_probs, reduction='batchmean')
            kl_loss = kl_loss.clamp(max=10.0)  # Prevent KL explosion
        ...
        batch_idx += 1
        if batch_idx % 10 == 0:
            print(f"    [{phase_name} Ep{epoch}] batch {batch_idx}/100 loss={loss.item():.3f} ce={ce_loss_val:.3f}", flush=True)
    ...
    with torch.no_grad():
        for i in range(0, min(len(_eval_data), n_batches * BS), BS):
            ...
            if out['loss'] is not None:
                # z2107: token-weighted PPL (not batch-weighted)
...
def evaluate_ppl_at_dvfs(model, test_data, regime, kargs, n_batches=20, kargs_b=None,
    ...
    with torch.no_grad():
        ...
        for i in range(0, min(len(_eval_data), n_batches * BS), BS):
            ...
            # z2105: PURE next-token for ALL regimes
            ...
            sensor_batch = {k: torch.zeros(B, d, device=DEVICE) for k, d in
                           zip(['delta', 'analog', 'energy', 'freq', 'intrinsic',
                               ...
                               [DELTA_DIM, ANALOG_DIM, ENERGY_DIM, FREQ_DIM, INTRINSIC_DIM,
                                ...
                                GPU_METRICS_DEEP_DIM, FENCE_DIM])}
            out = model(input_ids, sensor_batch, kargs, labels=labels,
                        regime_gate_override=torch.ones(BS, device=DEVICE),
                        skip_isa=False)  # z2109: force ISA to run even with zeroed sensors
            if out['loss'] is not None:
...
def train_backbone_probe(model, test_data, kargs, kargs_b=None):
    ...
    # z2106: Register hook on layer 20 (standard forward path)
    # Steering layers (12-29) use custom forward for ThermalSoftmax, but still
    # call layer components individually — hook on layer 8 (outside steering) is safest
...
def _generate_tokens_embodied_fixed(model, prompt_ids, sb_gen, kargs, n_tokens=32,
                                     mode='greedy', temperature=0.7, top_k=20, top_p=0.8,
                                     rep_penalty=1.2):
    """z2107: Generate with embodiment ON but sensors held constant and temporal state frozen.
    Tests whether substrate/LoRA/steering paths destabilize generation."""
    gen_ids = prompt_ids
    generated_tok_ids = []
    for _ in range(n_tokens):
        # Embodiment paths active, but ISA skipped, temporal state frozen
...
def generation_sanity_check(model, test_data, kargs, tokenizer, phase_name=""):
    ...
    model.eval()
    model.body_encoder.reset_state()
    print(f"\n[z2109] Generation sanity check after {phase_name}...")
    try:
        with torch.no_grad():
            for mode_name, mode, kwargs in [
                ('GREEDY (text-only)', 'greedy', {'embodied': False}),
                ('SAMPLED (text-only, t=0.7 k=20 p=0.8 rep=1.2)', 'sampled',
                 {'temperature': 0.7, 'top_k': 20, 'top_p': 0.8, 'rep_penalty': 1.2, 'embodied': False}),
                ...
                for sample_idx in range(2):  # 2 prompts per mode
                    ...
                    if embodied:
                        gen_ids = _generate_tokens_embodied_fixed(
...
def main():
    ...
    print("=" * 60)
    print("z2109: Rigorous Test Battery — Causal Masking + Test Fixes")
    ...
    # z2106: Domain-shift strategy — regime 1 trains on CODE (Python)
    # Both regimes do PURE next-token prediction. Kill-shot via steering mismatch.
    print(f"\n[DOMAIN-SHIFT] Loading code data for regime 1...")
    try:
        train_data_code = load_code_data(tokenizer, max_samples=2000)
        # Use last 500 code sequences for test
        test_data_code = train_data_code[-500:]
        train_data_code = train_data_code[:-500]
        ...
        train_data_code = None
        test_data_code = None
    print(f"\n[z2109 STEERING] Regime 0: wikitext (English), Regime 1: code (Python)")
    print(f"  Both regimes: pure next-token prediction (offset=1)")
    print(f"  Kill-shot: wrong steering direction → domain mismatch → PPL spike")
    ...
    print(f"\n{'='*60}")
    print(f"z2109 Qwen3-8B IA³-Embodied LM: {n_pass}/{n_total_tests} PASS")
    print(f"{'='*60}")

    # Save results
    out_path = '/home/ikaros/Documents/claude_hive/AMD_gfx1151_energy/results/z2109_rigorous_lm.json'
    final = {
        'experiment': 'z2109_rigorous_lm',
        'description': 'z2109: IA³ conditioning + T4 skip_isa + T20/T21 domain fix + T40 bidirectional thermal + T5/T6 deterministic + code baseline + low-level ISA control',
        'key_changes': {
            'backbone': 'Qwen3-8B (8.2B frozen, 36 layers, 4096 hidden, GQA 32Q/8KV)',
            'normbounded_lora': 'Dual LoRA (rank=16) on v/o/gate/up/down, output clamped to 10% of base norm',
            'norm_preserving_steering': 'L2-normalized direction vectors scaled to 5% of hidden_states norm',
            'steering_layers': list(STEERING_LAYERS),
            'lora_projections': 'v_proj, o_proj, gate_proj, up_proj, down_proj (NO q/k)',
            'thermal_softmax': 'Custom attention at steering layers, bidirectional thermal_scale [0.85, 1.40]',
            'domain_shift': 'regime 0=wikitext, regime 1=code (both next-token)',
            'why': 'Unconstrained LoRA/steering causes RMSNorm collapse. Norm bounding prevents it.',
        },
    ...
    try:
        ...
        torch.save({
            ...
            'ia3_conditioner_state': model.ia3_conditioner.state_dict(),  # z2109: IA³ gammas
