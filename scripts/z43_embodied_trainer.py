#!/usr/bin/env python3
"""
FEEL z43: Complete Embodied Trainer with FiLM TRAINING FIX
==========================================================

CRITICAL FIXES FROM z42:
1. FiLM + SKIP_PROJ GRADIENTS - Teacher-forced KL auxiliary loss!
2. MULTI-TARGET PREDICTOR - power, peak_power, j_tok, temp_end
3. FULL DISCOMFORT CALIBRATION - Not just thermal, full weighted sum
4. PROPER PARAM GROUPS - film_generator + skip_proj in optimizer

PREVIOUS FIXES (from z41/z42):
- No torch.tensor() wrapping, proper REINFORCE
- DVFS: "auto"/"min_sclk"/"peak"
- Persistent body state with decay/inertia
- Predictive head for next-window forecasting
- Lagged features + derivatives (52-dim)
- Real disturbances via DisturbanceScheduler
- reset_tracking_flags() not reset_decisions()

THE EMBODIMENT LOOP:
  sensors → body_state → gate_net → actions → hardware → sensors

NEW: Teacher-forced FiLM KL loss enables latent→express pathway

Author: FEEL Research Team
Date: 2026-01-16 (z43 - FiLM TRAINING FIX)
"""
...
class Z41Config:
    ...
    film_kl_coef: float = 0.1  # Weight for teacher-forced KL loss
    film_lr: float = 1e-4  # Learning rate for FiLM/skip_proj params

    val_every: int = 50  # More frequent checkpoints for z43
    checkpoint_dir: str = "models/z43_embodied"

    # Wandb
    wandb_project: str = "feel-z43-embodied"
...
class EmbodiedModelZ41(nn.Module):
    ...
    def compute_film_kl_loss(
        ...
        This gives gradient path to film_generator and skip_proj weights!
        ...
        # We want to MAXIMIZE KL (different states = different outputs)
        # So loss = -kl_mean (or use a target like KL > 0.1)
        kl_target = 0.1  # Target minimum KL divergence
        kl_loss = F.relu(kl_target - kl_mean)  # Loss = 0 if KL > target, else penalize
...
def train_epoch(
    ...
    for prompt_idx, prompt in enumerate(prompts):
        ...
        # FiLM KL loss for expression training - compute every 5 steps to balance speed/training
        if step % 5 == 0:
            # Create contrast between current sensors and a "baseline" version
            sensors_current = sensors.clone()
            sensors_baseline = sensors.clone()
            # Zero out derivatives (dims 36-44) and reduce power/temp anchors to create baseline
            sensors_baseline[36:44] = 0.0  # Derivatives = 0 means "stable"
            sensors_baseline[44:52] = sensors_baseline[44:52] * 0.5  # Reduce anchor signals
            ...
            kl_metrics = {"kl_mean": 0.0, "kl_loss": 0.0, "logit_diff": 0.0}

        # Total loss with FiLM KL
        total_loss = (
            policy_loss +
            entropy_loss +
            0.1 * pred_loss +
        ...
        if step % 1 == 0:  # Log every step
            ...
            print(f"  [{step:4d}] {stress_str:12s} gate={gate_mean:.3f} skip={m['skip_rate']:.2f} "
                  ...
                  f"strain={intero['strain_level'].item():.2f} {kl_str} {grad_str}", flush=True)
            ...
            if WANDB_AVAILABLE and config.use_wandb:
                wandb.log({
                    ...
                    "train/film_logit_diff": kl_metrics["logit_diff"],
                    # Predictions
