#!/usr/bin/env python3
"""
FEEL z47: Fixed Embodied Validator - Aligned with Training
============================================================

z47 VALIDATOR CRITICAL FIXES:

1. FIX PREDICTION ACCURACY TEST (State Mismatch Bug)
   - OLD: Read sensors, predict, then _generate_with_metrics reads AGAIN
   - NEW: _generate_with_metrics accepts pre-read sensors, uses SAME state
   - Prediction vs actual now aligned!

2. FIX INTEROCEPTIVE CALIBRATION TEST (Misaligned Load)
   - OLD: Stress applied, sensors read, THEN decode (sensors stale)
   - NEW: Read sensors DURING decode with concurrent sampling
   - Tests "interoception during decode" not "interoception during stress"

3. NEW TEST: LOOP CLOSURE
   - Measures correlation between sensor changes and action changes
   - Step response: inject stress → measure latency to action
   - This is THE test for true embodied control

4. STRONGER DISTURBANCE LEVELS
   - Matches z47 trainer's 50% disturbance probability
   - Larger matrix sizes for reliable power variance

5. MATCHES z47 TRAINER'S POWER-FIRST SAMPLING
   - Validator now uses same 8W power spread threshold
   - Aligned with training methodology

Author: FEEL Research Team
Date: 2026-01-17 (z47 - Aligned Validation)
"""
...
class Z47Validator:
    ...
    def run_all_tests(self) -> Dict:
        ...
        print("\n" + "=" * 70)
        print("Z47 EMBODIED VALIDATION SUITE (ALIGNED + LOOP CLOSURE)")
        print("=" * 70)
        print("z47 FIXES:")
        print("  - Prediction test uses SAME sensors for predict and generate")
        print("  - Interoception test reads sensors DURING decode")
        print("  - NEW: Loop closure test for true embodied control")
        print("=" * 70)

        results = {}

        # Test 1-6 (with z47 fixes for tests 5 and 6)
        ...
        results["prediction_accuracy"] = self.test_prediction_accuracy()  # z47 FIXED
        results["interoceptive_calibration"] = self.test_interoceptive_calibration()  # z47 FIXED

        # Test 7: Logits-KL Expression
        results["logits_kl_expression"] = self.test_logits_kl_expression()

        # Test 8: z47 NEW - Loop Closure (THE critical test!)
...
def main():
    parser = argparse.ArgumentParser(description="Z47 Embodied Validator")
    parser.add_argument("--checkpoint", type=str, required=True, help="Path to checkpoint")
    parser.add_argument("--output", type=str, default=None, help="Output JSON path")
    parser.add_argument("--trials", type=int, default=15, help="Default trials per test")
    args = parser.parse_args()

    validator = Z47Validator(args.checkpoint, default_trials=args.trials)
