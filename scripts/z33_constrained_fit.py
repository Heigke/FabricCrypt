"""z33_constrained_fit.py — DE fit with stricter physical bounds + weighted
objective. Addresses the z31 issue where DE found a patological VTH0=0.40
that broke the VG1=0.2 fit.

Changes vs z31:
  - VTH0 ∈ [0.50, 0.65]  (physical range; PTM130 nominal 0.54)
  - BJT_AREA ∈ [10⁻⁶, 10⁻⁴]  (≤100× schematic)
  - Per-VG1-group weighting in objective (each group contributes equally)
  - Over-prediction penalty: log(pred/meas) > 1 dec adds extra
  - Use both up + down sweep for fitting (z25-style bidirectional)
"""
...
def curve_score(pred, meas):
    """Asymmetric log-RMSE: over-prediction penalized harder."""
    m = (meas > ID_FLOOR) & (pred > 0)
    if m.sum() < 5: return None
    log_diff = np.log10(meas[m]) - np.log10(pred[m])
    # Asymmetric: over-pred (pred > meas → log_diff < 0) penalized 2× more
...
def objective(x):
    ...
    # Score: max-of-group-medians (so each group contributes equally;
    # worst-fitting group dominates → DE forces it to improve)
    return float(np.max(group_medians))


BOUNDS = [
    (0.50, 0.65),     # VTH0  ← physical range, PTM130 nominal 0.54
    (-6.0, -4.0),     # log10 BJT_AREA  ← schematic 1e-6 to 100×
...
def main():
    print(f"DE constrained fit (bounds: VTH0∈[0.50,0.65], AREA∈[10⁻⁶,10⁻⁴])")
    t0 = time.time(); ctr = [0]
    def cb(xk, conv):
        ctr[0] += 1
        o = objective(xk)
    ...
    res = differential_evolution(
        ...
        seed=11, tol=1e-3, workers=-1, updating="deferred",
    ...
    # All-curves visualization (3 panels, color by VG2)
    ...
    fig.suptitle(f"Constrained canonical NSRAM fit  ·  "
