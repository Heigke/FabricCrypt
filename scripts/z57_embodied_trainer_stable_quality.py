...
class Z49Config:
    ...
    quality_collapse_penalty: float = 0.25
    # z53+: Reduce teacher overhead by rescoring periodically or when skipping high
    teacher_eval_interval: int = 3  # compute teacher NLL every N steps
    teacher_eval_skip_threshold: float = 0.45  # always rescore when skip_avg >= this

    # z53+: Quality-as-constraint (homeostatic hormone / Lagrange multiplier)
    quality_lambda_init: float = 0.0
    quality_lambda_lr: float = 0.6
    quality_lambda_decay: float = 0.05
    quality_lambda_max: float = 2.0

    # z53+: Potential-based recovery shaping (prevents 'overheat then recover' reward hacking)
    recovery_potential_scale: float = 0.15
  # penalty multiplier when teacher quality collapses
    ...
    cool_power_cap_w: float = 90.0  # z54: avoid constant power overshoot penalty
    cool_j_target: float = 8.0
    hot_power_cap_w: float = 85.0  # z54: winnable on this platform (~80W decode)
    hot_j_target: float = 7.2  # z54: winnable (setpoints can be auto-calibrated)
    regime_switch_steps: int = 10
    regime_use_dvfs: bool = True

    # Safety targets
    power_cap_w: float = 90.0  # z54: only used when regime targets absent
    power_safety_w: float = 130.0
    temp_target_c: float = 70.0
    temp_safety_c: float = 80.0
    j_per_token_target: float = 7.5  # z54: baseline fallback near observed median
    ...
    # z54: Auto-calibrate regime setpoints from real hardware (prevents 'impossible game')
    ...
    # z54: Regime selection can be 'schedule' or 'homeostatic'

    # z55: Attention-window actuator (memory-bandwidth lever)
    # Rationale: decode-time inference is often memory-bandwidth bound due to KV-cache reads.
    # Restricting the *effective* attention horizon can reduce memory traffic on kernels that
    # honor padding masks / variable-length attention.
    use_attention_window_actuator: bool = True
    # Candidate windows (tokens). Larger last value approximates full context.
    attention_windows: Tuple[int, ...] = (256, 512, 1024, 2048, 4096)
    # If False, we neutralize this actuator in COOL regime (set to max window, zero out its logprob)
    attention_window_apply_in_cool: bool = False
    ...
    # z56 FIX: Prevent random latent injection from destroying text quality.
    # We start sensor-packet scale at ~0 and ramp it up after the model is stable.
    train_sensor_packet_encoder: bool = True
    sensor_packet_scale_max: float = 0.05
    sensor_packet_scale_warmup_steps: int = 2000
    sensor_packet_scale_start_step: int = 400
        ...
        with self.sensor_hub.measure_decode():
            ...
            while tokens_done < max_new_tokens:
                ...
                if getattr(config, 'inject_sensor_packets', True) and self.sensor_packet_encoder is not None:
                    ...
                    # z55: apply attention window after packet injection
                    if attn_window_tokens is not None and attention_mask.shape[1] > int(attn_window_tokens):
                        attention_mask[:, :-int(attn_window_tokens)] = 0


                # ---- Decode chunk ----
                remaining = max_new_tokens - tokens_done
                steps = min(chunk_tokens, remaining)

                # z53+: Adaptive chunking for tighter sensorimotor control under stress.
                # If we are near throttling / high memory pressure, reduce chunk length so we can react faster.
                try:
                    temp_now = float(fast.get("temp_gfx_deep", 0.0))
                    mem_now = float(fast.get("mem_ctrl_activity", 0.0))
                    thr = float(fast.get("throttle_status", 0.0))
                    if (thr > 0) or (temp_now > (getattr(config, "temp_target_c", 70.0) + 2.0)) or (mem_now > 90.0):
                        steps = min(max(4, chunk_tokens // 2), remaining)
                except Exception:
                    pass
                for _ in range(steps):
                    ...
                    # z55: restrict effective attention horizon (mask older tokens as padding)
                    if attn_window_tokens is not None and attention_mask.shape[1] > int(attn_window_tokens):
                        attention_mask[:, :-int(attn_window_tokens)] = 0
                    with torch.inference_mode():
                        out = self.base_model(
