#!/usr/bin/env python3
"""
FEEL v7.9: Intensity Signal Enhancement

FIXES from v7.8:
- FIX #4: Add direct intensity signal that doesn't get normalized away
  When GPU is cool (uniform REST state), group-relative advantage = 0
  because all samples have the same reward. v7.9 adds a direct intensity
  error signal to maintain gradient flow even on uniform data.

INHERITED FIXES from v7.8:
1. Use GROUP-RELATIVE advantage for BOTH tokens (not raw rewards)
2. Smooth derivative signals over 1s window (not instant 50Hz noise)
3. Proper swap test implementation (permute correct_actions correctly)
4. W&B confusion matrix for diagnostics
...
- Direct intensity error gradient (not just group-relative)
...
class ValidatedTrainer:
    ...
    def step(self, problems: List[Dict]) -> Dict:
        ...
        for g in groups:
            for t in g:
                ...
                # FIX #4: Intensity loss with DIRECT error signal
                # When GPU is cool, group-relative advantage = 0, so we add a direct signal
                # based on intensity error to maintain gradient flow
                ...
                # Scales from -1 (worst error) to +1 (perfect match)
                direct_intensity_signal = 1.0 - 2.0 * intensity_error

                # Combine group-relative advantage with direct intensity signal
                # - When adv != 0: uses group signal (good for relative comparisons)
                # - When adv == 0: direct signal provides gradients (uniform data case)
    ...
    def train(self, out_dir: str):
        ...
        print("\n" + "=" * 70)
        print("  FEEL v7.9: Intensity Signal Enhancement")
        print("  - FIX #1: Group-relative advantage for BOTH tokens")
        print("  - FIX #2: Smoothed derivatives (1s window)")
        print("  - FIX #3: Proper swap test")
        print("  - FIX #4: Direct intensity signal (works on uniform data)")
        print("  - Two-token: class (5) + intensity (64)")
        print("=" * 70 + "\n")
...
def main():
    ...
    ap.add_argument("--output", default="results/grpo_v7_8_fixed")
