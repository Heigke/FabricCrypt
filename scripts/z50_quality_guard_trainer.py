...
def load_prompts(path: str = None) -> List[str]:
    if path is None:
        ...
        # z50: Use combined dataset with GSM8K + MATH for difficulty spread
        ...
        if step % 1 == 0:
            ...
            if WANDB_AVAILABLE and config.use_wandb:
                wandb.log({
                    ...
                    # z50: Teacher rescoring metrics (use s['breakdown'] not bare breakdown)
                    "train/teacher_nll": s['breakdown'].get("teacher_nll") if 'breakdown' in s else None,
                    "train/teacher_quality": s['breakdown'].get("teacher_quality") if 'breakdown' in s else None,
                    "train/quality_collapse_penalty": s['breakdown'].get("quality_collapse_penalty", 0.0) if 'breakdown' in s else 0.0,
                    # z45: Recovery tracking
                    "train/recovery_count": s['breakdown'].get("recovery_count", 0) if 'breakdown' in s else 0,
                    "train/stuck_count": s['breakdown'].get("stuck_count", 0) if 'breakdown' in s else 0,
                    "train/recovery_score": s['breakdown'].get("recovery", 1.0) if 'breakdown' in s else 1.0,
                    # Losses
