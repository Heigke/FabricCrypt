/*
 * psp_cmd2_probe.c — Direct PSP command submission for RE
 *
...
/* Correct signature: psp_cmd2_submit_buf(psp, ucode, cmd, fence_mc_addr) */
typedef int (*psp_submit_fn)(void *psp, void *ucode, struct psp_cmd2_buf *cmd, u64 fence_mc_addr);

static int __init psp_cmd2_init(void)
{
	struct pci_dev *pdev;
	void *drm_dev, *adev, *psp;
	psp_submit_fn submit_buf;
	struct psp_cmd2_buf *cmd;
	int ret = 0;

	pr_info("psp_cmd2: === PSP COMMAND PROBE mode=%d ===\n", mode);

	pdev = pci_get_device(0x1002, 0x1586, NULL);
	if (!pdev) { pdev = pci_get_device(0x1002, PCI_ANY_ID, NULL); }
	if (!pdev) { pr_info("psp_cmd2: No AMD GPU\n"); return -ENODEV; }

	drm_dev = pci_get_drvdata(pdev);
	adev = drm_dev ? (void *)((u8 *)drm_dev - 0x10) : NULL;
	if (!adev) { pci_dev_put(pdev); return -ENODEV; }

	/* Find psp_cmd2_submit_buf function address
	 ...
	 * ring structures. */
	{
		...
		for (off = 0; off < 0x80000; off += 8) {
			...
			if (val == 0x97E0000000ULL) {
				...
				pr_info("psp_cmd2: TMR MC at adev+0x%X\n", off);
		...
		if (psp_off) {
			int j;
			pr_info("psp_cmd2: Context around TMR MC:\n");
			for (j = -16; j <= 16; j++) {
				u64 v = *(u64 *)((u8 *)adev + psp_off + j * 8);
				if (v != 0)
					pr_info("psp_cmd2:   adev+0x%lX = 0x%016llX\n",
						psp_off + j * 8, v);
			}
		}
	}
	 ...
	 * psp_cmd2_submit_buf takes (struct psp_context *psp, ...).
	...
	{
		/* Actually, psp_cmd2_submit_buf is a high-level function.
		 * Let's look at how the driver calls it:
		 *   psp_cmd2_submit_buf(psp, cmd_buf);
		 * where psp = &adev->psp
		 *
		...
		/* Let's try a different approach: use the KNOWN psp_cmd2_submit_buf
		 ...
		 * And psp_cmd2_submit_buf = 0xFFFFFFFFC0F2F840.
	...
	if (mode == 1) {
		...
		pr_info("psp_cmd2: GET_FW_ATTESTATION (cmd=0xF) — need PSP context\n");
	}

	if (mode == 2) {
		/* BOOT_CFG GET */
		pr_info("psp_cmd2: BOOT_CFG GET — building command\n");
		...
		pr_info("psp_cmd2: cmd_id=0x%X sub_cmd=%d\n",
			cmd->cmd_id, cmd->cmd.boot_cfg.sub_cmd);
		pr_info("psp_cmd2: (Cannot submit without PSP context pointer)\n");
	}

	/* Search for PSP context more aggressively */
	pr_info("psp_cmd2: === PSP CONTEXT SEARCH ===\n");
	{
				...
				/* Dump surrounding 256 bytes */
				{
					...
					for (j = -8; j <= 24; j++) {
						...
						if (v == 0x97E0000000ULL) tag = " ← TMR_MC";
						         ...
						         (v & 0xFFF) == 0) tag = " ← VRAM?";
						if (v != 0 || j == 0)
							pr_info("psp_cmd2:   [+0x%X] = 0x%016llX%s\n",
								off + j * 8, v, tag);
					}
				}
			}
	...
	/* Find PSP context by searching backward from TMR MC for adev pointer */
	{
		...
		pr_info("psp_cmd2: Searching for adev ptr 0x%llX backward from TMR...\n",
			adev_val);

		for (off = 0x3BAA0; off > 0x3A000; off -= 8) {
			u64 val = *(u64 *)((u8 *)adev + off);
			if (val == adev_val) {
				pr_info("psp_cmd2: *** adev ptr at adev+0x%X ***\n", off);
				pr_info("psp_cmd2: PSP context likely starts at adev+0x%X\n", off);
				psp_ctx = (u8 *)adev + off;

				/* Dump first 64 bytes of PSP context */
				{
					int j;
					pr_info("psp_cmd2: PSP context dump:\n");
					for (j = 0; j < 32; j++) {
						u64 v = *(u64 *)((u8 *)psp_ctx + j * 8);
						if (v != 0)
							pr_info("psp_cmd2:   psp+0x%03X = 0x%016llX\n",
								j * 8, v);
					}
				}
				break;
		...
		if (psp_ctx && mode >= 2) {
				...
				for (f = 0; f < 0x400; f += 8) {
					...
					if ((v >> 44) == 0x7FFF0 && v != 0x7FFF00700000ULL) {
						...
						if ((prev >> 48) == 0xFFFF) {
							...
							fence_mc = v;
							pr_info("psp_cmd2: fence_buf_mc at psp+0x%X = 0x%llX\n",
								f, v);
							break;
						}
					}
				...
				if (!fence_mc) {
					...
					for (f = 0; f < 0x400; f += 8) {
						...
						if (v >= 0x7FFF00000000ULL && v < 0x7FFF10000000ULL &&
						    v != 0x7FFF00700000ULL) {
							fence_mc = v;
							pr_info("psp_cmd2: candidate fence_mc at psp+0x%X = 0x%llX\n",
								f, v);
							break;
						}
					}
			...
			if (!fence_mc) {
				pr_info("psp_cmd2: Cannot find fence_buf_mc_addr, aborting\n");
				goto done;
			}

			/* NOW we can submit PSP commands! */
			pr_info("psp_cmd2: === SUBMITTING PSP COMMANDS ===\n");
			pr_info("psp_cmd2: fence_mc = 0x%llX\n", fence_mc);
			...
			if (mode == 2) {
				...
				pr_info("psp_cmd2: Submitting BOOT_CFG GET...\n");
				ret = submit_buf(psp_ctx, NULL, cmd, fence_mc);
				pr_info("psp_cmd2: BOOT_CFG result: ret=%d status=0x%X\n",
					ret, cmd->status);
				pr_info("psp_cmd2: boot_config = 0x%08X\n",
					cmd->cmd.boot_cfg.boot_config);
				pr_info("psp_cmd2: boot_config_valid = 0x%08X\n",
			...
			if (mode == 3) {
				...
				pr_info("psp_cmd2: SAVE MEC FW to GART BO mc=0x%llX\n",
					gart_mc);
				...
				pr_info("psp_cmd2: Submitting SAVE_RESTORE (save MEC)...\n");
				ret = submit_buf(psp_ctx, NULL, cmd, fence_mc);
				pr_info("psp_cmd2: SAVE result: ret=%d status=0x%X\n",
					ret, cmd->status);
				...
				if (ret == 0 && cmd->status == 0 && gart_cpu) {
					...
					pr_info("psp_cmd2: *** SAVED FW: %08X %08X %08X %08X ***\n",
			...
			if (mode == 4) {
				...
				pr_info("psp_cmd2: Submitting PROG_REG...\n");
				ret = submit_buf(psp_ctx, NULL, cmd, fence_mc);
				pr_info("psp_cmd2: PROG_REG result: ret=%d status=0x%X\n",
					ret, cmd->status);
			}
			...
			if (mode == 5) {
				...
				pr_info("psp_cmd2: Submitting GET_FW_ATTESTATION...\n");
				ret = submit_buf(psp_ctx, NULL, cmd, fence_mc);
				pr_info("psp_cmd2: FW_ATTEST result: ret=%d status=0x%X\n",
					ret, cmd->status);
				pr_info("psp_cmd2: Response data: %08X %08X %08X %08X\n",
...
static void __exit psp_cmd2_exit(void)
{
	pr_info("psp_cmd2: unloaded\n");
}

module_init(psp_cmd2_init);
module_exit(psp_cmd2_exit);
