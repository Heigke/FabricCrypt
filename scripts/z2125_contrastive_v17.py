#!/usr/bin/env python3
"""
z2125v17: Contrastive Only — No Agreement Dampening
====================================================
v17: z2124 (26/40) with agreement modulation REMOVED.

  z2124 diagnosis: Agreement modulation COUNTERACTS the kill-shot test.
  When T7 flips to wrong regime, agreement_scale drops, dampening LoRA,
  and model falls back to frozen GPT-2 (stable PPL). The dampening
  STABILIZES wrong-regime output instead of hurting it.

  v17 fix: Remove agreement modulation from forward pass entirely.
  Keep contrastive loss (λ=0.3, margin=0.3) — it trains the model to
  need correct HW state, but the LoRA output flows freely in all regimes.
  This means wrong regime → wrong gate → wrong LoRA modulation → higher PPL.

  Also: increase contrastive probability 25% → 40% for stronger training signal.
...
Hardware setup:
  ...
  sudo HSA_OVERRIDE_GFX_VERSION=11.0.0 PYTORCH_ROCM_ARCH=gfx1100 \\
    venv/bin/python -u scripts/z2125_contrastive_v17.py
"""
...
CONTRASTIVE_PROB = 0.40      # v17: increased from 0.25 for stronger training signal
...
class BodyGatedLoRA(nn.Module):
    ...
    def forward(self, x):
        ...
        # v17: NO agreement modulation — let LoRA flow freely so wrong gate
        # produces genuinely wrong output (critical for T7 kill-shot)
...
def run_phase_b(model, train_data, test_data, test_data_code, baseline_ppl, phase_a_results):
    print("\n" + "=" * 60)
    print("PHASE B: v17 3-Phase Curriculum (contrastive only)")
    print("  Phase1: Calibration 0-40 | Phase2: Reflex 40-120 | Phase3: Integration 120-200")
    print("  v17: contrastive kill-shot (no agreement mod) + z2120 silicon base")
    print("=" * 60)
...
def main():
    ...
    print("=" * 60)
    print("z2125v17: Contrastive Only — No Agreement Dampening")
    print("z2124 base + contrastive loss only (no agreement modulation)")
    ...
    print(f"  CONTRASTIVE_PROB={CONTRASTIVE_PROB}")
    print("=" * 60)
    ...
    print(f"\n" + "=" * 60)
    print(f"z2125v17 Contrastive Only GPT-2 LM: {n_pass}/{n_total} PASS")
    print("=" * 60)

    # Save results
    out_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'z2125_contrastive_v17.json')
    out_path = os.path.abspath(out_path)
    result_obj = {
        'experiment': 'z2125v17_contrastive_only',
        'description': (
            'v17: z2124 base (26/40) minus agreement modulation. Contrastive loss only '
            '(λ=0.3, margin=0.3, prob=0.40). LoRA flows freely so wrong gate → wrong output.'
        ),
