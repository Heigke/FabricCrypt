"""z237 — KMNIST (Kuzushiji-MNIST) at strong_input config.

Tests the "weak baseline → reservoir helps" hypothesis from z235/z236
on a 3rd image task. KMNIST = 10-class Japanese cursive characters,
28×28 grayscale, typical linear baseline ~60% (between MNIST 43%
and FashionMNIST 72%).

If projection ~60% AND reservoir > projection: hypothesis "weak
baseline favors reservoir" extends with a third datapoint.
If projection ~60% AND reservoir < projection: hypothesis "task-
specific, unrelated to baseline strength" gains evidence.

8 seeds. Same config as z235/z236: leak=0.30, g_VG2=0.20, N=1000.
"""
...
OUT = ROOT / "results/z237_kmnist"; OUT.mkdir(parents=True, exist_ok=True)
LOG = OUT / "live.log"
...
def main():
    ...
    log_line(f"=== z237 KMNIST 28x28 strong_input config (3rd image task) ===")
    ...
    log_line(f"loading KMNIST...")
    try:
        X, y = fetch_openml("Kuzushiji-MNIST", version=1, return_X_y=True,
                              as_frame=False, parser="auto")
    except Exception:
        try:
            X, y = fetch_openml("kmnist", version=1, return_X_y=True,
                                  as_frame=False, parser="auto")
        except Exception as e:
            log_line(f"KMNIST not available: {e}; trying EMNIST as fallback")
    ...
    for s in SEEDS:
        ...
        cooldown_to(60.0, 60)
        try:
            rng = np.random.default_rng(s + 4000)
    ...
    if results:
        ...
        summary = {
            ...
            "ci_excludes_0_positive": bool(ci_lo > 0),
            "ci_excludes_0_negative": bool(ci_hi < 0),
        }
        (OUT / "summary.json").write_text(json.dumps(summary, indent=2))
        log_line(f"\n=== n={len(results)} seeds ===")
        log_line(f"reservoir : {accs.mean():.4f}  projection: {projs.mean():.4f}")
        log_line(f"Δ mean    : {deltas.mean():+.2f}pp  CI95 [{ci_lo:+.2f}, {ci_hi:+.2f}]")
        log_line(f"positive  : {n_pos}/{len(results)}, p={p:.3g}")

        # Hypothesis check vs z235/z236
        log_line(f"\nProjection baselines across image tasks:")
        log_line(f"  MNIST (z235):    43%  → reservoir Δ=+5.1pp")
        log_line(f"  KMNIST (z237):   {projs.mean()*100:.0f}%  → reservoir Δ={deltas.mean():+.1f}pp")
        log_line(f"  FashionMNIST (z236): 72%  → reservoir Δ=-10.6pp")
