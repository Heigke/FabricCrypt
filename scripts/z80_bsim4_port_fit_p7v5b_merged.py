...
OUT = ROOT / "results/z80_bsim4_port_fit_p7v5b_merged"
...
def main():
    ...
    # ---------------- Stage 3 (MERGED v5b: Iii+BJT co-fit on full data) ---------------- #
    # Lesson from v5: fitting Iii alone first (Stage 3) and BJT alone after (Stage 4)
    # leaves Stage 4 unable to compensate when BJT amplifies an "alone-fitted" Iii.
    # v5b: co-fit alpha0, beta0, Bf, Rb_leak, gamma_VG2 simultaneously on ALL 33 curves
    # with BJT + Iii + GIDL all on. PT still off — let regenerative loop do the work.
    stage3_names = ["alpha0", "beta0", "Bf", "Rb_leak", "gamma_VG2"]
    gates3 = {"enable_iii": True, "enable_gidl": True,
              "enable_bjt": True, "enable_pt": False}
    thetas, l3 = multistart_stage(3, thetas, base_card, geom, curves,
                                   stage3_names, gates3, balanced=True,
                                   t0=t0)
    save_stage_summary(3, thetas, l3)

    # ---------------- Stage 4 (kept for backward compat — now no-op fine-tune) ---------------- #
    # Add punch-through to handle high-Vd tail beyond what Iii+BJT can do.
    stage4_names = ["I_PT0", "V_PT_th", "V_PT_scale", "k_Vb_PT"]
