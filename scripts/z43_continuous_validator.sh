#!/bin/bash
# Continuous validation for z43 - checks for new checkpoints and validates them

CHECKPOINT_DIR="models/z43_embodied"
RESULTS_DIR="results"
PYTHON="/home/ikaros/venvs/torch-rocm/bin/python"
VALIDATOR="scripts/z43_embodied_validator.py"
...
while true; do
    ...
    for ckpt in "$CHECKPOINT_DIR"/step_*.pt; do
        if [ -f "$ckpt" ]; then
            ...
            output_file="$RESULTS_DIR/z43_validation_step${step}.json"

            echo "[$(date '+%H:%M:%S')] Running validation..."
            $PYTHON -u "$VALIDATOR" --checkpoint "$ckpt" --output "$output_file" 2>&1 | tee -a z43_continuous_validation.log
    ...
    if sshpass -p "daedalus" ssh daedalus@192.168.0.37 "ls ~/AMD_gfx1151_energy/models/z43_embodied/*.pt 2>/dev/null" | head -1 > /dev/null 2>&1; then
        for remote_ckpt in $(sshpass -p "daedalus" ssh daedalus@192.168.0.37 "ls ~/AMD_gfx1151_energy/models/z43_embodied/*.pt 2>/dev/null" 2>/dev/null); do
