#!/usr/bin/env python3
"""
FEEL z48: Sensor-Responsive Embodied Trainer
=============================================

z48 MAJOR CHANGES (fixing "constant skip policy" problem):
...
Date: 2026-01-16 (z48 - SENSOR-RESPONSIVE TRAINING)
"""
...
@dataclass
class Z48Config:
    """z48 Sensor-Responsive Embodied Training Configuration."""
    # z48: Use DeepSeek-R1-Distill for reasoning capabilities
    ...
    # z48 FIX D: Body state settings - SLOWER DECAY for persistence
    body_dim: int = 64
    body_decay: float = 0.05  # z48: HALVED from 0.1 for longer memory
    body_noise_std: float = 0.01

    # Reward weights
    quality_weight: float = 0.15
    energy_weight: float = 0.20  # z48: Slightly reduced
    recovery_weight: float = 0.15
    throughput_weight: float = 0.15
    prediction_weight: float = 0.10
    discomfort_weight: float = 0.10
    sensor_reliance_weight: float = 0.15  # z48 FIX C: NEW - reward using sensors
    ...
    hot_power_cap_w: float = 65.0  # z47: was 50.0 (below observed idle during decode)
    ...
    # z48: SAFETY TARGETS (conservative!)
    ...
    j_per_token_target: float = 6.0  # z48: More realistic target

    # z48: Safe stress bounds
    stress_power_min_w: float = 80.0  # z47: was 70.0 (separate from idle)
    stress_power_max_w: float = 120.0  # z47: was 110.0
    ...
    # z48: Disturbance probability
    disturbance_prob: float = 0.50  # z47: was 0.30
    ...
    # z48 FIX B/C: Contrastive and sensor reliance
    contrastive_coef: float = 0.3  # z48: INCREASED from 0.2
    contrastive_margin: float = 0.05
    sensor_buffer_size: int = 200
    sensor_reliance_coef: float = 0.1  # z48 FIX C: g_real vs g_shuffled loss
    quantile_pct: float = 0.15  # z48 FIX B1: Use top/bottom 15% for contrastive

    # z48 FIX B3: Training phases - EXTENDED GATE PRETRAIN
    gate_pretrain_steps: int = 100     # z48: DOUBLED! No RL, contrastive only
    expected_skip_steps: int = 150     # z48: Extended
    ...
    val_every: int = 50
    checkpoint_dir: str = "models/z48_embodied"

    # Wandb
    wandb_project: str = "feel-z48-embodied"
    wandb_run_name: Optional[str] = None
    use_wandb: bool = True

    # z48: Closed-loop decode (fast embodiment loop)
    closed_loop_train: bool = False  # if True, training uses chunked closed-loop decisions
    decision_chunk_tokens: int = 16  # re-decide skip/DVFS every N generated tokens

    # z48: Sensor packets as a "new modality" in LLM latent space
    inject_sensor_packets: bool = True
    sensor_packet_tokens: int = 4    # number of learned pseudo-tokens per packet
    sensor_packet_interval_chunks: int = 1  # add a packet every chunk
    live_dashboard: bool = True      # print live telemetry during closed-loop generate



    def __post_init__(self):
        if self.gate_layers is None:
            # z48: DeepSeek-R1-Distill-Qwen-7B has 28 layers
            self.gate_layers = [5, 10, 15, 20, 25]
...
class RecoveryAwareReward:
    ...
    def compute(
        ...
        # z48: Apply regime quality bonus
        ...
        reward = (
            ...
            fast_signal_bonus -
            skip_penalty_value  # z48: Regime-based skip penalty
        )
...
class SensorBuffer:
    ...
    def __init__(
        ...
        min_power_spread_w: float = 8.0,
        min_discomfort_spread: float = 0.15,
    ):
    ...
    def _compute_discomfort(self, power_w: float, temp_c: float) -> float:
        ...
        return float(power_stress + temp_stress)
...
def train_epoch(
    ...
    for prompt_idx, prompt in enumerate(prompts):
        ...
        for sample_idx in range(config.num_samples):
            ...
            if getattr(config, 'closed_loop_train', False):
            ...
            log_probs.append(sample_out['total_log_prob'] if getattr(config,'closed_loop_train',False) else action_result['total_log_prob'])
...
def save_checkpoint(model: EmbodiedModel, step: int, config: Z48Config):
    """Save checkpoint."""
...
def main():
    parser = argparse.ArgumentParser(description="FEEL z48: Sensor-Responsive Embodied Trainer")
    parser.add_argument("--epochs", type=int, default=3)
    parser.add_argument("--max-prompts", type=int, default=500)
    parser.add_argument("--checkpoint-dir", type=str, default="models/z48_embodied")
    ...
    parser.add_argument("--contrastive-coef", type=float, default=0.3)  # z48: Increased
    parser.add_argument("--wandb-project", type=str, default="feel-z48-embodied")
    parser.add_argument("--no-wandb", action="store_true")
    parser.add_argument("--closed-loop-train", action="store_true", help="Use chunked closed-loop decisions during training (experimental)")
    ...
    config = Z48Config(
        ...
        closed_loop_train=args.closed_loop_train,
    ...
    if WANDB_AVAILABLE and config.use_wandb:
        ...
        run_name = config.wandb_run_name or f"z48_regime_{hostname}"
        wandb.init(
            project=config.wandb_project,
            name=run_name,
            config=asdict(config),
            tags=["z48", "regime-curriculum", "sensor-responsive", hostname],
        )

    print("=" * 70)
    print("FEEL z48: SENSOR-RESPONSIVE EMBODIED TRAINER")
    print("=" * 70)
    print("z48 KEY FIXES (from z45 constant-skip problem):")
    ...
    # z48: Load checkpoint if resuming
    global_step = 0
    if args.resume_from and Path(args.resume_from).exists():
        ...
        print(f"  NOTE: z48 continues training with NEW curriculum from this point")
