...
def run_sweep(k1_scales, mbjt_steps, bf_grid, out_path):
    ...
    # Push partial back to master (ikaros). Skip on master itself.
    try:
        import socket, subprocess
        host = socket.gethostname()
        if "ikaros" not in host.lower():
            cmd = ["scp", "-q", "-o", "BatchMode=yes",
                   "-o", "StrictHostKeyChecking=no",
                   str(out_path),
                   "ikaros@192.168.0.35:/home/ikaros/Documents/claude_hive/"
                   "AMD_gfx1151_energy/results/z323_v5_extended/"]
            r = subprocess.run(cmd, capture_output=True, text=True, timeout=60)
            if r.returncode == 0:
                print(f"[z323] pushed {out_path.name} to master", flush=True)
            else:
                print(f"[z323] WARN scp-back rc={r.returncode}: "
                      f"{r.stderr[:200]}", flush=True)
    except Exception as e:
        print(f"[z323] WARN scp-back exception: {e}", flush=True)
    return 0
