...
class GRPOConfig:
    ...
    # FIX: Lower bias for exploration (was 15)
    action_bias_start: float = 3.0
    ...
    # Protocol: ensure first generated token is one of the FEEL action tokens.
    # This is NOT choosing the action; it only enforces the output channel.
    force_action_first: bool = True

    # Optimize primarily for emitting the correct action token (first token).
    loss_on_action_only: bool = True

    # PPO/GRPO-style stability knobs (optional but recommended)
    ppo_clip: float = 0.2
    kl_beta: float = 0.0  # set >0 to penalize drift vs. a reference policy
    dtype: str = "bf16"
...
class DeepEmbodiedTrainer:
    ...
    def _setup_token_training(self):
        ...
        self._action_token_id_list = token_ids
        self._id_to_action = {tid: act for act, tid in self.action_token_ids.items()}
    ...
    def generate(self, question: str, epoch: int) -> Trajectory:
        ...
        with torch.no_grad(), torch.amp.autocast('cuda', dtype=self.dtype):
            for step in range(self.cfg.max_new_tokens):
                ...
                if step == 0:
                    # Enforce action-channel protocol: first token must be one of the FEEL action tokens.
                    if self.cfg.force_action_first:
                        # Keep only logits for action tokens; everything else becomes effectively impossible.
                        masked = torch.full_like(logits, -1e9)
                        masked[0, self._action_token_id_list] = logits[0, self._action_token_id_list]
                        logits = masked
                    # Bias the *correct* action among the action-token set (curriculum; decays over epochs).
                    if bias > 0:
        ...
        if gen_tokens.numel() > 0:
            ...
            if self.cfg.loss_on_action_only:
                tok_logp = tok_logp[:1]  # focus optimization on the action token
        else:
        ...
        # Determine action from the FIRST generated token id (protocol).
        action = None
        if gen_tokens.numel() > 0:
            first_id = int(gen_tokens[0].item())
            action = self._id_to_action.get(first_id, None)
            ...
            for epoch in range(self.cfg.num_epochs):
                ...
                for step in range(self.cfg.steps_per_epoch):
                    ...
                    print(f"  Step {step+1}: R={m['mean_reward']:.2f} "
                          ...
                          f"T={m['temp_avg']:.1f}C S={m['stress_avg']:.2f} "
                          f"∇={m['grad_norm']:.2f}")
