...
def main():
    ...
    # F6.v7: M1 S/D resistance bias-dependence per O24 oracle. PRWG=Vg-dep,
    # PRWB=Vbs-dep. Default PRWG=0 turns Vg-dep off — likely too flat for
    # the VG1=0.40 weak-inversion residual cluster.
    for k_env, k_model in [("NSRAM_M1_RDSW", "rdsw"),
                            ("NSRAM_M1_RDSWMIN", "rdswmin"),
                            ("NSRAM_M1_PRWG", "prwg"),
                            ("NSRAM_M1_PRWB", "prwb")]:
        v = os.environ.get(k_env)
        if v is not None:
            old = model_M1._values.get(k_model)
            model_M1._values[k_model] = float(v)
            print(f"[z91g] M1.{k_model}: {old} -> {v}", flush=True)
    ...
    # M3b F1.b Iii→Vb amplification gain
    iii_gain_env = os.environ.get("NSRAM_III_BODY_GAIN")
    if iii_gain_env is not None:
        cfg.iii_body_gain = float(iii_gain_env)
        print(f"[z91g] iii_body_gain = {cfg.iii_body_gain:g}", flush=True)
    ...
    for c in curves:
        ...
        # M3c.4 (post-O22 Vb-clamp finding 2026-05-04): the BJT card's
        # Is = 5e-9 is 6+ OoM too large for a 130 nm lateral parasitic
        # NPN. Real Is ~ 1e-15. Smaller Is shifts Vb-clamp upward and
        # restores bias-dependent variation (empirically validated:
        # at Is=1e-18, Vb varies 0.93→1.56 V across biases vs 0.39 V flat
        # at default Is). NSRAM_BJT_IS env var lets us sweep it without
        # editing the card.
        bjt.Is = float(os.environ.get("NSRAM_BJT_IS", str(bjt.Is)))
        # F6 (post-O24 oracle): VAF (forward Early voltage). Default 100V is
        # vertical-NPN value; lateral NPN with low-doping base typically 1-50V.
        # Both gpt-5 and gemini ranked this as the #1 untested knob.
        bjt.Va = float(os.environ.get("NSRAM_BJT_VA", str(bjt.Va)))
        bjt.Ikf = float(os.environ.get("NSRAM_BJT_IKF", str(bjt.Ikf)))
        # F6.v6: B-E recombination (low-Vbe regime). ISE adds extra current
        # at low forward bias; NE>1 makes it slope-shallower than ideal.
        # Targets VG1=0.40 weak-inversion cluster per O24 oracles.
        bjt.Ise = float(os.environ.get("NSRAM_BJT_ISE", str(bjt.Ise)))
        bjt.Ne = float(os.environ.get("NSRAM_BJT_NE", str(bjt.Ne)))
        # F6.v8 (post-O25 unanimous): bias-dependent η_lat = η(Vbe) sigmoid.
        # Activated by NSRAM_ETA_SIGMOID=1; tuned via NSRAM_ETA_{0,FINAL,K,VTURN}.
        if os.environ.get("NSRAM_ETA_SIGMOID", "0") == "1":
            cfg.eta_sigmoid = True
            cfg.eta_0     = float(os.environ.get("NSRAM_ETA_0", "0.6"))
            cfg.eta_final = float(os.environ.get("NSRAM_ETA_FINAL", "0.1"))
            cfg.eta_k     = float(os.environ.get("NSRAM_ETA_K", "30.0"))
            cfg.eta_vturn = float(os.environ.get("NSRAM_ETA_VTURN", "0.7"))
            print(f"[z91g] eta_sigmoid: η_0={cfg.eta_0}, η_final={cfg.eta_final}, "
                   f"k={cfg.eta_k}, V_turn={cfg.eta_vturn}", flush=True)
