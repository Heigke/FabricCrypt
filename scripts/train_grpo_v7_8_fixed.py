#!/usr/bin/env python3
"""
FEEL v7.8: Fixed Analog Embodied GRPO

CRITICAL FIXES from v7.7:
...
class ValidatedTrainer:
    ...
    def step(self, problems: List[Dict]) -> Dict:
        ...
        for g in groups:
            for t in g:
                ...
                # Intensity loss with entropy (using SAME group-relative advantage)
                if ent_coef > 0:
                    intensity_probs = intensity_logp.exp()
                    intensity_entropy = -(intensity_probs * intensity_logp).sum(dim=-1).mean()
                    intensity_loss = -(adv * chosen_intensity_logp) - ent_coef * intensity_entropy
                else:
                    intensity_loss = -(adv * chosen_intensity_logp)
    ...
    def train(self, out_dir: str):
        ...
        print("\n" + "=" * 70)
        print("  FEEL v7.8: Fixed Analog Embodied GRPO")
