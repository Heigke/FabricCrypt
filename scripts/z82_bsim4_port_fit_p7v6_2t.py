...
OUT = ROOT / "results/z82_bsim4_port_fit_p7v6_2t"
...
def forward_curve(values: dict, model: BSIM4Model, cfg: NSRAMCell2TConfig,
    ...
    with patch_sd(sd_M1, values), patch_sd(sd_M2, values):
        result = forward_2t(cfg, model, bjt, Vd_seq, VG1_t, VG2_t,
                            warm_start=True)
    return result["Id"].abs()


# --------------------------------------------------------------------------- #
# Loss: balanced (50% uniform + 50% knee-weighted) log-MSE
# --------------------------------------------------------------------------- #
def balanced_logmse(pred: torch.Tensor, meas: torch.Tensor) -> torch.Tensor:
    log_eps = 1e-15
    ...
    mse_uniform = (err ** 2).mean()
    mse_knee = ((err ** 2) * weights).mean() / weights.mean()
...
def stage_loss(thetas, model, cfg, curves):
    ...
    for c in curves:
        try:
            ...
            continue
        l = balanced_logmse(Id_pred, c["Id"])
        if torch.isfinite(l):
            losses.append(l)
    if not losses:
