...
OUT = ROOT / "results/z91c_bsim4_port_fit_polyVG"
...
PARAM_SPEC = {
    ...
    # z91c POLY(VG1, VG2) coefficients per Sebas's email:
    #   alpha0(VG1,VG2) = alpha0 + a0_b·VG1 + a0_c·VG2 + a0_d·VG1·VG2
    #   beta0(VG1,VG2)  = beta0  + b0_b·VG1 + b0_c·VG2 + b0_d·VG1·VG2
    #   Bf(VG1,VG2)     = Bf · exp(bf_b·VG1 + bf_c·VG2 + bf_d·VG1·VG2)
    # The constant terms (alpha0/beta0/Bf above) carry the bulk of magnitude;
    # the linear/cross terms add the bias-dependence Sebas observed.
    # Bounds chosen so the per-bias value can vary ±50% over the (VG1,VG2)
    # range used in the data: VG1∈[0.2,0.6], VG2∈[-0.3,+0.4].
    "a0_b": {"kind": "linb", "init": 0.0, "bounds": (-3e-2, 3e-2)},
    "a0_c": {"kind": "linb", "init": 0.0, "bounds": (-3e-2, 3e-2)},
    "a0_d": {"kind": "linb", "init": 0.0, "bounds": (-3e-2, 3e-2)},
    "b0_b": {"kind": "linb", "init": 0.0, "bounds": (-10.0, 10.0)},
    "b0_c": {"kind": "linb", "init": 0.0, "bounds": (-10.0, 10.0)},
    "b0_d": {"kind": "linb", "init": 0.0, "bounds": (-10.0, 10.0)},
    "bf_b": {"kind": "linb", "init": 0.0, "bounds": (-2.0, 2.0)},
    "bf_c": {"kind": "linb", "init": 0.0, "bounds": (-2.0, 2.0)},
    "bf_d": {"kind": "linb", "init": 0.0, "bounds": (-2.0, 2.0)},
}
...
def _apply_poly_VG(values: dict, VG1: float, VG2: float) -> dict:
    """z91c: build per-curve effective values using poly(VG1, VG2) for
    alpha0, beta0, Bf. The bare values dict carries the constant offsets +
    poly coefficients; here we replace alpha0/beta0/Bf with their evaluated
    scalars at this curve's (VG1, VG2). All other params pass through.
    """
    out = {k: v for k, v in values.items() if k not in POLY_COEFFS}
    vg1 = torch.tensor(float(VG1), dtype=torch.float64)
    vg2 = torch.tensor(float(VG2), dtype=torch.float64)
    cross = vg1 * vg2

    if "alpha0" in values:
        a0_b = values.get("a0_b", torch.tensor(0.0, dtype=torch.float64))
        a0_c = values.get("a0_c", torch.tensor(0.0, dtype=torch.float64))
        a0_d = values.get("a0_d", torch.tensor(0.0, dtype=torch.float64))
        # Soft positivity: alpha0 must stay > 0 for impact-ion to make sense.
        # Use ReLU-ish floor at 1e-6 so tiny negatives don't blow up.
        out["alpha0"] = (values["alpha0"] + a0_b*vg1 + a0_c*vg2 + a0_d*cross).clamp_min(1e-6)
    if "beta0" in values:
        b0_b = values.get("b0_b", torch.tensor(0.0, dtype=torch.float64))
        b0_c = values.get("b0_c", torch.tensor(0.0, dtype=torch.float64))
        b0_d = values.get("b0_d", torch.tensor(0.0, dtype=torch.float64))
        out["beta0"] = (values["beta0"] + b0_b*vg1 + b0_c*vg2 + b0_d*cross).clamp_min(1.0)
    if "Bf" in values:
        bf_b = values.get("bf_b", torch.tensor(0.0, dtype=torch.float64))
        bf_c = values.get("bf_c", torch.tensor(0.0, dtype=torch.float64))
        bf_d = values.get("bf_d", torch.tensor(0.0, dtype=torch.float64))
        # Multiplicative — exp(linear) keeps Bf > 0 always.
        out["Bf"] = values["Bf"] * torch.exp(bf_b*vg1 + bf_c*vg2 + bf_d*cross)
    return out
...
def forward_curve(values: dict, model: BSIM4Model, cfg: NSRAMCell2TConfig,
    ...
    z91c: evaluates poly(VG1, VG2) for alpha0, beta0, Bf before patching.
    """
    eff = _apply_poly_VG(values, VG1, VG2)
    sd_M1 = cfg._sd_M1
    sd_M2 = cfg._sd_M2
    ...
    with patch_sd(sd_M1, eff), patch_sd(sd_M2, eff):
...
def main():
    ...
    cfg3 = make_cfg_and_sd(model, gates={"use_iii": True, "use_gidl": True,
                                          "use_bjt": True})
    # z91c: also fit the bilinear poly(VG1, VG2) coefficients for α₀, β₀, Bf
