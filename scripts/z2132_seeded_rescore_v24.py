#!/usr/bin/env python3
"""
z2132v24: Pure Rescore — v18 Base + Minimal Test Mechanics
==========================================================
v24: v23 + seeded training (torch+numpy+random seed=42). Only changes:
  1. Add _force_wrong_gate/_frozen_gate_inputs to BodyGatedLoRA (test only)
  2. Add T7b test (flip gate → wrong bank PPL)
  3. Freeze gate inputs in T6/T34 for determinism
  4. T26a: zero ALL body inputs (fast+mid+slow)
  5. Scoring: remove T26d, T38>1.03, T34<1.0, add T7b>1.02

  v18 base scored 28/40. Training variance dominates (+/-5 tests per run).
  v23 target: v18 base tests + threshold fixes = 30-33/40.
...
Hardware setup:
  ...
  sudo HSA_OVERRIDE_GFX_VERSION=11.0.0 PYTORCH_ROCM_ARCH=gfx1100 \\
    venv/bin/python -u scripts/z2132_seeded_rescore_v23.py
"""
...
def run_phase_b(model, train_data, test_data, test_data_code, baseline_ppl, phase_a_results):
    ...
    N_TRAIN_BATCHES = 200
    torch.manual_seed(42)  # v24: seed for reproducible training
    np.random.seed(42)
    random.seed(42)
    LR = 1e-4
...
def main():
    ...
    print("=" * 60)
    print("z2132v24: Seeded Rescore — v18 Base + Seed + Minimal Test Mechanics")
    ...
    print(f"\n" + "=" * 60)
    print(f"z2132v24 Pure Rescore GPT-2 LM: {n_pass}/{n_total} PASS")
    print("=" * 60)

    # Save results
    out_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'z2132_seeded_rescore.json')
    out_path = os.path.abspath(out_path)
    result_obj = {
        'experiment': 'z2132v24_pure_rescore',
        'description': (
