#!/usr/bin/env python3
"""
FEEL z46: Sensor-Responsive Embodied Trainer
=============================================

z46 MAJOR CHANGES (fixing "constant skip policy" problem):

The z45 model discovered it could get good rewards with a FIXED skip policy,
so it learned to IGNORE sensors. This made counterfactual_swap and body_persistence
fail. z46 fixes this with:

FIX A: TWO-REGIME CURRICULUM ("must change policy")
   - COOL regime: DVFS=peak, high power budget -> reward LOW skip
   - HOT regime: DVFS=min, tight power cap -> reward HIGH skip + protection
   - Alternates regimes so constant policy CANNOT win
   - Uses DVFS sweeps (safe, no overheating)

FIX B: STRENGTHEN CONTRASTIVE SIGNAL
   B1. Quantile-based sampling: stressed/relaxed by ACTUAL power quantiles
       (top/bottom 15% by discomfort), not by disturbance type labels
   B2. Stateless body encoding for contrastive: no state mutation
   B3. Gate pretraining stage: first 100 steps = contrastive ONLY, no RL
       Verify gate responds to sensors BEFORE enabling RL

FIX C: SENSOR RELIANCE REGULARIZER
   - Compute g_real = gate(sensors_real, body)
   - Compute g_shuf = gate(shuffle(sensors), body)
   - L_sense = -|g_real - g_shuf| (maximize difference)
   - Forces gate to actually USE sensor information

FIX D: BODY PERSISTENCE FIXES
   - Slower decay (0.05 instead of 0.1) so history persists longer
   - Stronger body path to gate (separate MLP branch)
   - History-dependent reward: same sensors + different history = different reward

FIX E: PREDICTOR SCALING
   - Predict DELTA power/temp (not absolute) - easier to learn
   - Predict 200ms window average (not instantaneous)
   - Proper unit normalization

VALIDATION TARGETS:
   - counterfactual_swap: gate_diff > 0.01 (was 0.0001)
   - body_state_persistence: gate_diff > 0.005 (was 0.0002)
   - prediction_accuracy: error < 20% (was 95%)

Author: FEEL Research Team
Date: 2026-01-16 (z46 - SENSOR-RESPONSIVE TRAINING)
"""
...
@dataclass
class Z46Config:
    """z46 Sensor-Responsive Embodied Training Configuration."""
    ...
    # z46 FIX D: Body state settings - SLOWER DECAY for persistence
    body_dim: int = 64
    body_decay: float = 0.05  # z46: HALVED from 0.1 for longer memory
    body_noise_std: float = 0.01

    # Reward weights
    quality_weight: float = 0.15
    energy_weight: float = 0.20  # z46: Slightly reduced
    recovery_weight: float = 0.15
    throughput_weight: float = 0.15
    prediction_weight: float = 0.10
    discomfort_weight: float = 0.10
    sensor_reliance_weight: float = 0.15  # z46 FIX C: NEW - reward using sensors

    # z46 FIX A: TWO-REGIME CURRICULUM
    # COOL regime: high power budget, reward low skip (quality)
    cool_power_cap_w: float = 80.0
    cool_j_target: float = 8.0  # Can use more energy for quality
    # HOT regime: tight power cap, reward high skip (efficiency)
    hot_power_cap_w: float = 50.0
    hot_j_target: float = 5.0  # Must be efficient
    # Regime switching
    regime_switch_steps: int = 10  # Switch every N steps
    regime_use_dvfs: bool = True  # Use DVFS to create regimes (safe!)

    # z46: SAFETY TARGETS (conservative!)
    power_cap_w: float = 60.0      # Default soft target
    ...
    j_per_token_target: float = 6.0  # z46: More realistic target

    # z46: Safe stress bounds
    stress_power_min_w: float = 70.0
    stress_power_max_w: float = 110.0
    ...
    # z46 FIX B/C: Contrastive and sensor reliance
    contrastive_coef: float = 0.3  # z46: INCREASED from 0.2
    contrastive_margin: float = 0.05
    sensor_buffer_size: int = 200
    sensor_reliance_coef: float = 0.1  # z46 FIX C: g_real vs g_shuffled loss
    quantile_pct: float = 0.15  # z46 FIX B1: Use top/bottom 15% for contrastive

    # z46 FIX B3: Training phases - EXTENDED GATE PRETRAIN
    gate_pretrain_steps: int = 100     # z46: DOUBLED! No RL, contrastive only
    expected_skip_steps: int = 150     # z46: Extended
    ...
    val_every: int = 50
    checkpoint_dir: str = "models/z46_embodied"

    # Wandb
    wandb_project: str = "feel-z46-embodied"
...
class GateNetWithExpectedSkip(nn.Module):
    """
    z46: Gate network with STRONGER BODY PATH (Fix D) and sensor reliance.
    ...
    def __init__(
        ...
        # z46 FIX D: SEPARATE sensor and body encoders for stronger body path
        self.sensor_encoder = nn.Sequential(
        ...
        # z46: Interaction layer - body modulates sensor processing
    ...
    def forward(
        ...
        # z46 FIX D: Separate encoding paths
        h_sensor = self.sensor_encoder(sensors)
        h_body = self.body_encoder(body_state)

        # z46: Body modulates sensor processing through interaction
...
class RegimeCurriculum:
    """
    z46 FIX A: Two-regime curriculum that FORCES sensor-conditional behavior.
    ...
    def step(self, step_num: int) -> str:
        ...
        if self.steps_in_regime >= switch_interval:
            ...
            # z46: Use DVFS to create regime difference (SAFE!)
...
class RecoveryAwareReward:
    ...
    def compute(
        ...
        regime_targets: Dict = None,  # z46 FIX A: Regime-specific targets
        ...
        # z46 FIX A: Get regime-specific targets
        if regime_targets:
        ...
        # z46: Apply regime quality bonus
        quality = min(1.0, quality + quality_bonus)

        # 2. Energy efficiency (z46: use regime-specific target)
        ...
        # 3. Time-in-band (z46: use regime-specific power cap)
        ...
        # 6. Discomfort (z46: use regime-specific power cap)
        ...
        # z46 FIX A: Skip penalty for regime (COOL regime penalizes skipping)
        ...
        reward = (
            ...
            fast_signal_bonus -
            skip_penalty_value  # z46: Regime-based skip penalty
        )
...
class SensorBuffer:
    """
    z46 FIX B1: Buffer for contrastive learning with QUANTILE-BASED sampling.

    Key change: Sample stressed/relaxed by ACTUAL DISCOMFORT quantiles,
    not by disturbance type labels. Uses top/bottom 15% by discomfort score.
    """

    def __init__(self, max_size: int = 200, quantile_pct: float = 0.15):
        self.max_size = max_size
        self.quantile_pct = quantile_pct
        # Store: (sensors, power_w, temp_c, discomfort_score)
        self.buffer: Deque[Tuple[torch.Tensor, float, float, float]] = deque(maxlen=max_size)

    def _compute_discomfort(self, power_w: float, temp_c: float) -> float:
        """Compute discomfort score for quantile ranking."""
        # Combine power overshoot and thermal stress
        power_stress = max(0, power_w - 50.0) / 50.0  # Normalized power above 50W
        temp_stress = max(0, temp_c - 60.0) / 20.0    # Normalized temp above 60C
        return power_stress + temp_stress

    def add(self, sensors: torch.Tensor, power_w: float, temp_c: float):
        discomfort = self._compute_discomfort(power_w, temp_c)
        self.buffer.append((sensors.detach().clone(), power_w, temp_c, discomfort))

    def can_sample_pair(self) -> bool:
        if len(self.buffer) < 10:  # z46: Need enough for quantiles
            return False
        # z46: Check discomfort spread, not just power
        discomforts = [d for _, _, _, d in self.buffer]
        spread = max(discomforts) - min(discomforts)
        return spread >= 0.3  # Meaningful discomfort difference

    def sample_extremes(self) -> Tuple[Tuple, Tuple]:
        """
        z46 FIX B1: Sample by discomfort QUANTILES, not power quartiles.

        - Stressed = top 15% by discomfort
        - Relaxed = bottom 15% by discomfort
        """
        if len(self.buffer) < 10:
            s1, p1, t1, _ = random.choice(self.buffer)
            s2, p2, t2, _ = random.choice(self.buffer)
            return (s1, p1, t1), (s2, p2, t2)

        # z46: Sort by DISCOMFORT score, not just power
        sorted_buf = sorted(self.buffer, key=lambda x: x[3])  # Sort by discomfort
        n = len(sorted_buf)
    ...
    def get_discomfort_spread(self) -> float:
        """z46: Get discomfort spread for logging."""
        if len(self.buffer) == 0:
            return 0.0
        discomforts = [d for _, _, _, d in self.buffer]
...
# ============================================================================
# z46: CONTRASTIVE GATE LOSS + SENSOR RELIANCE REGULARIZER
# ============================================================================

class NormalizedContrastiveGateLoss:
    """
    z46: Contrastive loss with sensor reliance regularizer.
    ...
    def compute(
        ...
        gate_probs_shuffled: List[torch.Tensor] = None,  # z46 FIX C: For sensor reliance
    ) -> Tuple[torch.Tensor, Dict]:
        ...
        # z46 FIX C: SENSOR RELIANCE REGULARIZER
...
def train_epoch(
    ...
    regime_curriculum: RegimeCurriculum,  # z46 FIX A
    epoch: int,
    global_step: int,
) -> int:
    """Train one epoch with z46 SENSOR-RESPONSIVE TRAINING."""
    ...
    for prompt_idx, prompt in enumerate(prompts):
        ...
        for sample_idx in range(config.num_samples):
            ...
            # z46: Reward with recovery tracking AND regime targets
            reward, advantage, breakdown = reward_computer.compute(
                ...
                regime_targets=regime_targets,  # z46 FIX A: Pass regime-specific targets
            )
        ...
        if do_contrastive:
            ...
            # z46 FIX B2: Use STATELESS encoding for contrastive (no state mutation)
            ...
            # z46 FIX C: SENSOR RELIANCE - compute gate with SHUFFLED sensors
            ...
            contrastive_loss, contrastive_metrics = contrastive_loss_fn.compute(
                ...
                gate_probs_shuffled=gate_result_shuffled["gate_probs"],  # z46 FIX C
            )
...
def main():
    parser = argparse.ArgumentParser(description="FEEL z46: Sensor-Responsive Embodied Trainer")
    parser.add_argument("--epochs", type=int, default=3)
    parser.add_argument("--max-prompts", type=int, default=500)
    parser.add_argument("--checkpoint-dir", type=str, default="models/z46_embodied")
    ...
    parser.add_argument("--contrastive-coef", type=float, default=0.3)  # z46: Increased
    parser.add_argument("--wandb-project", type=str, default="feel-z46-embodied")
    ...
    if WANDB_AVAILABLE and config.use_wandb:
        ...
        run_name = config.wandb_run_name or f"z46_regime_{hostname}"
        wandb.init(
            project=config.wandb_project,
            name=run_name,
            config=asdict(config),
            tags=["z46", "regime-curriculum", "sensor-responsive", hostname],
        )

    print("=" * 70)
    print("FEEL z46: SENSOR-RESPONSIVE EMBODIED TRAINER")
    print("=" * 70)
    print("z46 KEY FIXES (from z45 constant-skip problem):")
    print("  FIX A: TWO-REGIME CURRICULUM - COOL/HOT alternation")
    print("         COOL: DVFS=peak, high power cap, reward quality (low skip)")
    print("         HOT:  DVFS=min, tight cap, reward efficiency (high skip)")
    print("         -> Constant policy CANNOT win!")
    print("  FIX B: STRONGER CONTRASTIVE")
    print("         B1: Quantile sampling (top/bottom 15% by discomfort)")
    print("         B2: Stateless body encoding")
    print("         B3: Gate pretrain 100 steps (no RL)")
    print("  FIX C: SENSOR RELIANCE REGULARIZER - |g_real - g_shuffled|")
    print("  FIX D: BODY PERSISTENCE - 0.05 decay (was 0.1)")
    print("  FIX E: PREDICTOR DELTAS - predict change not absolute")
    print("=" * 70)
    ...
    # z46: Load checkpoint if resuming
    global_step = 0
    if args.resume_from and Path(args.resume_from).exists():
        ...
        print(f"  NOTE: z46 continues training with NEW curriculum from this point")
    ...
    for epoch in range(config.epochs):
        ...
        global_step = train_epoch(
            ...
            regime_curriculum=regime_curriculum,  # z46 FIX A
        )
