#!/usr/bin/env python3
"""
FEEL v7.10: Complete Fixes

NEW FIXES from v7.9:
1. Two-token prompt description (was "ONE token", now correctly says TWO)
2. Default output dir updated to v7.10
3. Consistent intensity binning (both directions use NUM_INTENSITY_LEVELS-1)
4. Complete grad norm logging (all trainable params, not just injector)
5. Gated direct intensity signal (only when group variance is tiny)
6. W&B confusion matrix + intensity histograms for diagnostics

INHERITED FIXES:
- FIX #1: Group-relative advantage for BOTH tokens (from v7.8)
- FIX #2: Smoothed derivatives 1s window (from v7.8)
- FIX #3: Proper swap test (from v7.8)
- FIX #4: Direct intensity signal on uniform data (from v7.9, now gated)
...
- Gated direct intensity gradient (only uniform data regime)
...
def intensity_to_token_idx(intensity: float) -> int:
    """Convert 0-1 intensity to token index 0-63. Consistent with token_idx_to_intensity."""
    intensity = max(0.0, min(1.0, float(intensity)))
    return int(round(intensity * (NUM_INTENSITY_LEVELS - 1)))

def token_idx_to_intensity(idx: int) -> float:
    """Convert token index 0-63 to 0-1 intensity. Consistent with intensity_to_token_idx."""
...
class DeepTelemetry:
    """Auto-detecting telemetry with variance tracking and smoothed derivatives."""

    def __init__(self, poll_interval: float = 0.02, derivative_window: float = 1.0):
        self.poll_interval = poll_interval
        self.derivative_window = derivative_window  # Seconds for derivative smoothing
        ...
        # For smoothed derivatives (FIX #2: use 1s window instead of instant)
        self._derivative_history: Deque[Tuple[float, float, float]] = deque(maxlen=100)  # (timestamp, temp, power)
    ...
    def _compute_smoothed_derivatives(self, state: DeepGPUState) -> Tuple[float, float]:
        """FIX #2: Compute derivatives over 1s window instead of instant (noisy) rates."""
        now = state.timestamp
        self._derivative_history.append((now, state.temp_edge, state.power_socket))

        # Find state from ~derivative_window seconds ago
        ...
        if old_entry is not None:
            dt = now - old_entry[0]
            if dt > 0.1:  # At least 100ms window
    ...
    def _poll_loop(self):
        while self._running:
            try:
                ...
                with self._lock:
                    # FIX #2: Use smoothed derivatives over 1s window
                    temp_rate, power_rate = self._compute_smoothed_derivatives(state)
...
class EpisodeLogger:
    ...
    def run_swap_test(self) -> Dict:
        """FIX #3: Proper swap test - shuffle correct_actions and measure accuracy drop.

        If action depends on z_feel causally, then shuffling the pairing between
        actions taken and correct_actions should drop accuracy to ~20% (random among 5 classes).
        A large drop (real_acc - shuffled_acc) indicates causal dependence.
        """
        ...
        # FIX #3: Use single permutation and compare action to shuffled correct_action
        correct_actions = [e.correct_action_id for e in self.episodes]
        perm = np.random.permutation(n)

        # For each episode i, check if its action matches the correct_action of episode perm[i]
        ...
        # Expected shuffled accuracy if no causal relationship: ~20% (1/5 classes)
        # Large drop indicates the model learned to depend on z_feel
        return {
            "real_accuracy": real_acc,
            "shuffled_accuracy": shuffled_acc,
            "accuracy_drop": real_acc - shuffled_acc,
            "expected_random": 0.2,  # 1/5 classes
            "n_episodes": n,
        }
...
class ValidatedTrainer:
    ...
    def format_prompt(self, question: str) -> str:
        # FIX: Two-token description (was "ONE token")
        sys = (
    ...
    def step(self, problems: List[Dict]) -> Dict:
        ...
        # Track std per trajectory for gated direct signal
        for g in groups:
            ...
            for t in g:
                ...
                t._group_std = raw_std  # Store for gated direct signal
        ...
        for g in groups:
            for t in g:
                # FIX #1: Use GROUP-RELATIVE advantage for BOTH tokens (not raw rewards!)
                # This is the key GRPO insight: relative within group, not absolute rewards
                adv = torch.tensor(float(np.clip(t.advantage, -2.0, 2.0)), device=self.device, dtype=torch.float32)
                ...
                # Class loss with entropy (using group-relative advantage)
                if ent_coef > 0:
                    action_probs = action_logp.exp()
                    class_entropy = -(action_probs * action_logp).sum(dim=-1).mean()
                    class_loss = -(adv * class_logp) - ent_coef * class_entropy
                else:
                    class_loss = -(adv * class_logp)
                ...
                # FIX #4 (GATED): Intensity loss with DIRECT error signal
                # Only apply when group variance is tiny (uniform data regime)
                ...
                # Gate: only add direct signal when group is uniform
                uniform_group = getattr(t, '_group_std', 0.1) <= (self.cfg.min_advantage_std + 1e-6)
                if uniform_group:
                    # Direct signal: reward accurate predictions, penalize errors
                    direct_intensity_signal = 1.0 - 2.0 * intensity_error
                    intensity_adv = adv + 0.3 * torch.tensor(direct_intensity_signal, device=self.device, dtype=torch.float32)
                else:
                    # Normal GRPO: just use group-relative advantage
                    intensity_adv = adv
        ...
        if n > 0:
            ...
            if not torch.isfinite(loss_mean):
                ...
                loss_mean.backward()
                # FIX: Compute grad norm over ALL trainable params (not just injector)
        ...
        # Class metrics (for confusion matrix)
        ...
        return {
            ...
            # For W&B confusion matrix + histograms
    ...
    def train(self, out_dir: str):
        ...
        print("\n" + "=" * 70)
        print("  FEEL v7.10: Complete Fixes")
        print("  - Two-token prompt (class + intensity)")
        print("  - Consistent intensity binning")
        print("  - Complete grad norm (all trainables)")
        print("  - Gated direct signal (uniform data only)")
        print("  - W&B confusion matrix + histograms")
        print("=" * 70 + "\n")
            ...
            for epoch in range(self.cfg.num_epochs):
                ...
                for step in range(self.cfg.steps_per_epoch):
                    ...
                    if self.wandb_run is not None:
                        ...
                        # Log confusion matrix + histograms every 10 steps
...
def main():
    ...
    ap.add_argument("--output", default="results/grpo_v7_10_complete")
