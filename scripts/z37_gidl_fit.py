"""z37_gidl_fit.py — DE fit with series M1+M2 + GIDL body charging.

Adds GIDL term (band-band tunneling at M2's drain → body) to the body
KCL. This provides body charging at low VG1 where the channel current is
too small to drive impact ionization. Diagnostic from z33-z36: model
under-predicts VG1=0.2 + high VG2 by 2-6 decades because body never
latches without channel-current-driven Iii. GIDL fixes that.
...
Fit parameters (10):
  ...
  K2            ∈ [-0.20, 0.10]
  log10 ALPHA0_2∈ [-2, 1]
  log10 AGIDL   ∈ [-12, -3]   (NEW — GIDL pre-factor)
  BGIDL_x       ∈ [0.5, 5.0]  (NEW — BGIDL/2.4e9 multiplier; lower = stronger)
"""
...
def build(x):
    """x = [VTH0, log10_AREA, log10_alpha, log10_Rb, U0, RDSW, K2,
            log10_alpha2, log10_AGIDL, BGIDL_x]"""
    vth0, lga, lgam, lgrb, u0, rdsw, k2, lgam2, lg_agidl, bgidl_x = x
    return replace(make_pazos_130nm(),
                    ...
                    RDSW=float(rdsw),
                    K2=float(k2),
                    AGIDL=float(10**lg_agidl),
                    BGIDL=float(2.4e9 * bgidl_x))
...
BOUNDS = [
    ...
    (50.0, 500.0),    # RDSW
    (-0.20, 0.10),    # K2
    (-2.0, 1.0),      # log10 ALPHA0_2
    (-12.0, -3.0),    # log10 AGIDL
    (0.5, 5.0),       # BGIDL multiplier (1.0 = canonical 2.4e9 V/m)
]


def main():
    print(f"DE series M1+M2 + GIDL fit (10 params)  N_DS={N_DS}")
    t0 = time.time(); ctr = [0]
    def cb(xk, conv):
        ctr[0] += 1
        o = objective(xk)
        print(f"  iter {ctr[0]:3d} obj={o:.3f}  VTH0={xk[0]:.3f} "
              f"AREA=1e{xk[1]:.1f} A0=1e{xk[2]+xk[7]:.1f} Rb=1e{xk[3]:.1f} "
              f"U0={xk[4]:.3f} RDSW={xk[5]:.0f} K2={xk[6]:+.3f} "
              f"AGIDL=1e{xk[8]:.1f} BGx={xk[9]:.2f}  "
    ...
    res = differential_evolution(
        ...
        seed=73, tol=1e-3, workers=-1, updating="deferred",
    ...
    print(f"  VTH0={xo[0]:.3f}  BJT_AREA={10**xo[1]:.2e}  "
          f"ALPHA0_eff={10**(xo[2]+xo[7]):.2e}  Rb={10**xo[3]:.2e}")
    print(f"  U0={xo[4]:.3f}  RDSW={xo[5]:.0f}  K2={xo[6]:+.3f}")
    print(f"  AGIDL={10**xo[8]:.2e}  BGIDL={2.4e9*xo[9]:.2e}")
    ...
    with open(OUT / "summary.json", "w") as f:
        json.dump({
            ...
            "ALPHA0_mult": float(10**(xo[2]+xo[7])), "Rb": float(10**xo[3]),
            "U0": float(xo[4]), "RDSW": float(xo[5]), "K2": float(xo[6]),
            "AGIDL": float(10**xo[8]), "BGIDL": float(2.4e9 * xo[9]),
            "median_log_rmse": float(np.median(rs)),
            "worst_log_rmse": float(np.max(rs)),
            "group_stats": group_stats, "per_curve": per_curve,
            "topology": "series_M1_M2 + GIDL",
        }, f, indent=2)
    ...
    fig.suptitle(f"Series M1+M2 + GIDL fit  ·  "
                  f"overall median {np.median(rs):.2f} dec  ·  {len(rs)} curves\n"
                  f"VTH0={xo[0]:.3f} K2={xo[6]:+.3f} AGIDL={10**xo[8]:.1e} "
                  f"BGIDL={2.4e9*xo[9]:.1e}", fontsize=13)
