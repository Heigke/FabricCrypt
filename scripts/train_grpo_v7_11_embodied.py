#!/usr/bin/env python3
"""
FEEL v7.11: Embodied Learning with Decoupled Advantages

KEY CHANGES from v7.10:
1. Balanced Swap Test - subsample to 50/50 class balance for fairer causality test
2. Decoupled Advantages - Class: GRPO group-relative, Intensity: per-sample direct
3. Ungated Direct Signal - always apply intensity learning (gating was too strict)
4. Semi-Supervised Anchor - Cross-entropy loss on true intensity bin
5. Ablation Test - compare Real z_feel vs Zero z_feel accuracy

INHERITED:
- Two-token protocol (class + intensity)
- Consistent intensity binning
- Complete grad norm logging
- W&B confusion matrix + histograms

GPU Signal Findings:
- SCLK, power, GPU% DO vary with reasoning type (~2-3% cross-task variance)
- Simple tasks → Higher SCLK (~1894 MHz), Higher GPU% (~53%)
- Complex tasks → Lower SCLK (~1780 MHz), Lower GPU% (~48%)
- Signal is subtle but consistent - model needs to learn these patterns
"""
...
def compute_stress_level(
    ...
    thermal = min(1.0, max(0.0, (temp_c - 35) / 55))
    power = min(1.0, max(0.0, power_w / 180))
    gfx = gfx_activity / 100.0
    mem = mem_activity / 100.0
    vram = vram_pct / 100.0

    heating_fast = temp_rate > 2.5
    power_spiking = power_rate > 40
    hotspot_danger = hotspot_delta > 18
...
INTENSITY_INIT_WORDS = {
    "low": ["low", "minimal", "slight", "light", "easy"],
    "medium": ["moderate", "medium", "normal", "steady"],
    "high": ["high", "elevated", "intense", "strong"],
    "extreme": ["extreme", "maximum", "peak", "severe"],
}
...
class DeepGPUState:
    ...
    # v7.11: New signals that differentiate reasoning patterns
    vcn_activity: float = 0.0  # Video codec activity (variance ratio: 0.043)
    mm_activity: float = 0.0   # Multimedia activity (variance ratio: 0.038)
    sclk_rate: float = 0.0     # Shader clock rate of change (highly variable during reasoning)

    def to_z_feel(self, z_dim: int = 32, device="cuda", dtype=torch.bfloat16) -> torch.Tensor:
        """v7.11: Updated z_feel based on deep signal analysis.

        Top differentiating signals (by cross-task variance ratio):
        - sclk: 0.125 (highest! varies 2000-2800 between task types)
        - gfx_activity: 0.111
        - umc_activity: 0.080
        - temp_gfx/temp_soc: 0.066/0.063
        - vcn_activity: 0.043
        - mm_activity: 0.038
        """
        z = torch.zeros(z_dim, device=device, dtype=dtype)

        # TEMPS (0-5) - differentiate thermal patterns
        temp_norm = min(1.0, max(0.0, (self.temp_edge - 30) / 70))
        hotspot_norm = min(1.0, max(0.0, (self.temp_hotspot - 30) / 80))
        z[0] = temp_norm
        z[1] = hotspot_norm
        z[2] = max(0.0, min(1.0, (self.temp_hotspot - self.temp_edge) / 20))
        z[3] = max(-1.0, min(1.0, self.temp_rate / 5.0))
        z[4] = 1.0 if self.temp_edge >= 62 else 0.0
        z[5] = 1.0 if self.temp_edge >= 75 else 0.0

        # POWER (6-9)
        power_norm = min(1.0, max(0.0, self.power_socket / 180))
        z[6] = power_norm
        z[7] = power_norm ** 2
        z[8] = max(-1.0, min(1.0, self.power_rate / 50.0))
        z[9] = 1.0 if self.power_socket > 140 else 0.0

        # ACTIVITY (10-16) - KEY DIFFERENTIATING SIGNALS
        # gfx_activity: variance ratio 0.111 (2nd highest)
        z[10] = self.gfx_activity / 100.0
        z[11] = (self.gfx_activity / 100.0) ** 2  # Squared for nonlinearity

        # umc_activity: variance ratio 0.080 (3rd highest) - memory controller
        z[12] = self.umc_activity / 100.0
        z[13] = self.mem_activity / 100.0

        # vcn_activity: variance ratio 0.043 (video codec - interesting signal!)
        z[14] = self.vcn_activity / 100.0

        # mm_activity: variance ratio 0.038 (multimedia)
        z[15] = self.mm_activity / 100.0

        # Combined activity signals
        z[16] = (self.gfx_activity + self.umc_activity + self.vcn_activity) / 300.0

        # CLOCKS (17-22) - HIGHEST DIFFERENTIATING SIGNALS
        # sclk: variance ratio 0.125 (HIGHEST!) - varies 2000-2800 between reasoning/code tasks
        sclk_norm = min(1.0, max(0.0, (self.sclk - 500) / 2500))  # 500-3000 MHz range
        z[17] = sclk_norm
        z[18] = sclk_norm ** 2  # Squared for emphasis on high values
        z[19] = max(-1.0, min(1.0, self.sclk_rate / 500.0))  # Rate of change

        # Other clocks (lower variance but still useful)
        z[20] = min(1.0, self.mclk / 1000)
        z[21] = min(1.0, self.socclk / 1500)
        z[22] = min(1.0, self.fclk / 2000)
        ...
        z[25] = gtt_pct
        z[26] = 1.0 if self.gfx_activity > 80 else 0.0

        # PCIE (27-28)
        z[27] = min(1.0, self.pcie_speed_gt / 32.0)
        z[28] = self.pcie_width / 16.0

        # COMPOSITE REASONING SIGNALS (29-31) - combining top differentiators
        # These should capture the "reasoning vs fast generation" pattern
        thermal = (temp_norm + hotspot_norm) / 2
        compute = (z[10] + power_norm) / 2
        clock_pattern = sclk_norm  # Higher during fast generation, lower during reasoning

        z[29] = (thermal + compute) / 2
        z[30] = clock_pattern  # Direct sclk pattern
        z[31] = (compute + clock_pattern) / 2  # Compute-clock correlation
...
class DeepTelemetry:
    def __init__(self, poll_interval: float = 0.02, derivative_window: float = 1.0):
        ...
        self._derivative_history: Deque[Tuple[float, float, float]] = deque(maxlen=100)
    ...
    def _parse_gpu_metrics(self) -> Dict:
        """Parse AMD gpu_metrics binary blob for all signals.

        v7.11: Now extracts vcn_activity, mm_activity which showed
        variance ratios of 0.043 and 0.038 respectively.
        """
        result = {}
        ...
        try:
            ...
            # Activity metrics - try to get all of them
            if len(data) > 0x50:
                acts = struct.unpack_from('<8H', data, 0x40)
                # Parse individual activity values
                if 0 <= acts[0] <= 10000:
                    result['gfx_activity'] = float(acts[0]) / 100.0 if acts[0] > 100 else float(acts[0])
                if 0 <= acts[1] <= 10000:
                    result['mem_activity'] = float(acts[1]) / 100.0 if acts[1] > 100 else float(acts[1])
                # UMC (memory controller) activity
                if len(acts) > 2 and 0 <= acts[2] <= 10000:
                    result['umc_activity'] = float(acts[2]) / 100.0 if acts[2] > 100 else float(acts[2])
                # VCN (video codec) activity - key differentiating signal!
                if len(acts) > 4 and 0 <= acts[4] <= 10000:
                    result['vcn_activity'] = float(acts[4]) / 100.0 if acts[4] > 100 else float(acts[4])
                # MM (multimedia) activity
                if len(acts) > 5 and 0 <= acts[5] <= 10000:
                    result['mm_activity'] = float(acts[5]) / 100.0 if acts[5] > 100 else float(acts[5])
            ...
            # sclk (shader clock) - HIGHEST differentiating signal (0.125 variance ratio)
            if len(data) > 0x62:
    ...
    def _compute_smoothed_derivatives(self, state: DeepGPUState) -> Tuple[float, float, float]:
        """Compute smoothed derivatives for temp_rate, power_rate, and sclk_rate."""
        now = state.timestamp
        ...
        if old_entry is not None and len(old_entry) >= 4:
            dt = now - old_entry[0]
            if dt > 0.1:
                temp_rate = (state.temp_edge - old_entry[1]) / dt
                power_rate = (state.power_socket - old_entry[2]) / dt
                sclk_rate = (state.sclk - old_entry[3]) / dt
                temp_rate = max(-10.0, min(10.0, temp_rate))
                power_rate = max(-100.0, min(100.0, power_rate))
                sclk_rate = max(-1000.0, min(1000.0, sclk_rate))  # MHz/s
                return temp_rate, power_rate, sclk_rate
    ...
    def _poll_loop(self):
        while self._running:
            try:
                ...
                with self._lock:
                    temp_rate, power_rate, sclk_rate = self._compute_smoothed_derivatives(state)
                    state.temp_rate = temp_rate
                    state.power_rate = power_rate
                    state.sclk_rate = sclk_rate  # v7.11: Track sclk rate of change
...
class EpisodeLogger:
    ...
    def run_swap_test(self) -> Dict:
        """v7.11: BALANCED swap test - subsample to 50/50 class balance.

        This gives a fairer causality test by ensuring equal representation
        of correct and incorrect predictions before shuffling.
        """
        if len(self.episodes) < 20:
            return {"error": "not enough episodes"}

        # Separate episodes by correctness
        correct_eps = [e for e in self.episodes if e.is_correct]
        wrong_eps = [e for e in self.episodes if not e.is_correct]

        # Balance: subsample to min(len(correct), len(wrong))
        n_each = min(len(correct_eps), len(wrong_eps))
        if n_each < 5:
            # Not enough of both classes - fall back to standard test
            return self._standard_swap_test()

        # Random subsample for balance
        balanced_eps = (
            random.sample(correct_eps, n_each) +
            random.sample(wrong_eps, n_each)
        )
        random.shuffle(balanced_eps)
        n = len(balanced_eps)

        # Real pairing accuracy (should be ~50% by construction)
        real_correct = sum(1 for e in balanced_eps if e.is_correct)
        real_acc = real_correct / n

        # Shuffle correct_actions and measure accuracy drop
        correct_actions = [e.correct_action_id for e in balanced_eps]
        perm = np.random.permutation(n)

        shuffled_matches = sum(
            1 for i in range(n)
            if balanced_eps[i].action_id == correct_actions[perm[i]]
        )
        shuffled_acc = shuffled_matches / n
    ...
    def _standard_swap_test(self) -> Dict:
        """Standard (unbalanced) swap test as fallback."""
        n = len(self.episodes)
        ...
        shuffled_matches = sum(
            1 for i in range(n)
            if self.episodes[i].action_id == correct_actions[perm[i]]
        )
        shuffled_acc = shuffled_matches / n
...
class GRPOConfig:
    ...
    action_penalty_wrong: float = 1.5
    ...
    # v7.11: Semi-supervised intensity anchor
    intensity_ce_coef: float = 0.1  # Cross-entropy supervision weight
...
class ValidatedTrainer:
    def __init__(self, cfg: GRPOConfig):
        ...
        print(f"  v7.11: Decoupled advantages, ungated direct signal, CE anchor={cfg.intensity_ce_coef}")
        self.wandb_run = None
        if cfg.wandb:
            if wandb is None:
    ...
    def format_prompt(self, question: str) -> str:
        sys = (
            ...
            "1) one class token: " + ", ".join([ACTION_TOKENS[a] for a in FeelAction]) + "\n"
            "2) one intensity token: <|FEEL_I00|> ... <|FEEL_I63|> (00=lowest, 63=highest)\n"
            "After the two tokens, solve the user's problem normally.\n"
        )
    ...
    def generate(self, question: str, zero_z_feel: bool = False) -> Trajectory:
        """Generate trajectory. If zero_z_feel=True, use zeros instead of real z_feel (for ablation)."""
        ...
        if zero_z_feel:
            z_feel = torch.zeros(self.cfg.z_feel_dim, device=self.device, dtype=self.dtype)
        else:
        ...
        if chosen_action is None:
            chosen_action = FeelAction.OK

        if self.episode_logger and not zero_z_feel:
    ...
    def reward(self, traj: Trajectory, gt: str) -> float:
        ...
        # INTENSITY REWARD
        true_intensity = traj.gpu_state.intensity
        pred_intensity = token_idx_to_intensity(traj.intensity_idx)
    ...
    def step(self, problems: List[Dict]) -> Dict:
        ...
        for p in problems:
            ...
            for _ in range(self.cfg.group_size):
                try:
                    ...
                    print(f"  Warning: Trajectory error: {e}")
            if group:
                groups.append(group)
        ...
        # Compute group-relative advantages for CLASS (GRPO style)
        for g in groups:
            rs = [t.reward for t in g]
            mean = sum(rs) / len(rs)
            raw_std = (sum((x-mean)**2 for x in rs)/len(rs))**0.5
            std = max(self.cfg.min_advantage_std, raw_std)
            for t in g:
                t.advantage = (t.reward - mean) / std

        # GRPO update with DECOUPLED advantages (v7.11)
        ...
        for g in groups:
            for t in g:
                # CLASS: use group-relative advantage (GRPO)
                class_adv = torch.tensor(float(np.clip(t.advantage, -2.0, 2.0)), device=self.device, dtype=torch.float32)
                ...
                if self.cfg.force_action_first:
                    logits = logits.masked_fill(self.action_mask == 0, float('-inf'))
                action_logits = logits.index_select(-1, self.action_id_tensor)
                ...
                intensity_logits = logits2.index_select(-1, self.intensity_id_tensor)
                ...
                # v7.11: DECOUPLED intensity advantage - per-sample direct signal (UNGATED)
                target_intensity_norm = t.gpu_state.correct_intensity_idx / (NUM_INTENSITY_LEVELS - 1)
                pred_intensity_norm = t.intensity_idx / (NUM_INTENSITY_LEVELS - 1)
                intensity_error = abs(target_intensity_norm - pred_intensity_norm)
                direct_intensity_signal = 1.0 - 2.0 * intensity_error  # -1 to +1
                intensity_adv = torch.tensor(direct_intensity_signal, device=self.device, dtype=torch.float32)
                intensity_adv = torch.clamp(intensity_adv, -2.0, 2.0)
                ...
                if ent_coef > 0:
                    ...
                    intensity_pg_loss = -(intensity_adv * chosen_intensity_logp) - ent_coef * intensity_entropy
                else:
                    intensity_pg_loss = -(intensity_adv * chosen_intensity_logp)

                # v7.11: Semi-supervised anchor - CE loss on true intensity
                target_idx = torch.tensor([t.gpu_state.correct_intensity_idx], device=self.device, dtype=torch.long)
                intensity_ce_loss = F.cross_entropy(intensity_logits, target_idx)

                # Combined intensity loss
                intensity_loss = intensity_pg_loss + self.cfg.intensity_ce_coef * intensity_ce_loss
        ...
        if n > 0:
            ...
            if not torch.isfinite(loss_mean):
                print('  Warning: loss is non-finite; skipping optimizer step')
                self.optimizer.zero_grad(set_to_none=True)
                skip_update = True
            else:
                loss_mean.backward()
                trainables = list(self.injector.parameters()) + self._trainable_params
                gn2 = 0.0
                for p in trainables:
                    if p.grad is not None:
                        gn2 += (p.grad.detach().float().norm() ** 2).item()
                grad_norm = gn2 ** 0.5
        ...
        class_pred = [t.action.value for t in all_trajs]
        class_true = [t.gpu_state.correct_action.value for t in all_trajs]
        ...
        intensity_mae = sum(intensity_errors) / len(intensity_errors)
        intensity_exact = sum(1 for p, t in zip(intensity_pred, intensity_true) if p == t)
        intensity_close = sum(1 for p, t in zip(intensity_pred, intensity_true) if abs(p - t) <= 2)
    ...
    def run_ablation_test(self, n_samples: int = 20) -> Dict:
        """v7.11: Ablation test - compare real z_feel vs zero z_feel."""
        real_correct = 0
        zero_correct = 0

        problems = self.dataset.sample(n_samples)

        for p in problems:
            # Real z_feel
            try:
                traj_real = self.generate(p["question"], zero_z_feel=False)
                if traj_real.action == traj_real.gpu_state.correct_action:
                    real_correct += 1
            except:
                pass
            ...
            # Zero z_feel
            try:
                traj_zero = self.generate(p["question"], zero_z_feel=True)
                if traj_zero.action == traj_zero.gpu_state.correct_action:
                    zero_correct += 1
            except:
                pass

        return {
            "real_accuracy": real_correct / n_samples,
            "zero_accuracy": zero_correct / n_samples,
            "ablation_gap": (real_correct - zero_correct) / n_samples,
            "n_samples": n_samples,
        }
    ...
    def validate(self) -> Dict:
        ...
        if self.episode_logger and len(self.episode_logger.episodes) >= 20:
            ...
            balanced_str = " (balanced)" if swap.get("balanced", False) else ""
            print(f"  Swap Test{balanced_str}: real={swap['real_accuracy']:.1%} shuffled={swap['shuffled_accuracy']:.1%} drop={swap['accuracy_drop']:.1%}")

        # Ablation test
        ablation = self.run_ablation_test(n_samples=16)
        results["ablation"] = ablation
        print(f"  Ablation: real={ablation['real_accuracy']:.1%} zero={ablation['zero_accuracy']:.1%} gap={ablation['ablation_gap']:.1%}")
    ...
    def train(self, out_dir: str):
        ...
        print("\n" + "=" * 70)
        print("  FEEL v7.11: Embodied Learning with Decoupled Advantages")
        print("  - Balanced swap test (50/50 class balance)")
        print("  - Decoupled: Class=GRPO, Intensity=Direct per-sample")
        print("  - Ungated direct signal (always learning)")
        print("  - Semi-supervised CE anchor")
        print("  - Ablation test (Real vs Zero z_feel)")
        print("=" * 70 + "\n")
            ...
            for epoch in range(self.cfg.num_epochs):
                ...
                for step in range(self.cfg.steps_per_epoch):
                    ...
                    print(f"  Step {step+1:2d}: Class={m['act_corr']:.0%} IntMAE={m['intensity_mae']:.3f} Int±2={m['intensity_close']:.0%} "
                          f"T={m['temp']:.0f}C grad={m['grad']:.2f} | {stress_str}")
                    if self.wandb_run is not None:
                        ...
                        if self.global_step % 10 == 0:
                            class_names = [a.name for a in FeelAction]
                            log_data["cm_class"] = wandb.plot.confusion_matrix(
                                y_true=m['class_true'], preds=m['class_pred'],
                                class_names=class_names,
                            )
                            log_data["hist_intensity_true"] = wandb.Histogram(m['intensity_true'])
                            log_data["hist_intensity_pred"] = wandb.Histogram(m['intensity_pred'])
                        wandb.log(log_data)
...
def main():
    ...
    ap.add_argument("--output", default="results/grpo_v7_11_embodied")
    ...
    ap.add_argument("--intensity-ce-coef", type=float, default=0.1, help="CE supervision for intensity")
    ...
    ap.add_argument("--intensity-weight", type=float, default=1.0)
    args = ap.parse_args()
    ...
    cfg = GRPOConfig(
        ...
        intensity_ce_coef=args.intensity_ce_coef,
        wandb=args.wandb,
