#!/usr/bin/env python3
"""
z2094: Dual-LoRA Regime-Bound Embodied LM
==========================================
Key changes from z2093v6 (14/18 PASS):
  - MULTIPLICATIVE body-LoRA coupling: lora_out * body_scale (not additive 0.5+bs)
    → zero body ≈ 0.05x LoRA output, forcing genuine embodiment dependence
  - Freq-driven regime gate: gate = sigmoid(W @ [sclk_norm, freq_ratio])
    → direct HW signal, not learned body encoder (fixes T3 gate_sep)
  - gpu_metrics v3.0 channels: DRAM reads/writes + C0 activity (DVFS-correlated)
  - Safe DVFS: torch.cuda.synchronize() before EVERY switch (prevents GPU hang)
  - Falsification-first test design: tests designed to FAIL if architecture cheats

Target fixes:
  T4  (embodiment gap): multiplicative coupling → zero body kills LoRA
  T10 (gaslighting): dual-channel mismatch (delta + freq_est cross-validation)
  T13 (deep scramble): freq_est directly gates LoRA → wrong DVFS = wrong gate = wrong LoRA
  T14 (energy): forced DVFS exploration in phase 2 + gradient through demand
"""
...
class BodyEncoder(nn.Module):
    ...
    def __init__(self, token_dim=TOKEN_DIM):
        ...
        # === KEY FIX: Freq-driven regime gate ===
        # Gate driven DIRECTLY by hardware freq signal, not learned body state
        # sclk_norm + freq_ratio → gate value
        self.freq_gate_proj = nn.Linear(2, 1)
        # Initialize: high freq → gate=1, low freq → gate=0
        with torch.no_grad():
            self.freq_gate_proj.weight.fill_(3.0)  # both sclk and freq_ratio → positive
            self.freq_gate_proj.bias.fill_(-3.0)   # bias negative → low freq → gate≈0
    ...
    def forward(self, sensor_dict):
        ...
        # Presence-gated: count non-zero tokens → modulate body_scale
        ...
        # === Freq-driven regime gate ===
        # Uses raw freq signal: sclk_norm and freq_ratio from sanitised freq
        freq_input = sd['freq'][:, :2]  # [sclk_norm, freq_ratio]
        freq_gate = torch.sigmoid(self.freq_gate_proj(freq_input))  # [B, 1]
...
def train_lm_epoch(model, train_data, optimizer, epoch, kargs_a, tokenizer,
    ...
    for i in indices[:100]:  # max 100 batches per epoch
        ...
        # Phase 2: continuous target from sclk (sigmoid-mapped to 0-1)
        if use_continuous_loss:
            # sclk 600→0.0, 2900→1.0, continuous
            sclk_frac = min(max((sd['sclk_mhz'] - 600.0) / 2300.0, 0.0), 1.0)
        ...
        # Threshold 0.35 (not 0.5) because gfx1151 high DVFS is ~1361MHz/3000=0.45
...
def main():
    ...
    print("=" * 60)
    print("z2094: Dual-LoRA Regime-Bound Embodied LM v7 (focused thermal + rank8 LoRA)")
    print("GPT-2 small + Multiplicative Body + Freq-Gate + Continuous DVFS")
    print("=" * 60)
    ...
    print(f"  KEY CHANGES (v2):")
    print(f"    - Phase 2: CONTINUOUS gate-weighted loss at auto DVFS (600-2900MHz)")
    print(f"    - loss = (1-gate)*CE_next + gate*CE_skip  (no binary lookup table)")
    print(f"    - T19: Software Oracle / Zombie Test (Schneider 2019)")
    print(f"    - T20: OOD Frequency Generalization (eval at unseen continuous freq)")
    ...
    print(f"\n{'='*60}")
    print(f"z2094 Dual-LoRA Regime-Bound Embodied LM: {n_pass}/{n_total_tests} PASS")
    print(f"{'='*60}")

    # Save results
    out_path = '/home/ikaros/Documents/claude_hive/AMD_gfx1151_energy/results/z2094_dual_lora_regime_lm.json'
    final = {
        'experiment': 'z2094_dual_lora_regime_lm',
        'description': 'Dual-LoRA with CONTINUOUS gate-weighted loss, auto DVFS, Software Oracle + OOD Freq tests',
        'key_fixes': {
            'T4_fix': 'multiplicative body_scale (sigmoid(proj-3)) instead of additive (0.5+bs)',
            'T10_fix': 'dual-channel mismatch (delta vs freq cross-validation)',
            'T13_fix': 'freq_gate driven directly by sclk_norm/freq_ratio → wrong DVFS = wrong gate',
            'T14_fix': 'forced DVFS exploration (20% random) in phase 2 + energy penalty',
        },
