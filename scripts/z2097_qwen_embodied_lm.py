#!/usr/bin/env python3
"""
z2097: Qwen2.5-1.5B Embodied LM — Scaled Falsification
========================================================
Scale-up from z2096 (GPT-2 124M → Qwen2.5-1.5B, 12x larger backbone).
5 NEW falsification tests (T21-T25) that CANNOT pass by construction.

Key changes from z2096:
  1. BACKBONE: Qwen2.5-1.5B (1.54B params frozen, 28 layers, 1536 hidden, GQA 12Q/2KV)
     Dramatically lower baseline PPL → regime shifts more statistically significant
  2. LoRA: rank=8 on layers 10-18 (q_proj + v_proj), dual A/B for regime binding
     Standard nn.Linear (no Conv1D workaround needed)
  3. SUBSTRATE: same 15-token body encoder, 7 HW sensing layers, all analog access preserved
  4. NEW TESTS:
     T21: Zombie Twin — replay RECORDED telemetry instead of live. Should FAIL.
     T22: Cross-Substrate — CPU-only forward pass. Should FAIL if embodiment genuine.
     T23: Neural PCI — Lempel-Ziv complexity of perturbation response (clinical consciousness)
     T24: Feedforward Dissociation — static attention twin (tests dynamic vs fixed mixing)
     T25: Unannounced Perturbation — detect background workload without training
  5. All z2096 features retained: calibrated gate, contrastive loss, agreement modulation,
     spatial thermal (32 ADC), CPU PMU, availability mask, gaslighting detection.
"""
...
N_SUBSTRATE_TOKENS = 15  # +3 for thm_spatial_a, thm_spatial_b, cpu_pmu
TOKEN_DIM = 32
...
class BodyEncoder(nn.Module):
    ...
    def __init__(self, token_dim=TOKEN_DIM):
        ...
        # Output heads
        n_all = ANALOG_DIM + ENERGY_DIM + FREQ_DIM + THERMAL_DIM + PM_DEEP_DIM + SMN_RAW_DIM + GPU_METRICS_DIM + THM_SPATIAL_A_DIM + THM_SPATIAL_B_DIM + CPU_PMU_DIM
    ...
    def forward(self, sensor_dict, availability_mask=None):
        ...
        x = torch.stack(tokens, dim=1)  # [B, 15, token_dim]
        ...
        content_flat = x.reshape(B, -1)  # [B, 15*token_dim] — pure sensor content
        ...
        flat = x.reshape(B, -1)  # [B, 15*token_dim]
...
class EmbodiedQwen2(nn.Module):
    ...
    Architecture:
      ...
      - Body Encoder: 15 substrate tokens → transformer self-attention
...
def train_lm_epoch(model, train_data, optimizer, epoch, kargs_a, tokenizer,
    ...
    for i in indices[:100]:  # max 100 batches per epoch
        ...
        # z2096: Availability mask for lite-mode training (15-wide)
        ...
        # In training (lite=True): intrinsic(4), pm_deep(6), smn_raw(7), gpu_metrics(8),
        # thm_spatial_a(9), thm_spatial_b(10), cpu_pmu(11) are zero
...
def run_tests(model, test_data, kargs, baseline_ppl, tokenizer, dvfs_controller=None, kargs_b=None):
    """25-test battery. Tests designed to FALSIFY, not confirm."""
    results = {}
...
def main():
    ...
    print("=" * 60)
    print("z2097: Qwen2.5-1.5B Embodied LM — Scaled Falsification")
    print("Qwen2.5-1.5B + Calibrated Sharp Gate + Contrastive Loss + Agreement Mod")
    print("=" * 60)
    ...
    print(f"  z2097 KEY CHANGES (from z2096):")
    print(f"    1. Backbone: Qwen2.5-1.5B (12x scale-up from GPT-2 small)")
    print(f"    2. LoRA on q_proj + v_proj (nn.Linear, no Conv1D)")
    print(f"    3. Substrate bias at layers 13, 17, 21 (not 5, 6, 7)")
    print(f"    4. 5 NEW falsification tests T21-T25 (cannot pass by construction)")
    print(f"    5. All z2096 features retained: 15 substrate tokens, 7 HW layers")
    ...
    # Run test battery
    print("\n" + "=" * 60)
    print("RUNNING TEST BATTERY (25 tests)")
    print("=" * 60 + "\n")
    ...
    print(f"\n{'='*60}")
    print(f"z2097 Qwen2.5-1.5B Embodied LM: {n_pass}/{n_total_tests} PASS")
    print(f"{'='*60}")

    # Save results
    out_path = '/home/ikaros/Documents/claude_hive/AMD_gfx1151_energy/results/z2097_qwen_embodied_lm.json'
    final = {
        'experiment': 'z2097_qwen_embodied_lm',
        'description': 'Qwen2.5-1.5B backbone (12x scale-up) + 5 new falsification tests T21-T25',
        'key_changes': {
            'backbone': 'Qwen2.5-1.5B (1.54B frozen, 28 layers, 1536 hidden, GQA 12Q/2KV)',
            'lora_target': 'q_proj + v_proj (nn.Linear, no Conv1D)',
            'substrate_bias': 'layers 13, 17, 21 (scaled for 28-layer model)',
            'new_tests': 'T21 Zombie Twin, T22 Cross-Substrate, T23 Neural PCI, T24 FF Dissociation, T25 Unannounced Perturbation',
            'retained': 'all z2096 features: 15 substrate tokens, 7 HW layers, calibrated gate, contrastive, agreement mod',
        },
