...
OUT = ROOT / "results/z88_bsim4_port_fit_p7v10_skipnonconv"
...
PARAM_SPEC = {
    ...
    # impact-ion (HARD bounds, prevent suppression)
...
def forward_curve(values: dict, model: BSIM4Model, cfg: NSRAMCell2TConfig,
                  VG1: float, VG2: float, Vd_seq: torch.Tensor) -> torch.Tensor:
    ...
    with patch_sd(sd_M1, values), patch_sd(sd_M2, values):
        result = forward_2t(cfg, model, bjt, Vd_seq, VG1_t, VG2_t,
                            warm_start=True)
    # Return BOTH Id and convergence mask so loss can skip non-converged biases.
    # Non-converged biases produce nonsense Id values that, if included in loss,
    # generate spurious huge gradients (root cause of stage 3 explosion).
...
# --------------------------------------------------------------------------- #
# Loss: balanced (50% uniform + 50% knee-weighted) log-MSE
# --------------------------------------------------------------------------- #
def balanced_logmse(pred: torch.Tensor, meas: torch.Tensor,
                    mask: torch.Tensor = None) -> torch.Tensor:
    """Balanced log-MSE. If `mask` is given, only mask=1 points contribute
    (used to skip non-converged Newton biases that would produce bogus grad)."""
    log_eps = 1e-15
...
def stage_loss(thetas, model, cfg, curves):
    ...
    for c in curves:
        try:
            ...
            # Newton failure or numerical issue — large loss, skip grad on this point
...
def main():
    ...
    print("\n=== Stage 3: Snapback merged (alpha0+beta0+Bf) ===", flush=True)
    ...
    summary = {
        ...
        "n_curves": len(curves),
        "config": "P7v6 — proper 2T topology (no gamma/Rb/C/PT)",
    }
