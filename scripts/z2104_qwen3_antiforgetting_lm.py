#!/usr/bin/env python3
"""
z2104: Qwen3-8B Anti-Forgetting Embodied LM
=============================================
Built on z2103 (Deep Firmware LM). Major upgrade:
  - Qwen3-8B backbone (8.2B params, 36 layers, 4096 hidden, GQA 32Q/8KV)
  - Broad LoRA: rank=16, α=32 (2r), ALL modules (q,k,v,o,gate,up,down)
  - KL-divergence regularization: penalize drift from frozen base model
  - Conservative LR: 5e-5 with cosine decay

Anti-forgetting strategy (from "LoRA Learns Less and Forgets Less"):
  1. α=2r (constant scaling preserves regularization across ranks)
  2. Target ALL transformer modules (not just q+v)
  3. KL(frozen || tuned) loss term prevents language degradation
  4. Lower LR + weight decay for stability
...
EPOCHS = 17
LORA_RANK = 16
LORA_ALPHA = 32      # z2104: α=2r per "LoRA Learns Less and Forgets Less" — constant scaling
LORA_BLOCKS = range(12, 30)  # z2104: Qwen3-8B layers 12-29 (middle of 36)
# z2104: LoRA on ALL attention+MLP modules: q,k,v,o,gate_proj,up_proj,down_proj
LORA_ALL_MODULES = True
KL_LAMBDA = 0.1  # z2104: KL divergence regularization weight (frozen model → tuned model)
N_EVAL_BATCHES = 30
DVFS_SETTLE_S = 1.5
...
class EmbodiedQwen2(nn.Module):
    """z2104: Qwen3-8B + Body Encoder + Dual LoRA (ALL modules) + KL regularization.

    Architecture:
      - Frozen Qwen3-8B (8.2B params, 36 layers, 4096 hidden, GQA 32Q/8KV)
      - Body Encoder: 17 substrate tokens → transformer self-attention
      - Dual LoRA: A (regime 0) / B (regime 1) on q,k,v,o,gate,up,down
      - KL regularization: penalize drift from frozen base model
      - Multiplicative body_scale: controls LoRA amplitude
      - Substrate bias: injected at layers 18 and 27 (scaled for 36 layers)
      - Hidden modulation: at layer 22
    """
    def __init__(self, backbone_model, body_encoder, lora_blocks=range(12, 30),
                 rank=16, alpha=32):
        super().__init__()
        ...
        # z2104: Install LoRA on ALL attention + MLP modules (per anti-forgetting paper)
        ...
        for layer_idx in lora_blocks:
            ...
            for proj_name in ['q_proj', 'k_proj', 'v_proj', 'o_proj']:
                ...
                self.lora_layers[key] = LoRALinear(
                    getattr(layer.self_attn, proj_name), rank, alpha)
            # MLP modules: gate_proj, up_proj, down_proj
            if LORA_ALL_MODULES:
                for mlp_name in ['gate_proj', 'up_proj', 'down_proj']:
                    key = f'layer{layer_idx}_{mlp_name}'
                    self.lora_layers[key] = LoRALinear(
                        getattr(layer.mlp, mlp_name), rank, alpha)

        # z2104: Substrate injection layers scaled for 36-layer model
        ...
        # Hidden modulation at layer 22 (z2104: scaled from 17)
    ...
    def forward(self, input_ids, sensor_dict, kargs, labels=None,
        ...
        for i, layer in enumerate(self.backbone.model.layers):
            # z2104: Check if this layer has LoRA (keys: layer{i}_q_proj, etc.)
            key_q = f'layer{i}_q_proj'
            key_k = f'layer{i}_k_proj'
            key_v = f'layer{i}_v_proj'
            key_o = f'layer{i}_o_proj'

            if _dbg: print(f"    [FWD] Layer {i}...", end='', flush=True)
            if key_q in self.lora_layers:
                # Custom attention with LoRA on q, k, v, o projections
                residual = hidden_states
                hidden_states = layer.input_layernorm(hidden_states)

                # z2104: LoRA on ALL attention projections
                q = self.lora_layers[key_q](
                    hidden_states, regime_gate=regime_gate, body_scale=body_scale)
                k = self.lora_layers[key_k](
                    hidden_states, regime_gate=regime_gate, body_scale=body_scale)
                ...
                # z2104: LoRA on o_proj
                attn_output = self.lora_layers[key_o](
                    attn_output, regime_gate=regime_gate, body_scale=body_scale)

                hidden_states = residual + attn_output

                # z2104: MLP with LoRA on gate_proj, up_proj, down_proj
                residual = hidden_states
                hidden_states = layer.post_attention_layernorm(hidden_states)
                key_gate = f'layer{i}_gate_proj'
                key_up = f'layer{i}_up_proj'
                key_down = f'layer{i}_down_proj'
                if key_gate in self.lora_layers:
                    # Manual MLP with LoRA on all three projections
                    gate = self.lora_layers[key_gate](
                        hidden_states, regime_gate=regime_gate, body_scale=body_scale)
                    up = self.lora_layers[key_up](
                        hidden_states, regime_gate=regime_gate, body_scale=body_scale)
                    hidden_states = layer.mlp.act_fn(gate) * up
                    hidden_states = self.lora_layers[key_down](
                        hidden_states, regime_gate=regime_gate, body_scale=body_scale)
                else:
            ...
            if not skip_substrate:
                ...
                if i == 18:  # z2104: scaled from 13 for 36-layer model
                    bias = self.substrate_bias_early(body_flat)  # [B, 4096]
                ...
                elif i == 22:  # z2104: scaled from 17
                    mod = torch.sigmoid(self.hidden_modulation(body_flat)).to(hidden_states.dtype)  # [B, 4096]
                    hidden_states = hidden_states * (1.0 + 0.01 * mod.unsqueeze(1) * _wsg)
                elif i == 27:  # z2104: scaled from 21
    ...
    def _run_qwen3_attn(self, attn_module, q, k, v, position_embeddings, thermal_scale=None):
        """Run Qwen3 attention given pre-computed Q, K, V (with LoRA on all projections).

        z2104: Handles GQA (32 query heads, 8 KV heads) and RoPE.
        ...
        # z2104: Qwen3 applies RMSNorm to Q and K per-head before RoPE
        ...
        # z2104: o_proj handled by LoRA caller (not here)
        return attn_output
...
def train_lm_epoch(model, train_data, optimizer, epoch, kargs_a, tokenizer,
    ...
    for i in indices[:100]:  # max 100 batches per epoch
        ...
        # z2104: KL divergence regularization — penalize drift from frozen backbone
        # Compute frozen backbone logits (no LoRA) and KL(tuned || frozen)
        # This prevents catastrophic forgetting by keeping output distribution close to base
        if KL_LAMBDA > 0 and current_regime == 0:  # Only for regime 0 (standard next-token)
            with torch.no_grad():
...
def _generate_tokens(model, prompt_ids, sb_gen, kargs, n_tokens=32,
    ...
    for _ in range(n_tokens):
        ...
        # z2103: freeze_body_state + skip_agreement prevents temporal drift and
        # agreement-penalty collapse during autoregressive generation
...
def main():
    ...
    print("=" * 60)
    print("z2104: Qwen3-8B Anti-Forgetting Embodied LM")
    print("Qwen3-8B + Broad LoRA + KL Regularization + Workspace Bottleneck")
    print("=" * 60)
    print(f"  Backbone: Qwen3-8B (8.2B frozen, 36 layers, 4096 hidden, GQA 32Q/8KV)")
    print(f"  LoRA: rank={LORA_RANK}, α={LORA_ALPHA}, layers={list(LORA_BLOCKS)} (ALL modules: q,k,v,o,gate,up,down)")
    print(f"  KL regularization: λ={KL_LAMBDA} (penalize drift from frozen backbone)")
    ...
    # z2104: Load Qwen3-8B (safe CUDA allocation before ISA kernel)
    ...
    optimizer = torch.optim.AdamW(
        ...
        lr=5e-5, weight_decay=0.01)  # z2104: conservative LR for anti-forgetting
    ...
    print(f"\n{'='*60}")
    print(f"z2104 Qwen3-8B Anti-Forgetting LM: {n_pass}/{n_total_tests} PASS")
    print(f"{'='*60}")

    # Save results
    out_path = '/home/ikaros/Documents/claude_hive/AMD_gfx1151_energy/results/z2104_qwen3_antiforgetting_lm.json'
    final = {
        'experiment': 'z2104_qwen3_antiforgetting_lm',
        'description': 'z2104: Qwen3-8B Anti-Forgetting LM — broad LoRA (rank=16, all modules), KL regularization, conservative LR',
        'key_changes': {
