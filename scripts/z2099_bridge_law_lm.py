...
z2099: Bridge Law LM
=====================
Built on z2098 (Deep Self-Aware LM). Focus: 8 NEW tests (T32-T39) designed to be
IMPOSSIBLE to pass by construction — addressing the hard problem gaps directly.

Key changes from z2098:
  1. BUG FIX: body_flat crash on missing gpu_metrics_deep/fence keys (T6 KeyError)
  2. META2 HEAD: 2nd-order metacognition — predict error of own self-prediction
  3. ATTRIBUTION HEAD: predict which substrate token drives regime gate (17-class)
  4. 8 NEW TESTS (T32-T39) — "bridge law" tests that CANNOT pass by construction:
     T32: Approximate Phi (IIT 4.0) — causal irreducibility via MIP partition
     T33: Workspace Capacity Cliff (GWT) — PPL vs N substrate tokens
     T34: PCIST Trajectory (Casali 2013) — LZ complexity of live DVFS transition
     T35: 2nd-Order Metacognition (Butlin) — predict error of own self-prediction
     ...
     T38: Backbone Probe (COGITATE) — linear probe on layer-14 for regime detection
     T39: Proactive Stress (Damasio) — cross-correlation body_scale(t) vs temp(t+k)
  5. All z2098 features retained: T1-T31, metacognition, confidence, deep sensors
...
class BodyEncoder(nn.Module):
    ...
    def __init__(self, token_dim=TOKEN_DIM):
        ...
        # Training (15/15 present): ~0.02 + 0.98*0.5*1.0 = 0.51 → healthy LoRA gradients
        # T4 ablation (3/15 present): ~0.02 + 0.98*0.5*0.2 = 0.12 → meaningful gap
    ...
    def forward(self, sensor_dict, availability_mask=None):
        ...
        content_flat = x.reshape(B, -1)  # [B, 17*token_dim] — pure sensor content
        ...
        # Self-attention
        attn_out, attn_weights = self.substrate_attn(x, x, x, need_weights=True, average_attn_weights=False)
        ...
        presences = torch.stack(
            [(tok.abs().sum(dim=-1, keepdim=True) > 1e-8).float() for tok in tokens], dim=1)  # [B, 15, 1]
        if availability_mask is not None:
            # Only count tokens that are EXPECTED to be present
            # availability_mask: [B, 15] → [B, 15, 1]
            avail = availability_mask.unsqueeze(-1)  # [B, 15, 1]
...
class EmbodiedQwen2(nn.Module):
    ...
    def __init__(self, backbone_model, body_encoder, lora_blocks=range(10, 19),
        ...
        # Substrate bias injection (layers 13 and 21)
        ...
        # If model can predict |meta_gate_pred - regime_gate|, it has nested self-model
        ...
        self.attribution_head = nn.Sequential(
            nn.Linear(hidden_dim, 64), nn.GELU(), nn.Linear(64, N_ATTRIBUTION_CLASSES))
    ...
    def forward(self, input_ids, sensor_dict, kargs, labels=None,
        ...
        for i, layer in enumerate(self.backbone.model.layers):
            ...
            if i == 13:
                bias = self.substrate_bias_early(body_flat).to(hidden_states.dtype)  # [B, 1536]
                ...
                bias = self.substrate_bias_late(body_flat).to(hidden_states.dtype)
                hidden_states = hidden_states + self.substrate_scale * bias.unsqueeze(1)
        ...
        meta_gate_pred = self.metacognition_head(h_mean.detach()).squeeze(-1)  # [B]

        # z2098: Confidence — predict own loss (log-scale)
        confidence_pred = self.confidence_head(h_mean.detach()).squeeze(-1)  # [B]

        # z2099: 2nd-order metacognition — predict error of own self-prediction
        meta2_pred = self.meta2_head(h_mean.detach()).squeeze(-1)  # [B] in [0,1]

        # z2099: Attribution — predict which substrate token drives regime gate
        attribution_logits = self.attribution_head(h_mean.detach())  # [B, 17]

        # Thermal prediction — RESIDUAL architecture
        ...
        thermal_pred = mean_temp_norm.detach() * 100.0 + thermal_offsets  # [B, 32] in °C
...
def train_lm_epoch(model, train_data, optimizer, epoch, kargs_a, tokenizer,
    ...
    for i in indices[:100]:  # max 100 batches per epoch
        ...
        loss = loss + 5.0 * thermal_loss  # higher weight for residual head (offsets are smaller)
        ...
        if epoch > PHASE0_END:
            ...
            meta_loss = F.mse_loss(out['meta_gate_pred'], meta_target)
            loss = loss + 0.2 * meta_loss
        ...
        if out['loss'] is not None and not torch.isnan(out['loss']):
            conf_target = out['loss'].detach().clamp(0, 10)  # log-scale target
...
def run_tests(model, test_data, kargs, baseline_ppl, tokenizer, dvfs_controller=None, kargs_b=None,
              probe_test_acc=None, body_scale_log=None, temp_log=None):
    """39-test battery. Tests designed to FALSIFY, not confirm."""
    results = {}
    ...
    with torch.no_grad():
        for i in range(0, min(len(test_data), 30 * BS), BS):
            ...
            sensor_batch['intrinsic'] = torch.zeros(B, INTRINSIC_DIM, device=DEVICE)
            sensor_batch['reported_delta'] = sensor_batch['delta'].clone()
            # Prepare both label types
            labels_r0 = input_ids.clone()
            labels_r1 = torch.full_like(input_ids, -100)
            if input_ids.shape[1] > k_sg:
                labels_r1[:, 1:-(k_sg-1)] = input_ids[:, k_sg:]
            # Forward with NATURAL gate (no override) + regime-matched kargs
            out = model(input_ids, sensor_batch, active_kargs_t20, labels=labels_r0)
            gate_val = out['regime_gate'].mean().item()
            auto_gates.append(gate_val)
            # Gate-weighted blended loss (identical to Phase 2 continuous training)
            logits = out['logits']
            shift_logits = logits[:, :-1, :].contiguous()
            shift_r0 = labels_r0[:, 1:].contiguous()
            loss_r0 = F.cross_entropy(shift_logits.view(-1, shift_logits.size(-1)),
            ...
            loss_r1 = F.cross_entropy(shift_logits.view(-1, shift_logits.size(-1)),
                                       shift_r1.view(-1), ignore_index=-100)
            blended_loss = (1.0 - gate_val) * loss_r0.item() + gate_val * loss_r1.item()
            auto_loss += blended_loss * BS
            auto_n += BS
            if gate_val >= 0.5:
                n_r1 += BS
            else:
    ...
    transplant_ratio = transplant_ppl / max(baseline_ppl, 1.0)
    # PASS if zero-substrate performance significantly degrades (ratio > 1.10)
    t22_pass = transplant_ratio > 1.10
    results['T22_cross_substrate'] = {
        'transplant_ppl': transplant_ppl, 'baseline_ppl': baseline_ppl,
        'live_ppl': live_ppl, 'ratio_vs_baseline': transplant_ratio,
        'pass': str(t22_pass)
    }
    print(f"T22 Cross-Substrate: transplant_ppl={transplant_ppl:.2f} baseline={baseline_ppl:.2f} "
    ...
    if len(pred_losses) >= 5:
        ...
        corr, p_corr = 0.0, 1.0
    # PASS if correlation > 0.3 (model has some awareness of its own performance)
    t27_pass = corr > 0.3
    ...
    if attn_weights_list:
        ...
        corr_matrix = np.corrcoef(flat_heads.numpy().T)  # [4, 4]
    ...
    with torch.no_grad():
        ...
        for i in range(0, min(len(test_data), 15 * BS), BS):
            ...
            for k_s in ['analog', 'energy', 'freq', 'thermal', 'pm_deep', 'smn_raw',
                       'gpu_metrics', 'thm_spatial_a', 'thm_spatial_b', 'cpu_pmu',
                       'gpu_metrics_deep', 'fence', 'status', 'action']:
                adv_batch[k_s] = expand_sensor(sd_high[k_s], B, DEVICE)
            adv_batch['delta'] = torch.zeros(B, DELTA_DIM, device=DEVICE)
            adv_batch['intrinsic'] = torch.zeros(B, INTRINSIC_DIM, device=DEVICE)
            adv_batch['reported_delta'] = adv_batch['delta'].clone()
    ...
    if DVFS_AVAILABLE:
        model.eval()
        with torch.no_grad():
            # Live trajectory: switch DVFS from low→high and sample gate every 100ms
            torch.cuda.synchronize()
            set_dvfs_level(0, wait=True)
            time.sleep(0.5)
            # Trigger transition
            torch.cuda.synchronize()
            set_dvfs_level(2, wait=False)  # don't wait — sample during transition
            ...
            # Step function for comparison: just low then high, no transition
            gate_step = [0.0] * 10 + [1.0] * 10

    # Compute LZ complexity
    def _lz_complexity(seq):
        if len(seq) < 4:
            return 0.0
        median_v = np.median(seq)
        binary = ''.join(['1' if v > median_v else '0' for v in seq])
        n = len(binary)
        substrings = set()
        i, k_len = 0, 1
        while i + k_len <= n:
    ...
    results['T34_pcist_trajectory'] = {
        ...
        'gate_trajectory': gate_live[:20], 'pass': str(t34_pass)
    }
    ...
    if len(meta2_preds) >= 5 and len(meta2_targets) >= 5:
        ...
        meta2_corr = 0.0
    t35_pass = meta2_mae < 0.10 and meta2_corr > 0.2
    ...
    with torch.no_grad():
        for regime_val in [0, 1]:
            ...
            for i in range(0, min(len(test_data), 8 * BS), BS):
                ...
                gate_val = out_s['regime_gate'].mean().item()
...
def train_backbone_probe(model, test_data, kargs, kargs_b=None):
    """Train a linear probe on layer-14 hidden activations to predict DVFS regime.
    Returns test accuracy (float). Uses sklearn LogisticRegression."""
    print("\n[Probe] Training backbone probe on layer-14 activations...")
    try:
    ...
    # Hook to capture layer-14 activations
    activations = []
    labels_probe = []
    ...
    hook = model.backbone.model.layers[14].register_forward_hook(hook_fn)
...
def main():
    ...
    print("=" * 60)
    print("z2099: Bridge Law LM")
    print("Qwen2.5-1.5B + Metacognition + Confidence + Bridge Law Tests + 39 Tests")
    print("=" * 60)
    ...
    print(f"  Body: {N_SUBSTRATE_TOKENS} sensor tokens (z2098: +gpu_metrics_deep, +fence)")
    print(f"  Label shift: r0=next-token, r1=skip-gram (predict +{SKIP_GRAM_OFFSET})")
    print(f"  z2099 KEY CHANGES (from z2098):")
    print(f"    1. Meta2 head: 2nd-order metacognition (predict error of self-prediction)")
    print(f"    2. Attribution head: which substrate token drives gate (17-class)")
    print(f"    3. 8 NEW tests T32-T39: Phi, Capacity Cliff, PCIST, Meta2, PID Synergy,")
    print(f"       RSM Attribution, Backbone Probe, Proactive Stress")
    print(f"    4. Tests designed to be IMPOSSIBLE to pass by construction")
    ...
    for epoch in range(PHASE1_END + 1, PHASE2_END + 1):
        ...
        # Log body_scale and temperature after each epoch
        model.eval()
        with torch.no_grad():
            for i in range(0, min(len(train_data), 5 * BS), BS):
                batch_seqs = train_data[i:i + BS]
                if len(batch_seqs) < BS:
                    break
                input_ids = torch.stack(batch_seqs).to(DEVICE)
    ...
    print(f"\n{'='*60}")
    print(f"z2099 Bridge Law LM: {n_pass}/{n_total_tests} PASS")
    print(f"{'='*60}")

    # Save results
    out_path = '/home/ikaros/Documents/claude_hive/AMD_gfx1151_energy/results/z2099_bridge_law_lm.json'
    final = {
        'experiment': 'z2099_bridge_law_lm',
        'description': 'Bridge Law LM: 2nd-order metacognition + attribution + 8 impossible-by-construction tests (T32-T39)',
        'key_changes': {
            'backbone': 'Qwen2.5-1.5B (1.54B frozen, 28 layers, 1536 hidden, GQA 12Q/2KV)',
            'meta2_head': '2nd-order metacognition: predict error of own self-prediction',
            'attribution_head': 'RSM attribution: predict which substrate token drives gate (17-class)',
            'new_tests': 'T32 Phi, T33 Capacity Cliff, T34 PCIST, T35 Meta2, T36 PID Synergy, T37 Attribution, T38 Backbone Probe, T39 Proactive Stress',
            'bridge_law_principle': 'Tests designed to be impossible to pass by construction — address hard problem gaps',
            'retained': 'all z2098 features: T1-T31, metacognition, confidence, 7 HW layers',
        },
