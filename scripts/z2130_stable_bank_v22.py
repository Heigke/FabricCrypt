#!/usr/bin/env python3
"""
z2130v22: Stable Divergent Banks on v21 Base
=============================================
v22: Fix T7b (dual-bank kill-shot) by ensuring banks DIVERGE during training.

  v21 was 25-26/40 with T7b=1.000 (banks converged identically).
  v22 changes:
  1. Opposite-sign bank init: cold_B = +randn*0.01, hot_B = -randn*0.01
     (10x larger init + opposite signs → gradient must overcome gap)
  2. Training seed: torch.manual_seed(42) before training for reproducibility
  3. 300 batches (was 200) — more training for stable regime classifier
  4. All v21 test mechanics preserved (T7b, T34 freeze, T38/T34 thresholds)
...
Hardware setup:
  ...
  sudo HSA_OVERRIDE_GFX_VERSION=11.0.0 PYTORCH_ROCM_ARCH=gfx1100 \\
    venv/bin/python -u scripts/z2130_stable_bank_v22.py
"""
...
class BodyGatedLoRA(nn.Module):
    ...
    def __init__(self, analog_linear, layer_idx, rank=8, alpha=16):
        ...
        # v22: DUAL LoRA_B with OPPOSITE-sign init to ensure divergence
        _init_B = torch.randn(out_f, rank, dtype=dtype) * 0.01
        self.lora_B_cold = nn.Parameter(_init_B.clone())
        self.lora_B_hot = nn.Parameter(-_init_B.clone())  # opposite sign!
        ...
        # v21: Flags for test mechanics
        self._force_wrong_gate = False   # flip gate for Phase B kill-shot
    ...
    def forward(self, x):
        ...
        # v21: Allow frozen gate inputs for determinism tests
        ...
        # v21: Force wrong gate for Phase B kill-shot test
...
def run_phase_a(model, test_data, test_data_code, baseline_ppl):
    ...
    # T6: Determinism — v21: freeze gate inputs for true determinism
...
def run_phase_b(model, train_data, test_data, test_data_code, baseline_ppl, phase_a_results):
    ...
    print("  Phase1: Calibration 0-60 | Phase2: Reflex 60-180 | Phase3: Integration 180-300")
    ...
    N_TRAIN_BATCHES = 300  # v22: more batches for stable regime classifier
    LR = 1e-4
    torch.manual_seed(42)  # v22: reproducible training
    ...
    for batch_idx in range(N_TRAIN_BATCHES):
        ...
        # v22: scaled phase boundaries for 300 batches (was 40/120/200)
        if batch_idx < 60:
...
def _run_phase_b_tests(model, soma_head, soma_probe, test_data, test_data_code, baseline_ppl,
    ...
    # v21: T7b — Phase B Kill-Shot (flip dual-bank gate after training)
    ...
    # v21: T26d removed — WGP correction always produces ratio=1.000
...
def _test_causal_ablation_v6(model, test_data):
    """v21: Zero ALL body inputs (fast+mid+slow) — complete ablation."""
    set_analog_mode_override(model, ANALOG_LAYERS, None)
    METAB.update_context()
...
def _test_conditional_determinism(model, test_data):
    ...
    v21: Freeze gate inputs + manual seed to eliminate telemetry variance."""
    if PULSE_BUFFERS is None:
        return {'mean_diff': 999.0}
    set_analog_mode_override(model, ANALOG_LAYERS, None)
    METAB.update_context()

    # v21: Freeze gate inputs so telemetry drift doesn't affect replays
    for layer_idx in LORA_LAYERS:
        g = model.transformer.h[layer_idx].mlp.c_fc
        if isinstance(g, BodyGatedLoRA):
            fast_vec = CTX.get_per_layer_fast_vec(layer_idx)
...
def score_tests(phase_a, phase_b, baseline_ppl):
    ...
    if phase_b:
        ...
        # v21: T7b — Phase B kill-shot (flip dual-bank gate)
        ...
        # v21: T26d removed — WGP correction always ratio=1.000
...
def main():
    ...
    print("=" * 60)
    print("z2130v22: Stable Divergent Banks on v21 Base")
    print("Opposite-sign bank init + seed + 300 batches + v21 test mechanics")
    ...
    print(f"\n" + "=" * 60)
    print(f"z2130v22 Stable Divergent Banks GPT-2 LM: {n_pass}/{n_total} PASS")
    print("=" * 60)

    # Save results
    out_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'z2130_stable_bank.json')
    out_path = os.path.abspath(out_path)
    result_obj = {
        'experiment': 'z2130v22_stable_divergent_banks',
        'description': (
            'v22: Opposite-sign bank init (cold=+, hot=-) to ensure divergence, '
            'torch.manual_seed(42) for reproducibility, 300 training batches, '
            'all v21 test mechanics (T7b, frozen gate, thresholds).'
        ),
