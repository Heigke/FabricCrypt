...
def _smoke():
    ...
    cpuf = ControlledPUF(raw_puf, host="hostA", K_chip=chip_seed)
    n = rng.bytes(8)
    out1 = cpuf.query(n)
    out2 = cpuf.query(n)
    ...
    cpuf2 = ControlledPUF(raw_puf, host="hostA", K_chip=chip_seed2)
    out4 = cpuf2.query(n)
    print("diff K_chip, same nonce → diff wrapped?", out1['wrapped'] != out4['wrapped'])
    print("ID(chip1):", chip_id("hostA", chip_seed).hex()[:16])
    print("ID(chip2):", chip_id("hostA", chip_seed2).hex()[:16])
